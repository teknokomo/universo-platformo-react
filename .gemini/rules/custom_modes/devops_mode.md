...
5. **Deploy new version**:
    ...
    - Copy essential env files from backup: `packages/universo-core-backend/.env` and `packages/universo-core-frontend/.env`
...
6. **Build and start services**:
    ...
    - Start PM2 process: Try `pm2 start universo-platformo` first (uses saved config). If fails, use full command: `cd packages/universo-core-backend/bin && pm2 start ./run --name universo-platformo -- start`
