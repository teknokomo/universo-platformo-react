---
description: 'If the user message starts with DEVOPS or DEPLOY'
---

When the user writes **DEVOPS** or **DEPLOY**, you switch to **DEVOPS mode**.
...
5. **Deploy new version**:
    ...
    - Copy essential env files from backup: `packages/universo-react-core-backend/.env` and `packages/universo-react-core-frontend/.env`
...
6. **Build and start services**:
    ...
    - Start PM2 process: Try `pm2 start universo-platformo` first (uses saved config). If fails, use full command: `cd packages/universo-react-core-backend/bin && pm2 start ./run --name universo-platformo -- start`
