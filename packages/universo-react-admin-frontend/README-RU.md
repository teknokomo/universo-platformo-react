...
```bash
# Сборка пакета
pnpm --filter @universo-react/admin-frontend build

# Режим разработки
pnpm --filter @universo-react/admin-frontend dev
```
...
```tsx
import { AdminAccess, AdminBoard } from '@universo-react/admin-frontend'
...
```tsx
import { useIsSuperadmin, useGrantGlobalRole } from '@universo-react/admin-frontend'
...
```typescript
import { adminQueryKeys } from '@universo-react/admin-frontend'
...
```typescript
import { createAdminApi } from '@universo-react/admin-frontend'
...
-   `@universo-react/admin-backend` - Backend API для админ-панели
-   `@universo-react/auth-frontend` - Хуки аутентификации
