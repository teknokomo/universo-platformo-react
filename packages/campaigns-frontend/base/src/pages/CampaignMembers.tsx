...
const CampaignMembers = () => {
    ...
    const { user } = useAuth()
    const { t, i18n } = useTranslation(['campaigns', 'roles', 'common', 'flowList'])
    ...
    const paginationResult = usePaginated<CampaignMember, 'email' | 'role' | 'created'>({
        queryKeyFn: (params) => campaignsQueryKeys.membersList(campaignId!, params),
        queryFn: (params) => campaignsApi.listCampaignMembers(campaignId!, params),
        initialLimit: 20,
        sortBy: 'created',
        sortOrder: 'desc',
        enabled: !!campaignId
    ...
    const handleInviteMember = async (data: { email: string; role: AssignableRole; comment?: string }) => {
        ...
        try {
            ...
            } else if (isHttpStatus(error, 409) && isApiError(error, 'CAMPAIGN_MEMBER_EXISTS')) {
                message = t('campaigns:members.userAlreadyMember', { email: data.email })
            } else {
    ...
    const handleChange = (_event: React.MouseEvent<HTMLElement>, nextView: string | null) => {
        ...
        localStorage.setItem('campaignMembersDisplayStyle', nextView)
        setView(nextView)
    }
    ...
    const memberColumns = [
        ...
        },
        {
            ...
            align: 'center',
            render: (row: CampaignMember) => {
                const roleKey = (row.role || 'member') as CampaignRole
    ...
    ] satisfies TableColumn<CampaignMember>[]

    const createMemberContext = useCallback(
        (baseContext: Partial<ActionContext<CampaignMember, MemberData>>): ActionContext<CampaignMember, MemberData> => ({
            ...baseContext,
            activity: baseContext.activity!,
            activityKind: 'member',
            t: baseContext.t!,
            api: {
                updateActivity: async (id: string, data: MemberData) => {
                    if (!campaignId) return
                    // Validate data
            ...
            helpers: {
                ...
                openDeleteDialog: (member: CampaignMember) => {
    ...
    if (!campaignId) {
        return (
            <MainCard
                ...
                shadow={false}
            >
                <EmptyListState image={APIEmptySVG} imageAlt='Invalid campaign' title={tc('errors.connectionFailed')} />
            </MainCard>
        )
    }
                            ...
                            {view === 'card' ? (
                                                ...
                                                headerAction={
                                                    descriptors.length > 0 ? (
                                                        <Box onClick={(e) => e.stopPropagation()}>
                                                            <BaseEntityMenu<CampaignMember, MemberFormData>
                                                                entity={member}
                                                                entityKind='member'
                                ...
                                <Box sx={{ mx: { xs: -1.5, md: -2 } }}>
                                    <FlowListTable
                                        ...
                                        renderActions={(row: CampaignMember) => {
                                            // Owner role cannot be edited or removed
                                            if (row.role === 'owner') {
                                                return null
                                            }
