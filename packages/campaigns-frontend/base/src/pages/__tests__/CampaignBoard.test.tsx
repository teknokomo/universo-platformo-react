...
const renderWithProviders = (ui: React.ReactElement, { route = '/campaigns/test-campaign-id/board' }: RenderOptions = {}) => {
                ...
                <SnackbarProvider maxSnack={3}>
                    <I18nextProvider i18n={i18n}>
                        <MemoryRouter initialEntries={[route]}>
                            <Routes>
                                <Route path='/campaigns/:campaignId/board' element={ui} />
                            </Routes>
                        </MemoryRouter>
                    </I18nextProvider>
                </SnackbarProvider>
...
describe('CampaignBoard', () => {
    ...
    describe('Loading State', () => {
        it('should display loading indicator while fetching data', () => {
            ...
            vi.mocked(campaignsApi.getCampaign).mockImplementation(
                () => new Promise(() => {}) // Never resolves
            )

            renderWithProviders(<CampaignBoard />)
    ...
    describe('Error State', () => {
        it('should display error message when API fails', async () => {
            ...
            vi.mocked(campaignsApi.getCampaign).mockRejectedValue(new Error('Network error'))

            renderWithProviders(<CampaignBoard />)

            // Wait for error state
            await waitFor(() => {
                expect(screen.getByText(/failed to load campaign data/i)).toBeInTheDocument()
            })
        ...
        it('should display generic error when no campaign data returned', async () => {
            // Mock API to return null/undefined data
            vi.mocked(campaignsApi.getCampaign).mockResolvedValue({ data: null } as any)
    ...
    describe('Success State - Dashboard Rendering', () => {
        ...
        beforeEach(() => {
            vi.mocked(campaignsApi.getCampaign).mockResolvedValue({
                data: mockCampaignData
            })
        })

        it('should render campaign name and description in header', async () => {
            renderWithProviders(<CampaignBoard />)

            await waitFor(() => {
                expect(screen.getByText('Test Campaign')).toBeInTheDocument()
                expect(screen.getByText('A test campaign for unit tests')).toBeInTheDocument()
            })
        })
        ...
        it('should render activity and activities charts', async () => {
            renderWithProviders(<CampaignBoard />)

            await waitFor(() => {
                // Charts should be present (check for Overview event)
    ...
    describe('User Interactions', () => {
        ...
        it('should open documentation in new tab when button clicked', async () => {
            const { user } = renderWithProviders(<CampaignBoard />)
    ...
    describe('Edge Cases', () => {
        it('should handle campaign with zero counts', async () => {
            vi.mocked(campaignsApi.getCampaign).mockResolvedValue({
                data: {
                    id: 'empty-campaign',
                    name: 'Empty Campaign',
        ...
        it('should handle campaign without description', async () => {
            ...
            expect(screen.getByText(/campaign analytics and statistics/i)).toBeInTheDocument()
        })
    ...
    describe('Navigation', () => {
        it('should handle invalid campaignId parameter', () => {
            // Render without campaignId in route
            vi.mocked(campaignsApi.getCampaign).mockImplementation(() => new Promise(() => {}))

            renderWithProviders(<CampaignBoard />, { route: '/campaigns//board' })

            // Should not call API without valid ID
            expect(campaignsApi.getCampaign).not.toHaveBeenCalled()
        })
    })
})
