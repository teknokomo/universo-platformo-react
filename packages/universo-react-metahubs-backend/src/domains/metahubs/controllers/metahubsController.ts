...
import { type MetahubRuntimePolicySnapshot, type VersionedLocalizedContent } from '@universo-react/types'
import { validateListQuery } from '../../shared/queryParams'
import { sanitizeLocalizedInput, buildLocalizedContent, ensureVLC } from '@universo-react/utils/vlc'
...
import type { CodenameStyle, CodenameAlphabet } from '@universo-react/types'
...
const toMetahubCountsSummary = (entityCounts: Record<string, number>): { treeEntitiesCount: number; objectCollectionsCount: number } => ({
    treeEntitiesCount: getEntityCount(entityCounts, 'hub'),
    objectCollectionsCount: getEntityCount(entityCounts, 'object')
})
...
const cleanupImportedMetahubInTx = async (tx: DbExecutor, metahubId: string, userId: string): Promise<void> => {
    ...
    await softDelete(tx, 'metahubs', 'obj_metahubs', metahubId, userId)
}
...
export function createMetahubsController(getDbExecutor: () => DbExecutor) {
    ...
    const list = async (req: Request, res: Response) => {
        ...
        const result = metahubs.map((m, index) => {
            ...
            return {
                ...
                objectCollectionsCount: counts.objectCollectionsCount,
    ...
    const getById = async (req: Request, res: Response) => {
        ...
        let objectCollectionsCount = 0
        try {
            objectCollectionsCount = await objectsService.countByKind(metahubId, 'object', userId)
        } catch {
        ...
        try {
            const copied = await exec.transaction(async (tx) => {
                ...
                for (const planItem of branchClonePlan) {
                    ...
                    await tx.query(`UPDATE metahubs.obj_metahub_branches SET source_branch_id = $1, _upl_updated_by = $2 WHERE id = $3`, [
                        mappedSourceId,
                        userId,
                        branchId
                    ])
