...
import { SHARED_ENTITY_KIND_TO_POOL_KIND, type FixedValueDataType, type SharedBehavior } from '@universo-react/types'
...
const ALLOWED_CONSTANT_TYPES: FixedValueDataType[] = ['STRING', 'NUMBER', 'BOOLEAN', 'DATE']
...
const normalizeCompatibleTargetKinds = (targetKinds: readonly string[] | undefined, fallbackKind: string): string[] => {
    const normalized = Array.from(new Set(targetKinds?.map((kind) => kind.trim()).filter((kind) => kind.length > 0) ?? [fallbackKind]))
...
export class MetahubFixedValuesService {
    ...
    async findAll(metahubId: string, valueGroupId: string, userId?: string) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const rows = await this.listRowsByObjectId(schemaName, valueGroupId, this.exec)
        return rows.map((row) => this.mapRowToFixedValue(row))
    }

    async findAllMerged(metahubId: string, valueGroupId: string, userId?: string, db?: SqlQueryable) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const run = async (runner: SqlQueryable) => {
            await this.ensureSequentialSortOrder(schemaName, valueGroupId, runner)

            const sharedContainerService = new SharedContainerService(this.exec, this.schemaService)
            const sharedValueGroupId = await sharedContainerService.findContainerObjectId(
                metahubId,
            ...
            const overrides = await sharedOverridesService.findByTargetObject(metahubId, 'constant', valueGroupId, userId, runner)

            const localItems = (await this.listRowsByObjectId(schemaName, valueGroupId, runner)).map((row) => this.mapRowToFixedValue(row))
            const sharedItems = sharedValueGroupId
                ? (await this.listRowsByObjectId(schemaName, sharedValueGroupId, runner)).map((row) => this.mapRowToFixedValue(row))
                : []
    ...
    async findById(metahubId: string, id: string, userId?: string, db?: SqlQueryable, options?: { includeDeleted?: boolean }) {
        ...
        return row ? this.mapRowToFixedValue(row) : null
    }

    async findByCodename(metahubId: string, valueGroupId: string, codename: string, userId?: string, db?: SqlQueryable) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const qt = qSchemaTable(schemaName, '_mhb_constants')
    ...
    async create(
        ...
        const sortOrder = data.sortOrder ?? (await this.getNextSortOrder(schemaName, data.valueGroupId, runner))
        const qt = qSchemaTable(schemaName, '_mhb_constants')
        const now = new Date()
        ...
        return this.mapRowToFixedValue(row)
    }
    ...
    async update(
        ...
        if (data.expectedVersion !== undefined) {
            ...
            return this.mapRowToFixedValue(updated)
        }

        const updated = await incrementVersion(this.exec, schemaName, '_mhb_constants', id, updateData)
        return updated ? this.mapRowToFixedValue(updated) : null
    }
    ...
    async moveFixedValue(metahubId: string, objectId: string, fixedValueId: string, direction: 'up' | 'down', userId?: string) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const qt = qSchemaTable(schemaName, '_mhb_constants')
        ...
        return this.exec.transaction(async (tx: SqlQueryable) => {
            ...
            const current = await queryOne<Record<string, unknown>>(
                ...
                `SELECT * FROM ${qt}
                 ...
                 LIMIT 1`,
                [fixedValueId, objectId]
            )
            if (!current) throw new MetahubNotFoundError('Constant', fixedValueId)
            ...
            if (!neighbor) {
                return this.mapRowToFixedValue(current)
            }

            await tx.query(`UPDATE ${qt} SET sort_order = $1 WHERE id = $2 RETURNING id`, [neighbor.sort_order, fixedValueId])
    ...
    async reorderConstant(metahubId: string, objectId: string, fixedValueId: string, newSortOrder: number, userId?: string) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const qt = qSchemaTable(schemaName, '_mhb_constants')
        ...
        return this.exec.transaction(async (tx: SqlQueryable) => {
            ...
            const currentIndex = constants.findIndex((item) => item.id === fixedValueId)
            if (currentIndex === -1) throw new MetahubNotFoundError('Constant', fixedValueId)
    ...
    async reorderConstantMergedOrder(
        metahubId: string,
        valueGroupId: string,
        fixedValueId: string,
        ...
        return this.exec.transaction(async (tx: SqlQueryable) => {
            const items = await this.findAllMerged(metahubId, valueGroupId, userId, tx)
            const current = items.find((item) => item.id === fixedValueId)
            if (!current) {
                throw new MetahubNotFoundError('Constant', fixedValueId)
            }
            ...
            const updatedItems = await this.findAllMerged(metahubId, valueGroupId, userId, tx)
            const updated = updatedItems.find((item) => item.id === fixedValueId)
            if (!updated) {
                throw new MetahubNotFoundError('Constant', fixedValueId)
            }
            return updated
    ...
    async ensureUniqueCodenameWithRetries(options: {
        ...
        const { metahubId, valueGroupId, desiredCodename, codenameStyle, userId, db } = options
        ...
        for (let attempt = 1; attempt <= CODENAME_RETRY_MAX_ATTEMPTS; attempt += 1) {
            ...
            const exists = await this.findByCodename(metahubId, valueGroupId, candidate, userId, db)
        ...
        const rows = await this.exec.query<{ id: string }>(
            `SELECT id FROM ${qt}
             WHERE data_type = 'REF'
               ...
               AND ${ACTIVE}
             LIMIT 1`,
            [targetValueGroupId, compatibleTargetKinds, targetConstantId]
        )
        return rows.length > 0
    }

    async belongsToSet(metahubId: string, valueGroupId: string, fixedValueId: string, userId?: string) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const qt = qSchemaTable(schemaName, '_mhb_constants')
    ...
    private mapRowToFixedValue(row: Record<string, unknown>): MetahubConstantRecord {
        const presentation = row.presentation as Record<string, unknown> | null
        return {
            id: row.id as string,
            valueGroupId: row.object_id as string,
            codename: ensureCodenameValue(row.codename),
            dataType: row.data_type as FixedValueDataType,
