...
import { ensureApplicationAccess, type ApplicationRole } from '@universo-react/applications-backend'
import { resolveUserId } from '../../shared/routeAuth'
import { createLogger } from '../../../utils/logger'
import { ApplicationSchemaStatus, type ApplicationMigrationStatusResponse, type StructuredBlocker } from '@universo-react/types'
import { determineSeverity } from '@universo-react/migration-guard-shared/utils'
import {
    findApplicationById,
...
function snapshotToEntities(snapshot: SchemaSnapshot): import('@universo-react/schema-ddl').EntityDefinition[] {
    const entities: import('@universo-react/schema-ddl').EntityDefinition[] = []
    const now = new Date().toISOString()

    const createEmptyPresentation = (codename: string): import('@universo-react/types').MetaPresentation => ({
        name: {
            _schema: '1' as const,
            _primary: 'en',
            locales: {
    ...
    for (const [entityId, entity] of Object.entries(snapshot.entities)) {
        const fields: import('@universo-react/schema-ddl').Component[] = []
