...
import { MetahubOptionValuesService } from '../../domains/metahubs/services/MetahubOptionValuesService'

describe('MetahubOptionValuesService active-row filtering', () => {
    type MockTx = {
    ...
    } as unknown as ConstructorParameters<typeof MetahubOptionValuesService>[1]
    ...
    const service = new MetahubOptionValuesService(
        mockExec as unknown as ConstructorParameters<typeof MetahubOptionValuesService>[0],
        schemaService
    )
    ...
    it('passes the active transaction runner into optimistic locking updates without opening a nested transaction', async () => {
        mockUpdateWithVersionCheck.mockResolvedValueOnce({
            id: 'value-1',
            object_id: 'enum-1',
            codename: 'open-v2',
            presentation: { name: { en: 'Open' }, description: null },
