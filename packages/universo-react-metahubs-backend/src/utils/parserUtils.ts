 ...
 */
export function parseIntSafe(value: unknown, defaultValue: number, min: number, max: number): number {
    const parsed = parseInt(String(value || ''), 10)
    if (!Number.isFinite(parsed)) return defaultValue
    return Math.max(min, Math.min(max, parsed))
}

// Re-export centralized database utilities from @universo-react/utils
export { escapeLikeWildcards, getRequestDbExecutor, getRequestDbSession } from '@universo-react/utils/database'
export type { DbExecutor, SqlQueryable } from '@universo-react/utils/database'
