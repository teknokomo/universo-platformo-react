...
При запуске платформы `@universo-react/core-backend` выполняет fixed system-app pipeline для `metahubs`:
...
`@universo-react/metahubs-backend` владеет branch migration endpoints:
...
Они не заменяют runtime sync ownership пакета `@universo-react/applications-backend`.

## Граница ответственности с applications

`@universo-react/metahubs-backend` владеет design-time metadata, branch runtime migrations, publication metadata и migration-control surface.
`@universo-react/applications-backend` владеет runtime application schema sync, diff calculation, release-bundle export/apply и сохранением `installed_release_metadata` в `applications.obj_applications`.
Publication-driven application creation пересекает эту границу, но финальная runtime sync route surface остаётся в `@universo-react/applications-backend`.
