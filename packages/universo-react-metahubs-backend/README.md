...
-   Standard metadata entity types use direct kind keys: `hub`, `page`, `object`, `set`, `enumeration`, and `ledger`.
...
-   The built-in `lms` template is entity-first: classes, students, content projects, resources, courses, course sections, course items, learning tracks, track stages, track steps, quizzes, assignments, events, certificates, reports, knowledge spaces, development plans, access links, progress, enrollments, sharing/star/recent/trash projections, supporting enumerations, and Object-backed ledgers are ordinary seeded metadata entities.
-   LMS ledgers are exported as metadata with `config.ledger`; operational ledger facts are not exported as normal Object runtime rows.
...
-   Runtime schema generation, diffing, and rollback primitives come from `@universo-react/schema-ddl`.
-   The package consumes the shared runtime from `@universo-react/database`; it does not own a private Knex singleton.
-   Publication routes may trigger downstream application sync seams, but application runtime route ownership stays in `@universo-react/applications-backend`.
...
-   `@universo-react/migrations-core` and `@universo-react/migrations-catalog` for migration planning and lifecycle tracking.
-   `@universo-react/schema-ddl` for runtime schema generation and DDL execution.
-   `@universo-react/applications-backend` for downstream application runtime sync ownership.
