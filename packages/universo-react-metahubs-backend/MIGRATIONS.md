...
On platform startup `@universo-react/core-backend` runs the fixed system-app pipeline for `metahubs`:
...
`@universo-react/metahubs-backend` owns the branch migration endpoints:
...
They do not replace the runtime sync ownership of `@universo-react/applications-backend`.

## Ownership Boundary With Applications

`@universo-react/metahubs-backend` owns design-time metadata, branch runtime migrations, publication metadata, and the migration-control surface.
`@universo-react/applications-backend` owns runtime application schema sync, diff calculation, release-bundle export/apply, and persistence of `installed_release_metadata` in `applications.obj_applications`.
Publication-driven application creation crosses this boundary, but the final runtime sync route surface stays in `@universo-react/applications-backend`.
