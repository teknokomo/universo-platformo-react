...
const AUTH_DEBUG_ID = (() => {
    try {
        ...
        return `auth-frt:${(hash >>> 0).toString(16)}`
    } catch {
        return `auth-frt:${Math.random().toString(36).slice(2)}`
    }
})()
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ client, children }) => {
    ...
    if (import.meta.env?.MODE !== 'production' && typeof window !== 'undefined') {
        ...
        if (w[KEY]) {
            // eslint-disable-next-line no-console
            console.warn('[auth] Multiple copies of @universo/auth-frt detected', { moduleId: import.meta.url })
        }
        w[KEY] = true
    }
