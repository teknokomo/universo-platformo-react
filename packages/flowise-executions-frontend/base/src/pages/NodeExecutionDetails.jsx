...
// Workspace imports (adapted from @/ aliases)
import { MemoizedReactMarkdown } from '@flowise/template-mui/ui-components/markdown/MemoizedReactMarkdown'
import { SafeHTML } from '@flowise/template-mui/ui-components/safe/SafeHTML'
import { AGENTFLOW_ICONS, baseURL } from '@flowise/template-mui/constants'
import { JSONViewer } from '@flowise/template-mui/ui-components/json/JsonViewer'
import ReactJson from 'flowise-react-json-view'
import { CodeEditor } from '@flowise/template-mui/ui-components/editor/CodeEditor'
import SourceDocDialog from '@flowise/template-mui/ui-components/dialog/SourceDocDialog'
...
export const NodeExecutionDetails = ({ data, label, status, metadata, isPublic, onProceedSuccess }) => {
    ...
    const onSubmitResponse = async (type, feedback = '') => {
        ...
        try {
            let response
            if (isPublic) {
                response = await api.predictions.sendMessageAndGetPredictionPublic(metadata?.agentflowId, params)
            } else {
                response = await api.predictions.sendMessageAndGetPrediction(metadata?.agentflowId, params)
            }
