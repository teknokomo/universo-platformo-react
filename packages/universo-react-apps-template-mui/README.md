...
-   **Entity-owned content**: The published application runtime receives localized records from the `marketing-page` metahub template; no section owns a hardcoded demo array. `MarketingPageHero` owns Hero copy, while its widget placement owns only composition and presentation.
...
-   **Application appearance**: Theme mode, bounded colors, brand media, and action policy are configured in the typed application layout; persisted marketing widget instances own zone, order, active state, and presentation flags. Entity-backed placements carry validated semantic bindings; source-backed widgets keep their source filters.
-   **Hero runtime projection**: Authenticated and public runtimes resolve the bound `MarketingPageHero` Object into one bounded `heroContent` record inside the established `data.records` envelope. The DTO contains localized fields and typed actions, never storage IDs or duplicate Site Settings content.
