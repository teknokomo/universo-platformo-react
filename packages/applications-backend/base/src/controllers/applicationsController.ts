...
import {
  isSuperuser as isSuperuserCheck,
...
export function createApplicationsController(getDbExecutor: () => DbExecutor) {
  ...
  const hasGlobalApplicationAdminAccess = async (
    executor: DbExecutor,
    userId: string
  ): Promise<boolean> => {
  ...
  const list = async (req: Request, res: Response) => {
    ...
    const hasGlobalApplicationsReadAccess = await hasSubjectPermission(
      ds,
      userId,
      'applications',
      'read'
    )
    const hasGlobalApplicationsAdminAccess = isSuperuser
      ? true
      : await hasGlobalApplicationAdminAccess(ds, userId)
    ...
    const { items: applications, total } = await listApplications(
      ...
      },
      {
        ...
        offset,
        sortBy:
          sortBy === 'name' || sortBy === 'created' || sortBy === 'updated' ? sortBy : 'updated',
        sortOrder,
        search: escapedSearch
      }
    )

    const elevatedGlobalRoleName = hasGlobalApplicationsAdminAccess
      ? await getGlobalRoleCodename(ds, userId)
      : null

    const result = applications.map((a) => {
      const membershipState = a.membershipRole
        ? ApplicationMembershipState.JOINED
        : ApplicationMembershipState.NOT_JOINED
      const role = a.membershipRole
        ? (a.membershipRole as ApplicationRole)
      ...
      return {
        ...
        membershipState,
        canJoin:
          membershipState === ApplicationMembershipState.NOT_JOINED &&
          a.isPublic === true &&
          !elevatedGlobalRoleName,
  ...
  const create = async (req: Request, res: Response) => {
    ...
    if (Object.keys(sanitizedName).length === 0) {
      return res
        .status(400)
        .json({ error: 'Validation failed', details: { name: ['Name is required'] } })
    }
    ...
    try {
      ...
      saved = await createApplicationWithOwner(getRequestDbExecutor(req, getDbExecutor()), {
        ...
        validateSchemaName: (schemaName) =>
          schemaName.startsWith('app_') &&
          isValidSchemaName(schemaName) &&
          IDENTIFIER_REGEX.test(schemaName)
      })
    } catch (error) {
  ...
  const copy = async (req: Request, res: Response) => {
    ...
    const resolvedWorkspacesEnabled =
      parsed.data.workspacesEnabled ?? sourceApplication.workspacesEnabled
    ...
    const nameVlc = buildLocalizedContent(
      ...
      sourceApplication.name?._primary ?? 'en'
    )
    if (!nameVlc) {
      return res
        .status(400)
    ...
    let descriptionVlc: VersionedLocalizedContent<string> | null =
      sourceApplication.description ?? null
    ...
    if (requestedSlug) {
      ...
      if (!hasSlugCandidate) {
        return res
          .status(409)
          .json({ error: 'Unable to generate unique slug for copied application' })
      }
    }

    const [{ id: newApplicationId }] = (await query<{ id: string }>(
      req,
    ...
    if (
      !newSchemaName.startsWith('app_') ||
      !isValidSchemaName(newSchemaName) ||
      !IDENTIFIER_REGEX.test(newSchemaName)
    ) {
      return res.status(400).json({ error: 'Invalid generated application schema name' })
    }
    ...
    const runCopyTransaction = () =>
      copyApplicationWithOptions(ds, {
        ...
        schemaStatus: copyOptions.copyConnector
          ? ApplicationSchemaStatus.OUTDATED
          : ApplicationSchemaStatus.DRAFT,
  ...
  const getLimits = async (req: Request, res: Response) => {
    ...
    if (!application.workspacesEnabled)
      return res.status(400).json({ error: 'Workspace limits require workspace mode' })
    ...
    const { items: members, total } = await listApplicationMembers(
      ...
      },
      {
        ...
        search: validatedQuery.search
          ? escapeLikeWildcards(validatedQuery.search.toLowerCase())
          : undefined
      }
    )
    return res.json({
  ...
  const addMember = async (req: Request, res: Response) => {
    ...
    const normalizedComment = normalizeMemberCommentInput(
      result.data.comment,
      result.data.commentPrimaryLocale
    )
    ...
    if (existing) {
      return res
        .status(409)
        .json({ error: 'User is already a member', code: 'APPLICATION_MEMBER_EXISTS' })
    }
  ...
  const updateMember = async (req: Request, res: Response) => {
    ...
    const updatedMember = await updateApplicationMember(executor, {
      ...
      comment:
        result.data.comment !== undefined ? normalizedComment.commentVlc ?? null : undefined,
      updatedBy: userId
    })
