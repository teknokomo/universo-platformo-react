/**
 * MSW (Mock Service Worker) handlers for organizations API
 *
...
export const handlers = [
    ...
    http.get(`${API_BASE_URL}/organizations/:id`, async ({ params }) => {
    ...
    // List organizations
    http.get(`${API_BASE_URL}/organizations`, async ({ request }) => {
        await delay(50)
        ...
        let filteredData = [...mockOrganizationsList]
    ...
    // List departments
    http.get(`${API_BASE_URL}/organizations/:organizationId/departments`, async ({ request }) => {
        await delay(50)
        ...
        const paginatedData = mockDepartmentsList.slice(offset, offset + limit)

        return HttpResponse.json({
            data: paginatedData,
            pagination: {
                total: mockDepartmentsList.length,
                limit,
                offset,
                count: paginatedData.length,
                hasMore: offset + limit < mockDepartmentsList.length
            }
        })
    }),

    // List positions
    http.get(`${API_BASE_URL}/organizations/:organizationId/departments/:departmentId/positions`, async ({ request }) => {
        await delay(50)
        ...
        const paginatedData = mockPositionsList.slice(offset, offset + limit)

        return HttpResponse.json({
            data: paginatedData,
            pagination: {
                total: mockPositionsList.length,
                limit,
                offset,
                count: paginatedData.length,
                hasMore: offset + limit < mockPositionsList.length
            }
        })
    }),

    // List organization members
    http.get(`${API_BASE_URL}/organizations/:organizationId/members`, async ({ request }) => {
        await delay(50)
    ...
    // XSS Test: Organization with HTML-encoded malicious content for safe testing
    // Tests should verify that this encoded content is NOT executed and displays as text
    http.get(`${API_BASE_URL}/organizations/xss-test`, async () => {
        await delay(50)
...
export const errorHandlers = {
    networkError: http.get(`${API_BASE_URL}/organizations/:id`, () => {
        return HttpResponse.error()
    }),

    timeout: http.get(`${API_BASE_URL}/organizations/:id`, async () => {
        ...
        await delay(4000)
        return HttpResponse.json({ data: mockOrganizationData })
    }),

    unauthorized: http.get(`${API_BASE_URL}/organizations/:id`, () => {
        return HttpResponse.json({ message: 'Unauthorized' }, { status: 401 })
    }),

    forbidden: http.get(`${API_BASE_URL}/organizations/:id`, () => {
