...
import { useOrganizationDetails } from '../api/useOrganizationDetails'

/**
 * Organization Board Page
 *
 * Displays analytics dashboard for a organization with:
 * - Real-time statistics (departments, positions, members)
 * - Documentation positions
 * - Activity and position charts (demo data)
 *
 * Layout:
...
const OrganizationBoard = () => {
    ...
    // Fetch organization details with TanStack Query
    const {
        data: organization,
        isLoading,
        error,
    ...
    if (isError || !organization) {
        const errorMessage = error instanceof Error ? error.message : t('board.error', 'Failed to load organization data')

        return (
            <Stack spacing={3} sx={{ maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto', width: '100%', p: 2 }}>
                <EmptyListState
                    image={APIEmptySVG}
                    imageAlt='Error loading organization'
                    title={t('board.error', 'Failed to load organization data')}
                />
    ...
    // Use current position count as trend data (shows actual value in chart)
    const positionsCount = organization.positionsCount ?? 0
    const positionsData = Array(30).fill(positionsCount)
        ...
        <Stack spacing={2} sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto' }}>
            ...
            <Box sx={{ px: { xs: 1.5, md: 2 } }}>
                <ViewHeader
                    ...
                    description={organization.description || t('board.defaultDescription', 'Organization analytics and statistics')}
                    search={false}
                />
            </Box>
            ...
            <Box sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' } }}>
                <Grid container spacing={2} columns={12}>
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <StatCard
                            title={t('board.stats.departments.title')}
                            value={organization.departmentsCount ?? 0}
                            interval={t('board.stats.departments.interval')}
                            description={t('board.stats.departments.description')}
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <StatCard
                            title={t('board.stats.positions.title')}
                            value={organization.positionsCount ?? 0}
                            interval={t('board.stats.positions.interval')}
                            description={t('board.stats.positions.description')}
                            data={positionsData}
                        />
                    </Grid>
                    ...
                    {/* Positions Chart (Demo Data) */}
                    <Grid item xs={12} md={6}>
                        <PageViewsBarChart
                            title={t('board.charts.positions.title')}
                            description={t('board.charts.positions.description')}
                        />
                    </Grid>
                </Grid>
                ...
                <Box display='flex' justifyContent='center' sx={{ mt: 2 }}>
                    <Button variant='text' startIcon={<ArrowBackRoundedIcon />} onClick={() => navigate('/organizations')}>
                        {t('actions.backToList', 'Back to Organizations')}
                    </Button>
                </Box>
            </Box>
        </Stack>
