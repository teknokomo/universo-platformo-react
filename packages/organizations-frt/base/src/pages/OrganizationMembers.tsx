...
const OrganizationMembers = () => {
    ...
    const paginationResult = usePaginated<OrganizationMember, 'email' | 'role' | 'created'>({
        queryKeyFn: (params) => organizationsQueryKeys.membersList(organizationId!, params),
    ...
    const handleChange = (_event: React.MouseEvent<HTMLElement>, nextView: string | null) => {
        ...
        localStorage.setItem('organizationMembersDisplayStyle', nextView)
        setView(nextView)
    }
    ...
    const memberColumns = [
        ...
        },
        {
            ...
            align: 'center',
            render: (row: OrganizationMember) => {
                const roleKey = (row.role || 'member') as OrganizationRole
    ...
    ] satisfies TableColumn<OrganizationMember>[]

    const createMemberContext = useCallback(
        (baseContext: Partial<ActionContext<OrganizationMember, MemberData>>): ActionContext<OrganizationMember, MemberData> => ({
            ...baseContext,
            position: baseContext.position!,
            positionKind: 'member',
            t: baseContext.t!,
            api: {
            ...
            helpers: {
                ...
                openDeleteDialog: (member: OrganizationMember) => {
    ...
    if (!organizationId) {
        return (
            <MainCard
                ...
                shadow={false}
            >
                <EmptyListState image={APIEmptySVG} imageAlt='Invalid organization' title={tc('errors.connectionFailed')} />
            </MainCard>
        )
    }
                            ...
                            {view === 'card' ? (
                                                ...
                                                headerAction={
                                                    descriptors.length > 0 ? (
                                                        <Box onClick={(e) => e.stopPropagation()}>
                                                            <BaseEntityMenu<OrganizationMember, MemberFormData>
                                                                entity={member}
                                                                entityKind='member'
                                ...
                                <Box sx={{ mx: { xs: -1.5, md: -2 } }}>
                                    <FlowListTable
                                        ...
                                        renderActions={(row: OrganizationMember) => {
                                            // Owner role cannot be edited or removed
                                            if (row.role === 'owner') {
                                                return null
                                            }
