...
// Type for campaign update/create data
type CampaignData = {
    name: string
    description?: string
}
...
const CampaignList = () => {
    // Use campaigns namespace for view-specific keys, roles and access for role/permission labels
    const { t, i18n } = useTranslation(['campaigns', 'roles', 'access', 'flowList'])
    ...
    const [view, setView] = useState(localStorage.getItem('activitiesCampaignDisplayStyle') || 'card')

    // State management for dialog
    const [isCreating, setCreating] = useState(false)
    const [dialogError, setDialogError] = useState<string | null>(null)

    // Use paginated hook for campaigns list
    const paginationResult = usePaginated<Campaign, 'name' | 'created' | 'updated'>({
        queryKeyFn: campaignsQueryKeys.list,
    ...
    const { data: campaigns, isLoading, error } = paginationResult
    ...
    useEffect(() => {
        ...
        console.log('[CampaignList Pagination Debug]', {
    ...
    const handleChange = (_event: any, nextView: string | null) => {
        ...
        localStorage.setItem('activitiesCampaignDisplayStyle', nextView)
        setView(nextView)
    }
        ...
        () => [
            {
                ...
                align: 'left',
                render: (row: Campaign) => (
                    <Link to={`/campaign/${row.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                        <Typography
                            sx={{
                                fontSize: 14,
                                fontWeight: 500,
            ...
            },
            {
                ...
                width: '10%',
                align: 'center',
                render: (row: Campaign) => (row.role ? <RoleChip role={row.role} /> : '—')
            },
            {
                id: 'events',
                label: tc('table.events', 'Events'),
                width: '10%',
                align: 'center',
                render: (row: Campaign) => (typeof row.eventsCount === 'number' ? row.eventsCount : '—')
            },
            {
                id: 'activities',
                label: tc('table.activities', 'Activities'),
                width: '10%',
                align: 'center',
                render: (row: Campaign) => (typeof row.activitiesCount === 'number' ? row.activitiesCount : '—')
            }
        ],
        [tc]
    )
    ...
    const createCampaignContext = useCallback(
        (baseContext: any) => ({
            ...baseContext,
            api: {
            ...
            helpers: {
                ...
                openDeleteDialog: (campaign: Campaign) => {
                    setDeleteDialogState({ open: true, campaign })
                }
            }
        }),
    ...
    return (
        ...
        >
            {error ? (
                ...
                <Stack flexDirection='column' sx={{ gap: 1 }}>
                    ...
                    {isLoading && campaigns.length === 0 ? (
                        ...
                        <EmptyListState image={APIEmptySVG} imageAlt='No campaigns' title={t('noCampaignsFound')} />
                    ) : (
                        <>
                            {view === 'card' ? (
                                <Box
                                ...
                                >
                                    {campaigns.map((campaign: Campaign) => {
                                        ...
                                        const descriptors = campaignActions.filter((descriptor) => {
                                            if (descriptor.id === 'edit' || descriptor.id === 'delete') {
                                                return campaign.permissions?.manageCampaign
                                            }
                                            return true
                                        })
                                        ...
                                        return (
                                            <ItemCard
                                                key={campaign.id}
                                                data={campaign}
                                                images={images[campaign.id] || []}
                                                href={`/campaign/${campaign.id}`}
                                                footerEndContent={campaign.role ? <RoleChip role={campaign.role} /> : null}
                                                headerAction={
                                                    descriptors.length > 0 ? (
                                                        <Box onClick={(e) => e.stopPropagation()}>
                                                            <BaseEntityMenu<Campaign, CampaignData>
                                                                entity={campaign}
                                                                entityKind='campaign'
                                ...
                                <Box sx={{ mx: { xs: -1.5, md: -2 } }}>
                                    <FlowListTable
                                        ...
                                        images={images}
                                        isLoading={isLoading}
                                        getRowLink={(row: Campaign) => (row?.id ? `/campaign/${row.id}` : undefined)}
                                        customColumns={campaignColumns}
                                        i18nNamespace='flowList'
                                        renderActions={(row: Campaign) => {
                                            const descriptors = campaignActions.filter((descriptor) => {
                                                if (descriptor.id === 'edit' || descriptor.id === 'delete') {
                                                    return row.permissions?.manageCampaign
                                                }
                                                return true
                                            })
                                            ...
                                            return (
                                                <BaseEntityMenu<Campaign, CampaignData>
                                                    ...
                                                    entityKind='campaign'
                                                    descriptors={descriptors}
                                                    // Use campaigns namespace for action item labels (edit/delete)
            ...
            <EntityFormDialog
                open={isDialogOpen}
                title={t('createCampaign', 'Create Campaign')}
            ...
            <ConfirmDeleteDialog
                ...
                description={t('confirmDeleteDescription', { name: deleteDialogState.campaign?.name || '' })}
                confirmButtonText={tc('actions.delete', 'Delete')}
                deletingButtonText={tc('actions.deleting', 'Deleting...')}
                cancelButtonText={tc('actions.cancel', 'Cancel')}
                onCancel={() => setDeleteDialogState({ open: false, campaign: null })}
                onConfirm={async () => {
