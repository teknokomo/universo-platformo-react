...
const ClusterMembers = () => {
    ...
    const paginationResult = usePaginated<ClusterMember, 'email' | 'role' | 'created'>({
        queryKeyFn: (params) => clustersQueryKeys.membersList(clusterId!, params),
        queryFn: (params) => clustersApi.listClusterMembers(clusterId!, params),
        initialLimit: 20,
        sortBy: 'created',
        sortOrder: 'desc',
        enabled: !!clusterId
    ...
    const handleInviteMember = async (data: { email: string; role: AssignableRole; comment?: string }) => {
        ...
        try {
            ...
            let message = t('clusters:members.inviteError')

            // Use type-safe axios error utilities
            if (isHttpStatus(error, 404)) {
                message = t('clusters:members.userNotFound', { email: data.email })
            } else if (isHttpStatus(error, 409) && isApiError(error, 'CLUSTER_MEMBER_EXISTS')) {
                message = t('clusters:members.userAlreadyMember', { email: data.email })
            } else {
    ...
    const handleChange = (_event: React.MouseEvent<HTMLElement>, nextView: string | null) => {
        ...
        localStorage.setItem('clusterMembersDisplayStyle', nextView)
        setView(nextView)
    }
    ...
    const memberColumns = [
        ...
        },
        {
            ...
            align: 'center',
            render: (row: ClusterMember) => {
                const roleKey = (row.role || 'member') as ClusterRole
    ...
    ] satisfies TableColumn<ClusterMember>[]

    const createMemberContext = useCallback(
        (baseContext: Partial<ActionContext<ClusterMember, MemberData>>): ActionContext<ClusterMember, MemberData> => ({
            ...baseContext,
            ...
            helpers: {
                ...
                openDeleteDialog: (member: ClusterMember) => {
    ...
    if (!clusterId) {
        return (
            <MainCard
                ...
                shadow={false}
            >
                <EmptyListState image={APIEmptySVG} imageAlt='Invalid cluster' title={t('errors.connectionFailed')} />
            </MainCard>
        )
    }
                    ...
                    {isLoading && members.length === 0 ? (
                        ...
                        <>
                            {view === 'card' ? (
                                ...
                                >
                                    {members.map((member: ClusterMember) => {
                                                ...
                                                headerAction={
                                                    descriptors.length > 0 ? (
                                                        <Box onClick={(e) => e.stopPropagation()}>
                                                            <BaseEntityMenu<ClusterMember, MemberFormData>
                                                                entity={member}
                                                                entityKind='member'
                                ...
                                <Box sx={{ mx: { xs: -1.5, md: -2 } }}>
                                    <FlowListTable
                                        ...
                                        renderActions={(row: ClusterMember) => {
                                            // Owner role cannot be edited or removed
                                            if (row.role === 'owner') {
                                                return null
                                            }
