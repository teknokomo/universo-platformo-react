 ...
 * Cluster Board Page
 *
 * Displays analytics dashboard for a cluster with:
 * - Real-time statistics (domains, resources, members)
...
const ClusterBoard = () => {
    ...
    // Fetch cluster details with TanStack Query
    const {
        data: cluster,
        isLoading,
        error,
    ...
    // Error state
    if (isError || !cluster) {
        const errorMessage = error instanceof Error ? error.message : t('board.error', 'Failed to load cluster data')

        return (
            <Stack spacing={3} sx={{ maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto', width: '100%', p: 2 }}>
                <EmptyListState
                    image={APIEmptySVG}
                    imageAlt='Error loading cluster'
                    title={t('board.error', 'Failed to load cluster data')}
                />
    ...
    // Use current resource count as trend data (shows actual value in chart)
    const resourcesCount = cluster.resourcesCount ?? 0
    const resourcesData = Array(30).fill(resourcesCount)
    ...
    return (
        <Stack spacing={2} sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto' }}>
            ...
            <Box sx={{ px: { xs: 1.5, md: 2 } }}>
                <ViewHeader
                    title={cluster.name}
                    description={cluster.description || t('board.defaultDescription', 'Cluster analytics and statistics')}
                    search={false}
                />
            </Box>
            ...
            <Box sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' } }}>
                <Grid container spacing={2} columns={12}>
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <StatCard
                            title={t('board.stats.domains.title')}
                            value={cluster.domainsCount ?? 0}
                            interval={t('board.stats.domains.interval')}
                            description={t('board.stats.domains.description')}
                            data={domainsData}
                        />
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <StatCard
                            title={t('board.stats.resources.title')}
                            value={cluster.resourcesCount ?? 0}
                            interval={t('board.stats.resources.interval')}
                            description={t('board.stats.resources.description')}
                            data={resourcesData}
                        />
                    </Grid>
                ...
                <Box display='flex' justifyContent='center' sx={{ mt: 2 }}>
                    <Button variant='text' startIcon={<ArrowBackRoundedIcon />} onClick={() => navigate('/clusters')}>
                        {t('actions.backToList', 'Back to Clusters')}
                    </Button>
                </Box>
            </Box>
        </Stack>
