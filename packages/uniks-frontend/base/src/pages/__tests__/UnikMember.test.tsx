...
import UnikMembers from '../UnikMembers'
...
// Mock API module
vi.mock('../../api/uniks', () => ({
    listUnikMembers: vi.fn(),
    inviteUnikMember: vi.fn(),
    updateUnikMemberRole: vi.fn(),
    removeUnikMember: vi.fn()
}))
...
const renderWithProviders = (ui: React.ReactElement, { route = '/uniks/test-unik-id/members' } = {}) => {
                ...
                <SnackbarProvider maxSnack={3}>
                    <I18nextProvider i18n={i18n}>
                        <MemoryRouter initialEntries={[route]}>
                            <Routes>
                                <Route path='/uniks/:unikId/members' element={ui} />
                            </Routes>
                        </MemoryRouter>
                    </I18nextProvider>
                </SnackbarProvider>
...
describe('UnikMembers', () => {
    beforeEach(() => {
        vi.clearAllMocks()
        // Default mock: return empty list with proper structure
        vi.mocked(uniksApi.listUnikMembers).mockResolvedValue({
            data: [],
            pagination: {
                total: 0,
                limit: 20,
    ...
    describe('Component rendering with happy-dom', () => {
        it('should render component with translated UI elements', () => {
            renderWithProviders(<UnikMembers />)
        ...
        it('should render toolbar with search and controls', () => {
            const { container } = renderWithProviders(<UnikMembers />)
    ...
    describe('Loading state', () => {
        it('should display skeleton loader while fetching members', () => {
            vi.mocked(uniksApi.listUnikMembers).mockImplementation(
                () =>
                    new Promise((resolve) =>
                        setTimeout(
                            () =>
    ...
    describe('Error handling', () => {
        it('should display error message when API fails', async () => {
            vi.mocked(uniksApi.listUnikMembers).mockRejectedValue(new Error('Network error'))
    ...
    describe('Search functionality', () => {
        ...
        it('should update search field on user input', async () => {
            const { user } = renderWithProviders(<UnikMembers />)
    ...
    describe('Navigation', () => {
        it('should handle missing unikId parameter', () => {
            vi.mocked(uniksApi.listUnikMembers).mockImplementation(() => new Promise(() => {}))

            renderWithProviders(<UnikMembers />, { route: '/uniks//members' })

            // Should not call API without valid unikId
            expect(uniksApi.listUnikMembers).not.toHaveBeenCalled()
        })
    })
})
