...
```bash
pnpm add @universo-react/api-client
```

## Базовое использование

```typescript
import { createUniversoApiClient } from '@universo-react/api-client'
...
Внутренний HTTP-клиент создаётся через `@universo-react/auth-frontend`, поэтому он
...
```bash
pnpm --filter @universo-react/api-client build
pnpm --filter @universo-react/api-client test
pnpm --filter @universo-react/api-client lint
```
...
-   [Auth Frontend](../universo-react-auth-frontend/README-RU.md)
-   [REST Docs](../../universo-react-rest-docs/README-RU.md)
