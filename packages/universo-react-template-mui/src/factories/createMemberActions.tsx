...
import type { BaseMemberEntity, DynamicMemberEntity, AnyMemberEntity, VersionedLocalizedContent } from '@universo-react/types'
...
export interface MemberActionsConfig<TMember extends AnyMemberEntity> {
     ...
     */
    availableRoles?: ReadonlyArray<import('@universo-react/types').MemberRole>
     ...
     */
    roleLabels?: Partial<Record<import('@universo-react/types').MemberRole, string>>
 ...
 */
function getMemberErrorMessage(error: unknown): string | undefined {
    if (
        error &&
        typeof error === 'object' &&
        'response' in error &&
...
export function notifyMemberError(
    ...
    const candidateMessage = getMemberErrorMessage(error) ?? fallback
 ...
 * ```typescript
 * import { createMemberActions } from '@universo-react/template-mui'
