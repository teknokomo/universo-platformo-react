...
export default defineConfig({
    ...
    external: [
        ...
        /^@mui\//,
        /^@emotion\//,
        /^@universo-react\//, // CRITICAL: All @universo-react/* packages must be external to prevent bundling (especially auth-frontend)
    ...
    hooks: {
        'build:prepare': async (context) => {
            ...
            console.info('[tsdown:universo-react-template-mui] build prepare', payload)
        },
        'build:done': async () => {
            // eslint-disable-next-line no-console
            console.info('[tsdown:universo-react-template-mui] build complete')
        }
    },
    onSuccess: async () => {
