...
The project is structured as a **workspace package** (`@universo/profile-backend`) within the monorepo, enabling clean separation and future extraction to separate repositories:

```
packages/profile-backend/base/
├── package.json              # Package config with scoped name "@universo/profile-backend"
...
-   **Package Name**: `@universo/profile-backend` (scoped name for organization)
-   **Integration**: Used as dependency `"@universo/profile-backend": "workspace:*"` in main server
...
```bash
# Run linting
pnpm --filter @universo/profile-backend lint
```
...
node -e "console.log(Object.keys(require('@universo/profile-backend')))"
...
1. **Package Restructure** – Converted from relative imports to workspace package `@universo/profile-backend`
2. **Clean Import System** – Main server now imports via `import { Profile, profileMigrations, createProfileRoutes } from '@universo/profile-backend'`
...
-   **Frontend Changes**: Nickname input field commented out in `packages/flowise-core-frontend/base/src/views/up-auth/Auth.jsx`
