...
const expectedMetrics = (() => {
    ...
    const averagePoints = points.length > 0 ? Math.round((totalPoints / points.length) * 100) / 100 : 0
    return {
...
vi.mock('@universo/spaces-frontend', () => ({
    spacesApi: {
...
describe('Analytics dashboard', () => {
    ...
    it('analytics:renders analytics summary and lead table once data resolves', async () => {
        ...
        await waitFor(() => {
            const metrics = screen.getAllByRole('heading', { level: 4 }).map((heading) => heading.textContent)
