...
This package (`@universo-react/applications-frontend`) provides the React UI for standalone applications, including the **Application Migration Guard** system.
...
Route guard that checks migration status before allowing access to application pages. Uses `MigrationGuardShell` from `@universo-react/migration-guard-shared` for common guard logic.
