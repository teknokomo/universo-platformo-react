...
Публичные контракты API не изменялись; существующие клиенты продолжают работать без изменений.
...
Настройка выполняется в основном сервере; данный пакет добавляет только middleware на уровне маршрутов.
