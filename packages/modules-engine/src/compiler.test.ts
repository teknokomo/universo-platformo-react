...
const VALID_SOURCE = `import { ExtensionModule, AtClient, AtServer } from '@universo/extension-sdk'
...
const SERVER_AND_CLIENT_SOURCE = `import { ExtensionModule, AtServerAndClient } from '@universo/extension-sdk'

export default class SharedModule extends ExtensionModule {
    @AtServerAndClient()
    async ping() {
        return { ok: true }
    }
...
const POSTING_LIFECYCLE_SOURCE = `import { ExtensionModule, OnEvent } from '@universo/extension-sdk'
...
const SAFE_SERVER_ONLY_QUIZ_SOURCE = `import { ExtensionModule, AtClient, AtServer } from '@universo/extension-sdk'
...
export default class SpaceQuizModule extends ExtensionModule {
    @AtClient()
    async mount() {
...
const VALID_LIBRARY_SOURCE = `import { SharedLibraryModule } from '@universo/extension-sdk'

export default class SharedHelpers extends SharedLibraryModule {
...
describe('compileModuleSource', () => {
    it('compiles embedded modules that stay within the supported SDK boundary', async () => {
        const artifact = await compileModuleSource(createInput(VALID_SOURCE))

        expect(artifact.manifest.className).toBe('ExampleModule')
    ...
    it('keeps AtServerAndClient methods available in both bundles', async () => {
        const artifact = await compileModuleSource(createInput(SERVER_AND_CLIENT_SOURCE))
    ...
    it('rejects unsupported static imports', async () => {
        await expect(compileModuleSource(createInput(`import { readFileSync } from 'node:fs'\n${VALID_SOURCE}`))).rejects.toThrow(
    ...
    it('rejects dynamic import expressions', async () => {
        await expect(
            compileModuleSource(
                createInput(`import { ExtensionModule, AtClient } from '@universo/extension-sdk'
...
`)
    ...
    it('rejects top-level runtime bindings shared by client and server methods', async () => {
        await expect(
            compileModuleSource(
                createInput(`import { ExtensionModule, AtClient, AtServer } from '@universo/extension-sdk'
...
`)
    ...
    it('keeps server-only quiz data out of the client bundle when the source stays within target boundaries', async () => {
        const artifact = await compileModuleSource(createInput(SAFE_SERVER_ONLY_QUIZ_SOURCE))
    ...
    it('compiles pure shared libraries and keeps them decorator-free', async () => {
        const artifact = await compileModuleSource(createLibraryInput(VALID_LIBRARY_SOURCE))
    ...
    it('bundles @shared imports from the provided metahub library map', async () => {
        const artifact = await compileModuleSource(
            createInput(
                `import { ExtensionModule, AtServer } from '@universo/extension-sdk'
...
`,
    ...
    it('rejects library modules that use runtime decorators', async () => {
        await expect(
            compileModuleSource(
                createLibraryInput(`import { SharedLibraryModule, AtServer } from '@universo/extension-sdk'
...
`)
    ...
    it('rejects library modules that access runtime ctx state', async () => {
        await expect(
            compileModuleSource(
                createLibraryInput(`import { SharedLibraryModule } from '@universo/extension-sdk'
...
`,
                    {
                        sharedLibraries: {
                            'shared-a': {
                                codename: 'shared-a',
                                sourceCode: `import { SharedLibraryModule } from '@universo/extension-sdk'
