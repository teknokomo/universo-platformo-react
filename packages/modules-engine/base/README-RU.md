...
- Embedded-модули могут импортировать только `@universo/extension-sdk`; неподдерживаемые static imports, `require()`, dynamic `import()` и `import.meta` завершают compilation с ошибкой.
...
`pnpm --filter @universo/modules-engine build`
`pnpm --filter @universo/modules-engine test`
`pnpm --filter @universo/modules-engine lint`
