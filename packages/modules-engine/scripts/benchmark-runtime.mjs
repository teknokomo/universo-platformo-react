...
import { IsolatedVmModuleRuntimeHost, assertIsolatedVmRuntimeAvailable } from '../dist/index.js'

const warmupIterations = Number.parseInt(process.env.MODULE_BENCH_WARMUP ?? '10', 10)
const benchmarkIterations = Number.parseInt(process.env.MODULE_BENCH_ITERATIONS ?? '100', 10)
const outputPath = process.env.MODULE_BENCHMARK_OUTPUT_PATH

if (!Number.isFinite(warmupIterations) || warmupIterations < 0) {
    throw new Error('MODULE_BENCH_WARMUP must be a non-negative integer')
}

if (!Number.isFinite(benchmarkIterations) || benchmarkIterations <= 0) {
    throw new Error('MODULE_BENCH_ITERATIONS must be a positive integer')
}
...
const coldHost = new IsolatedVmModuleRuntimeHost()
...
const warmHost = new IsolatedVmModuleRuntimeHost()
