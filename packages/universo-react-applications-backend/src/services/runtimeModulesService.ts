...
import {
    canCallModuleMethodOverPublicRpc,
    hasModuleCapability,
    ...
    type RuntimeModuleCallRequest,
    type ModuleAttachmentKind,
    type ModuleCapability,
    type ModuleLifecyclePayload
} from '@universo-react/types'
import { createLocalizedContent, resolveApplicationLifecycleContractFromConfig, type DbExecutor } from '@universo-react/utils'
...
type RuntimeModuleRow = {
    ...
    module_role: ApplicationModuleDefinition['moduleRole']
    source_kind: ApplicationModuleDefinition['sourceKind']
...
type RuntimeModuleComponentRow = {
    id: string
    codename: unknown
    column_name: string
    data_type: string
...
type RuntimeModuleRecordBinding = {
    object: RuntimeModuleObjectRow & {
...
type RuntimeLifecycleDispatchParams = {
    ...
    schemaName: string
    attachmentKind?: ModuleAttachmentKind | null
...
const ACTIVE_MODULE_WHERE = '_upl_deleted = false AND _app_deleted = false AND is_active = true'

const createFallbackModulePresentation = (): ApplicationModuleDefinition['presentation'] => ({
    name: createLocalizedContent('en', '')
})

const normalizeModule = (row: RuntimeModuleRow): ApplicationModuleDefinition => {
    const moduleRole = normalizeModuleRole(row.module_role)
    const sourceKind = normalizeModuleSourceKind(row.source_kind)
    const rawManifest =
        row.manifest && typeof row.manifest === 'object' ? (row.manifest as Partial<ApplicationModuleDefinition['manifest']>) : {}
    const manifestModuleRole = normalizeModuleRole(rawManifest.moduleRole ?? moduleRole)
...
const CLIENT_VISIBLE_MODULE_CAPABILITIES = new Set<ModuleCapability>(['metadata.read', 'rpc.client'])

const sanitizeClientModule = (module: ApplicationModuleDefinition): ApplicationModuleDefinition => {
    const { serverBundle: _serverBundle, clientBundle: _clientBundle, config: _config, ...publicModule } = module

    return {
        ...publicModule,
        manifest: {
            ...module.manifest,
            capabilities: module.manifest.capabilities.filter((capability) => CLIENT_VISIBLE_MODULE_CAPABILITIES.has(capability)),
            methods: module.manifest.methods.filter((method) => isClientModuleMethodTarget(method.target) && !method.eventName)
        },
        config: {}
    } as ApplicationModuleDefinition
}
...
const createPublicRpcError = (): Error => new Error('Runtime module method is not callable through public RPC')

const buildAttachmentKey = (kind: ModuleAttachmentKind, id: string | null | undefined): string => `${kind}:${id ?? ''}`

const resolveRecordFieldKey = (cmp: RuntimeModuleComponentRow): string => {
    const codenameKey = resolveRuntimeCodenameText(cmp.codename)
    return codenameKey.trim().length > 0 ? codenameKey : cmp.column_name
}

const resolveLifecycleAttachmentKind = (kind: string): ModuleAttachmentKind | null =>
    isModuleAttachmentKind(kind) ? normalizeModuleAttachmentKind(kind) : null

const collectTouchedRecordComponentIds = (attrs: RuntimeModuleComponentRow[], payload: Record<string, unknown>): string[] => {
...
const isLocalizedStringComponent = (cmp: RuntimeModuleComponentRow): boolean =>
    cmp.data_type === 'STRING' && Boolean(cmp.validation_rules?.versioned || cmp.validation_rules?.localized)

const buildModuleRecordPayload = (row: Record<string, unknown>, attrs: RuntimeModuleComponentRow[]): Record<string, unknown> => {
...
const assertCurrentUserId = (currentUserId: string | null): string => {
    if (!currentUserId) {
        throw new Error('Runtime module write context is missing current user id')
    }
...
const ensureNoUnsupportedTableFields = (attrs: RuntimeModuleComponentRow[], payload: Record<string, unknown>) => {
...
const resolveFilterComponent = (attrs: RuntimeModuleComponentRow[], key: string): RuntimeModuleComponentRow | null => {
...
const resolveFilterColumnSql = (cmp: RuntimeModuleComponentRow): string => {
...
const normalizeRecordFilterValue = (cmp: RuntimeModuleComponentRow, rawValue: unknown): unknown => {
...
const buildWritableAttrs = (attrs: RuntimeModuleComponentRow[]): RuntimeModuleComponentRow[] =>
    attrs.filter((cmp) => IDENTIFIER_REGEX.test(cmp.column_name) && RUNTIME_WRITABLE_TYPES.has(cmp.data_type) && cmp.data_type !== 'TABLE')

const isClientVisibleRuntimeModule = (module: ApplicationModuleDefinition): boolean =>
    Boolean(module.clientBundle) && module.manifest.methods.some((method) => isClientModuleMethodTarget(method.target))

export class RuntimeModulesService {
    constructor(
        private readonly engine: ModuleEngine = new ModuleEngine(),
        private readonly ledgers: RuntimeLedgersService = new RuntimeLedgersService()
    ) {}

    async listClientModules(params: {
        executor: DbExecutor
        schemaName: string
        ...
        return modules
            .filter((module) => {
                if (!isClientVisibleRuntimeModule(module)) return false
            ...
            .map(sanitizeClientModule)
    }

    async getClientModuleBundle(params: {
        executor: DbExecutor
        schemaName: string
        moduleId: string
    }): Promise<{ bundle: string; checksum: string }> {
        const module = await this.getActiveModuleById(params.executor, params.schemaName, params.moduleId)
        if (!module) {
            throw new Error('Runtime module not found')
        }

        if (!isClientVisibleRuntimeModule(module) || !module.clientBundle) {
            throw new Error('Runtime module does not expose a client bundle')
        }

        return {
            bundle: module.clientBundle,
    ...
    async callServerMethod(params: {
        ...
        currentUserId?: string | null
        permissions?: Record<RolePermission, boolean> | null
        request: RuntimeModuleCallRequest
    }): Promise<unknown> {
        const module = await this.getActiveModuleById(params.executor, params.schemaName, params.moduleId)
        if (!module) {
            throw new Error('Runtime module not found')
        ...
        const method = module.manifest.methods.find((item) => item.name === params.request.methodName)
        if (!method) {
        ...
        if (!hasModuleCapability(module.manifest, 'rpc.client')) {
            throw createCapabilityError('rpc.client')
        }

        if (!canCallModuleMethodOverPublicRpc(module.manifest, method)) {
    ...
    async dispatchLifecycleEvent(params: RuntimeLifecycleDispatchParams): Promise<unknown[]> {
        ...
        for (const module of modules) {
            if (!module.serverBundle) continue
            if (!hasModuleCapability(module.manifest, 'lifecycle')) continue
            if (!attachmentKeys.has(buildAttachmentKey(module.attachedToKind, module.attachedToId))) continue

            const handlers = module.manifest.methods.filter(
                (method) => isServerModuleMethodTarget(method.target) && method.eventName === params.payload.eventName
            )
    ...
    private dispatchLifecycleEventAfterCommit(params: RuntimeLifecycleDispatchParams | null): void {
        ...
        void this.dispatchLifecycleEvent(params).catch((error) => {
            console.error(`[RuntimeModulesService] ${params.payload.eventName} lifecycle hook failed`, error)
        })
    }
    ...
    private buildLifecycleDispatchParams(params: {
        ...
        applicationId: string
        schemaName: string
        binding: RuntimeModuleRecordBinding
        currentWorkspaceId: string | null
        currentUserId: string | null
        permissions: Record<RolePermission, boolean> | null
        componentIds?: string[]
    ...
    private async listActiveModules(executor: DbExecutor, schemaName: string): Promise<ApplicationModuleDefinition[]> {
        if (!(await this.hasModulesTable(executor, schemaName))) {
            return []
        }

        const modulesTable = qSchemaTable(schemaName, '_app_modules')
    ...
    private async getActiveModuleById(
        executor: DbExecutor,
        schemaName: string,
        moduleId: string
    ): Promise<ApplicationModuleDefinition | null> {
        ...
        SELECT id, codename, presentation, attached_to_kind, attached_to_id, module_role, source_kind,
               sdk_api_version, manifest, server_bundle, client_bundle, checksum, is_active, config
        FROM ${modulesTable}
        WHERE id = $1 AND ${ACTIVE_MODULE_WHERE}
        LIMIT 1
      `,
            [moduleId]
        )) as RuntimeModuleRow[]

        return rows[0] ? normalizeModule(rows[0]) : null
    }

    private async hasModulesTable(executor: DbExecutor, schemaName: string): Promise<boolean> {
        const rows = (await executor.query('SELECT to_regclass($1) AS table_name', [`${schemaName}._app_modules`])) as Array<{
    ...
    private createExecutionContext(params: {
        ...
        registrarKind?: ModuleAttachmentKind | null
    }) {
        const canReadRecords = hasModuleCapability(params.module.manifest, 'records.read')
        const canWriteRecords = hasModuleCapability(params.module.manifest, 'records.write')
        const canReadMetadata = hasModuleCapability(params.module.manifest, 'metadata.read')
        const canReadLedger = hasModuleCapability(params.module.manifest, 'ledger.read')
        const canWriteLedger = hasModuleCapability(params.module.manifest, 'ledger.write')
        const canCallServerMethod = hasModuleCapability(params.module.manifest, 'rpc.client')
        ...
        const denyCapability = async (capability: ModuleCapability): Promise<never> => {
    ...
    private async resolveRecordBinding(params: {
        ...
        entityCodename: string
        currentWorkspaceId: string | null
    }): Promise<RuntimeModuleRecordBinding> {
    ...
    private async getRecordById(params: {
      ...
      `,
        ...
        return rows[0] ? buildModuleRecordPayload(rows[0], attrs) : null
    }
    ...
    private async listRecords(params: {
      ...
      `,
        ...
        return rows.map((row) => buildModuleRecordPayload(row, attrs))
    }
