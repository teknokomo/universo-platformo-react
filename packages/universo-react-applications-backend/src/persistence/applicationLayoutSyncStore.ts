...
/**
 * Sync-generated widgets have no source row id. Keep their logical lineage in
 * the existing UUID source column so a changed materialization placeholder
 * cannot allocate a second physical row on the next sync.
 */
const stableLineageUuidV7 = (layoutId: string, lineageKey: string): string => {
    if (!isUuidV7(layoutId)) {
        throw new Error('[SchemaSync] Generated widget lineage requires a UUID v7 layout id')
    }

    // A generated widget does not have a source row id, so keep its logical
    // lineage stable without introducing another schema column. Reuse the
    // source layout timestamp as the UUID v7 timestamp and derive only the
    // random bits from the lineage key. This remains a real, time-ordered
    // UUID v7 rather than a hash with the version nibble overwritten.
