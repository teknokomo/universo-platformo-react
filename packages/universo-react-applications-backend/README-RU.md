...
-   Admin, bootstrap и background flows используют Tier 2 pool executor-ы из `@universo-react/database`.
...
-   Применять declarative `beforePost` module movements через generic Ledger service внутри posting transaction.
...
-   Выполнять опубликованные runtime modules через fail-closed server bridge, который переиспользует runtime row helpers, workspace context и permission maps.
-   Экспортировать `ctx.ledger` для runtime modules только при объявленных capabilities `ledger.read` или `ledger.write`.
-   Экспортировать runtime sync, diff и release-bundle routes для managed application schemas.
-   Выполнять строгие транзакционные команды трактовочной сети для одной системной Структуры, шаблонов, прикрепления Материалов и создания/перемещения ячеек Матрицы. Для сохранения и создания из шаблона нужны права создания и редактирования, для метаданных — редактирования, для удаления — удаления; несовпадение Приложения или рабочего пространства отклоняется. При копировании создаются новые UUID v7 строк, ячеек и Материалов, Связи и файлы не клонируются, а обычные внешние URL сохраняются только как созданные пользователем значения внутри скопированного `Body` Материала.
-   Хранить последнюю базовую конфигурацию виджета метахаба в nullable-поле `_app_widgets.source_config`. Администраторы приложения могут атомарно сбросить эффективную конфигурацию с оптимистической блокировкой, а удалённый исходный виджет не восстанавливает устаревшие настройки.
...
-   Активные runtime module codenames уникальны в scope `(attached_to_kind, attached_to_id, module_role, codename)`, а sync чинит scoped index для существующих схем.
...
-   `@universo-react/applications-frontend` для UI управления приложениями.
-   `@universo-react/database` для владения Knex runtime и фабрик executor-ов.
-   `@universo-react/utils` для нейтральных executor/query helper контрактов.
-   `@universo-react/schema-ddl` для runtime-генерации схем и выполнения diff.
