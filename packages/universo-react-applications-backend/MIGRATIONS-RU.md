...
При запуске платформы `@universo-react/core-backend` выполняет fixed system-app pipeline для `applications`:
...
`@universo-react/applications-backend` владеет runtime orchestration endpoints:
...
Migration guard и migration history endpoints для runtime applications сейчас монтируются в `@universo-react/metahubs-backend`, а не в этом пакете:
...
Это разделение сделано намеренно: `@universo-react/applications-backend` владеет runtime sync и release-bundle execution, а `@universo-react/metahubs-backend` пока продолжает хостить migration-control surface, используемую текущим authoring flow.
