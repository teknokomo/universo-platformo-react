...
On platform startup `@universo-react/core-backend` runs the fixed system-app pipeline for `applications`:
...
`@universo-react/applications-backend` owns the runtime orchestration endpoints:
...
The migration guard and migration history endpoints for runtime applications are currently mounted by `@universo-react/metahubs-backend`, not by this package:
...
This split is intentional: `@universo-react/applications-backend` owns runtime sync and release-bundle execution, while `@universo-react/metahubs-backend` still hosts the migration-control surface used by the current authoring flow.
