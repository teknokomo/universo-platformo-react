...
import { api } from '@universo/api-client' // Replaced import configApi from '@/api/config'
// import { api } from '@universo/api-client' // Replaced import { api } from '@universo/api-client' // Duplicate removed
...
const VectorStoreDialog = ({ show, dialogProps, onCancel, onIndexResult }) => {
    ...
    const getConfigApi = useApi((...args) => api.config.getConfig(...args))
...
-F "openAIApiKey[openAIEmbeddings_0]=sk-my-openai-2nd-key" \\`
    ...
    const onUpsertClicked = async (vectorStoreNode) => {
        ...
        setLoading(true)
        try {
            const res = await api.vectorStores.upsertVectorStore(canvasId, { stopNodeId: vectorStoreNode.data.id })
            enqueueSnackbar({
