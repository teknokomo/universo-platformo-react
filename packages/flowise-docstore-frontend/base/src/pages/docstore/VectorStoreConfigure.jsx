...
import UpsertResultDialog from '../vectorstore/UpsertResultDialog'
...
const VectorStoreConfigure = () => {
    ...
    const getSpecificDocumentStoreApi = useApi((...args) => api.documentStore.getSpecificDocumentStore(...args))
    const insertIntoVectorStoreApi = useApi((...args) => api.documentStore.insertIntoVectorStore(...args))
    const saveVectorStoreConfigApi = useApi((...args) => api.documentStore.saveVectorStoreConfig(...args))
    const getEmbeddingNodeDetailsApi = useApi((...args) => api.nodes.getSpecificNode(...args))
    const getVectorStoreNodeDetailsApi = useApi((...args) => api.nodes.getSpecificNode(...args))
    const getRecordManagerNodeDetailsApi = useApi((...args) => api.nodes.getSpecificNode(...args))
    ...
    const showEmbeddingsList = () => {
        const dialogProp = {
            title: t('vector-store:embeddings.selectProvider'),
            unikId: unikId
        }
    ...
    const showVectorStoreList = () => {
        const dialogProp = {
            title: t('vector-store:selectProvider'),
            unikId: unikId
        }
        setDialogProps(dialogProp)
    ...
    const showRecordManagerList = () => {
        const dialogProp = {
            title: t('vector-store:recordManager.selectProvider'),
            unikId: unikId
        }
        setDialogProps(dialogProp)
    ...
    const checkMandatoryFields = () => {
        ...
        if (!canSubmit) {
            enqueueSnackbar({
                message: t('vector-store:messages.mandatoryFields'),
                options: {
        ...
        return (
            <Box sx={{ width: '100%' }}>
                <Stepper activeStep={getActiveStep()} alternativeLabel>
                    {steps.map((label) => (
                        <Step key={label}>
                            <StepLabel>{t('vector-store:steps.' + label.toLowerCase())}</StepLabel>
                        </Step>
                    ))}
                </Stepper>
            </Box>
    ...
    useEffect(() => {
        if (saveVectorStoreConfigApi.data) {
            setLoading(false)
            enqueueSnackbar({
                message: t('vector-store:messages.configSaved'),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'success',
                    action: (key) => (
        ...
        <>
                        ...
                        {storeId && (
                            <Stack flexDirection='column' sx={{ gap: 3 }}>
                                <ViewHeader
                                    ...
                                    description={t('vector-store:configureDescription')}
                                    onBack={() => navigate(-1)}
                                >
                                    ...
                                    {(Object.keys(selectedEmbeddingsProvider).length > 0 ||
                                        ...
                                        >
                                            {t('vector-store:actions.saveConfig')}
                                        </Button>
                                    )}
                                    {Object.keys(selectedEmbeddingsProvider).length > 0 &&
                                        Object.keys(selectedVectorStoreProvider).length > 0 && (
                                    ...
                                    <IconButton
                                        ...
                                        size='small'
                                        color='inherit'
                                        title={t('vector-store:actions.upsertHistory')}
                                    >
                                        <IconClock />
                                    </IconButton>
                                </ViewHeader>
                                ...
                                <Grid container spacing={1}>
                                    <Grid item xs={12} sm={4} md={4}>
                                        {Object.keys(selectedEmbeddingsProvider).length === 0 ? (
                                            ...
                                            >
                                                {t('vector-store:embeddings.selectEmbeddings')}
                                            </Button>
                                        ) : (
                                            <Box>
                                    ...
                                    <Grid item xs={12} sm={4} md={4}>
                                        {Object.keys(selectedVectorStoreProvider).length === 0 ? (
                                            ...
                                            >
                                                {t('vector-store:selectVectorStore')}
                                            </Button>
                                        ) : (
                                            <Box>
                                                <Grid container spacing='2'>
                                    ...
                                    <Grid item xs={12} sm={4} md={4}>
                                        {Object.keys(selectedRecordManagerProvider).length === 0 ? (
                                            ...
                                            >
                                                {isRecordManagerUnavailable
                                                    ? t('vector-store:recordManager.notApplicable')
                                                    : t('vector-store:recordManager.selectRecordManager')}
                                            </Button>
                                        ) : (
                                            <Box>
                                                <Grid container spacing='2'>
            ...
            {showEmbeddingsListDialog && (
                <ComponentsListDialog
                    ...
                    apiCall={(...args) => api.documentStore.getEmbeddingProviders(...args)}
            ...
            {showVectorStoreListDialog && (
                <ComponentsListDialog
                    ...
                    apiCall={(...args) => api.documentStore.getVectorStoreProviders(...args)}
            ...
            {showRecordManagerListDialog && (
                <ComponentsListDialog
                    ...
                    apiCall={(...args) => api.documentStore.getRecordManagerProviders(...args)}
