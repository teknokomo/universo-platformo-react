...
import chunks_emptySVG from '@flowise/template-mui/assets/images/chunks_empty.svg'
...
const ShowStoredChunks = () => {
    ...
    const getChunksApi = useApi((...args) => api.documentStore.getFileChunks(...args))
    ...
    const onChunkEdit = async (newPageContent, newMetadata, chunk) => {
        ...
        try {
            const editResp = await api.documentStore.editChunkFromStore(
                unikId,
                chunk.storeId,
                chunk.docId,
                chunk.id,
            ...
            if (editResp.data) {
                enqueueSnackbar({
                    message: t('document-store:chunks.editSuccess'),
                    options: {
            ...
            setLoading(false)
            enqueueSnackbar({
                message: t('document-store:chunks.editError', {
    ...
    const onDeleteChunk = async (chunk) => {
        const confirmPayload = {
            title: t('document-store:chunks.delete'),
            description: t('document-store:chunks.deleteConfirm').replace('{{id}}', chunk.id),
            confirmButtonName: t('document-store:common.delete'),
        ...
        if (isConfirmed) {
            ...
            setShowExpandedChunkDialog(false)
            try {
                const delResp = await api.documentStore.deleteChunkFromStore(unikId, chunk.storeId, chunk.docId, chunk.id)
                if (delResp.data) {
                    enqueueSnackbar({
                        message: t('document-store:chunks.deleteSuccess'),
                        options: {
                            key: new Date().getTime() + Math.random(),
                            variant: 'success',
                            action: (key) => (
                ...
                setLoading(false)
                enqueueSnackbar({
                    message: t('document-store:chunks.deleteError', {
                        error: typeof error.response.data === 'object' ? error.response.data.message : error.response.data
                    }),
                    options: {
                        key: new Date().getTime() + Math.random(),
                    ...
                    <div style={{ width: '100%' }}>
                        ...
                        >
                            <div style={{ marginRight: 20, display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                {t('document-store:chunks.showing', { start: Math.min(start, totalChunks), end, total: totalChunks })}
                                <IconButton
                                    size='small'
                                    onClick={() => changePage(currentPage + 1)}
                                    style={{ marginLeft: 10 }}
                            ...
                            <div style={{ marginRight: 20, display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                                ...
                                {t('document-store:chunks.totalCharacters', { count: getChunksApi.data?.characters?.toLocaleString() })}
                            </div>
                        </div>
                    </div>
                    <div>
                        <Grid container spacing={2}>
                            {!documentChunks.length && (
                                ...
                                >
                                    ...
                                    <div>{t('document-store:chunks.noChunks')}</div>
                                </div>
                            )}
                                            ...
                                            <Card>
                                                <CardContent sx={{ p: 2 }}>
                                                    <Typography sx={{ wordWrap: 'break-word', mb: 1 }} variant='h5'>
                                                        {`#${row.chunkNo}. ${t('document-store:chunks.charactersCount', {
