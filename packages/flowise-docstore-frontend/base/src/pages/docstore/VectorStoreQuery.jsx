...
const VectorStoreQuery = () => {
    ...
    const queryVectorStoreApi = useApi((...args) => api.documentStore.queryVectorStore(...args))
    ...
    const saveConfig = async () => {
        ...
        try {
            const updateResp = await api.documentStore.updateVectorStoreConfig(unikId, data)
            setLoading(false)
            if (updateResp.data) {
                enqueueSnackbar({
                    message: t('vector-store:query.configSuccess'),
                    options: {
            ...
            enqueueSnackbar({
                message: t('vector-store:query.configError', { error: errorData }),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
            ...
            <MainCard style={{ position: 'relative' }}>
                <Stack flexDirection='column' sx={{ gap: 1 }}>
                    <ViewHeader
                        isBackButton={true}
                        search={false}
                        title={documentStore?.name || t('vector-store:query.documentStore')}
                        description={t('vector-store:query.description')}
                        onBack={() => navigate(-1)}
                    >
                        <Button
                            variant='outlined'
                                ...
                                <Box>
                                    <div style={{ display: 'flex', flexDirection: 'row' }}>
                                        <Typography variant='overline'>
                                            {t('vector-store:query.enterQuery')}
                            ...
                            <Grid sx={{ ml: 1, mr: 1, mt: 1 }} container spacing={1}>
                                ...
                                <Grid item xs={12} sm={8} md={8}>
                                    ...
                                    >
                                        ...
                                        <Typography sx={{ ml: 2 }} variant='h3'>
                                            {t('vector-store:query.retrievedDocuments')}
                                            {timeTaken > -1 && (
                                                <Typography variant='body2' sx={{ color: 'gray' }}>
                                                    {t('vector-store:query.countAndTime', {
                                    ...
                                    {!documentChunks.length && (
                                        ...
                                        >
                                            ...
                                            <div>{t('vector-store:query.noDocuments')}</div>
                                        </div>
                                    )}
                                    <Grid container spacing={2}>
