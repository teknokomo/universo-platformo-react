...
const LoaderConfigPreviewChunks = () => {
    ...
    const getNodeDetailsApi = useApi((...args) => api.nodes.getSpecificNode(...args))
    const getNodesByCategoryApi = useApi((...args) => api.nodes.getNodesByCategory(...args))
    ...
    const onPreviewChunks = async () => {
        if (checkMandatoryFields()) {
            ...
            try {
                const previewResp = await api.documentStore.previewChunks(unikId, config)
                if (previewResp.data) {
    ...
    const onSaveAndProcess = async () => {
        if (checkMandatoryFields()) {
            ...
            const config = prepareConfig()
            try {
                const saveResp = await api.documentStore.saveProcessingLoader(unikId, config)
                setLoading(false)
                if (saveResp.data) {
                    enqueueSnackbar({
                    ...
                    api.documentStore.processLoader(unikId, config, saveResp.data?.id)
                            ...
                            >
                                <Box sx={{ display: 'flex', alignItems: 'center', flexDirection: 'row' }}>
                                    <StyledFab
                                        ...
                                        color='secondary'
                                        aria-label='back'
                                        title={t('document-store:loaders.common.back')}
                                        onClick={() => navigate(-1)}
                                    >
                                        <IconArrowLeft />
                                    </StyledFab>
                                                                ...
                                                                <Card>
                                                                    <CardContent sx={{ p: 1 }}>
                                                                        <Typography sx={{ wordWrap: 'break-word', mb: 1 }} variant='h5'>
                                                                            {`#${index + 1}. ${t(
                                                                                'document-store:loaders.common.characters'
