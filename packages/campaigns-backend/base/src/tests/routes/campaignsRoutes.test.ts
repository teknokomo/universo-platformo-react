...
import { createCampaignsRoutes } from '../../routes/campaignsRoutes'
...
describe('Campaigns Routes', () => {
    ...
    const buildApp = (dataSource: any) => {
        ...
        app.use(
            '/campaigns',
            createCampaignsRoutes(ensureAuth, () => dataSource, mockRateLimiter, mockRateLimiter)
        )
    ...
    const buildDataSource = () => {
        const campaignRepo = createMockRepository<any>()
        const campaignUserRepo = createMockRepository<any>()
        const activityRepo = createMockRepository<any>()
        const linkRepo = createMockRepository<any>()
        const eventRepo = createMockRepository<any>()
        const eventLinkRepo = createMockRepository<any>()
        const authUserRepo = createMockRepository<any>()

        const dataSource = createMockDataSource({
            Campaign: campaignRepo,
            CampaignMember: campaignUserRepo,
            Activity: activityRepo,
            ActivityCampaign: linkRepo,
            Event: eventRepo,
            EventCampaign: eventLinkRepo,
    ...
    describe('GET /campaigns', () => {
        it('should return empty array for user with no campaigns', async () => {
            const { dataSource, campaignRepo } = buildDataSource()

            // Mock QueryBuilder to return empty array
            const mockQB = campaignRepo.createQueryBuilder()
            mockQB.getRawMany.mockResolvedValue([])

            const app = buildApp(dataSource)

            const response = await request(app).get('/campaigns').expect(200)
        ...
        it('should return campaigns with counts for authenticated user', async () => {
            const { dataSource, campaignRepo } = buildDataSource()
            ...
            const mockQB = campaignRepo.createQueryBuilder()
            mockQB.getRawMany.mockResolvedValue(mockCampaigns)
        ...
        it('should handle pagination parameters correctly', async () => {
            ...
            await request(app).get('/campaigns?limit=5&offset=10&sortBy=name&sortOrder=asc').expect(200)
        ...
        it('should validate pagination parameters', async () => {
            ...
            await request(app).get('/campaigns?offset=-5').expect(400)

            // Test 2: Limit over 1000 is rejected by validation
            await request(app).get('/campaigns?limit=2000').expect(400)

            // Test 3: Valid params pass through
            await request(app).get('/campaigns?limit=50&offset=10').expect(200)
        ...
        it('should use default pagination when no parameters provided', async () => {
            ...
            await request(app).get('/campaigns').expect(200)
        ...
        it('should return 401 when user is not authenticated', async () => {
            ...
            app.use(
                '/campaigns',
                createCampaignsRoutes(noAuthMiddleware, () => dataSource, mockRateLimiter, mockRateLimiter)
            )
            app.use(errorHandler)

            await request(app).get('/campaigns').expect(401)
        })
        ...
        it('should handle database errors gracefully', async () => {
            ...
            const response = await request(app).get('/campaigns').expect(500)
        ...
        it('should filter by search query (case-insensitive)', async () => {
            ...
            await request(app).get('/campaigns?search=test').expect(200)
        ...
        it('should set correct pagination headers', async () => {
            ...
            await request(app)
                .get('/campaigns?limit=50&offset=25')
                .expect(200)
                .expect((res) => {
    ...
    describe('Members management endpoints', () => {
        const buildApp = () => {
            ...
            app.use(
                '/campaigns',
                createCampaignsRoutes(ensureAuth, () => context.dataSource, mockRateLimiter, mockRateLimiter)
            )
        ...
        it('should return members when user has manageMembers permission', async () => {
            ...
            campaignUserRepo.findOne.mockResolvedValueOnce({
                id: 'membership-admin',
            ...
            const mockQB = campaignUserRepo.createQueryBuilder()
            const membersList = [
                {
                    id: 'membership-owner',
                    campaign_id: 'campaign-1',
            ...
            dataSource.manager.find.mockImplementation((activity: any, _options: any) => {
                ...
                if (activityName === 'Profile') {
                    return Promise.resolve([])
                }
                return Promise.resolve([])
            })

            const response = await request(app).get('/campaigns/campaign-1/members').expect(200)
        ...
        it('should reject listing members without manageMembers permission', async () => {
            const { app, campaignUserRepo } = buildApp()
            ...
            const response = await request(app).get('/campaigns/campaign-1/members')
            expect(response.status).toBe(403)
            expect(campaignUserRepo.find).not.toHaveBeenCalled()
        })

        it('should create member when requester can manage members', async () => {
            const { app, campaignUserRepo, authUserRepo } = buildApp()

            campaignUserRepo.findOne
            ...
            expect(campaignUserRepo.save).toHaveBeenCalled()
        })
        ...
        it('should reject creating member without permission', async () => {
            ...
            await request(app).post('/campaigns/campaign-1/members').send({ email: 'target@example.com', role: 'member' }).expect(403)
        ...
        it('should update member role when authorized', async () => {
            ...
            const response = await request(app)
                .patch('/campaigns/campaign-1/members/membership-target')
                .send({ role: 'editor' })
                .expect(200)
            ...
            expect(campaignUserRepo.save).toHaveBeenCalledWith(expect.objectContaining({ id: 'membership-target', role: 'editor' }))
        })
        ...
        it('should forbid updating member without permission', async () => {
            ...
            await request(app).patch('/campaigns/campaign-1/members/membership-target').send({ role: 'editor' }).expect(403)
        })
        ...
        it('should delete member when authorized', async () => {
            ...
            await request(app).delete('/campaigns/campaign-1/members/membership-target').expect(204)
            expect(campaignUserRepo.remove).toHaveBeenCalledWith(expect.objectContaining({ id: 'membership-target' }))
        })
        ...
        it('should reject deleting member without permission', async () => {
            ...
            await request(app).delete('/campaigns/campaign-1/members/membership-target').expect(403)
        })
        ...
        describe('Members data enrichment', () => {
            ...
            it('should fetch comment from campaigns_users table', async () => {
                const { app, campaignUserRepo, dataSource } = buildApp()
                    ...
                    [
                        {
                            ...
                            user_id: 'editor-id',
                            role: 'editor',
                            comment: 'This is a test comment from campaigns_users table',
                            created_at: now
                        }
                    ],
                    1
                ...
                dataSource.manager.find.mockImplementation((activity: any) => {
                    const activityName = activity.name || (typeof activity === 'function' ? activity.name : String(activity))
                    if (activityName === 'AuthUser') {
                ...
                // Verify comment is returned from CampaignMember activity
                expect(response.body).toEqual(
                    expect.arrayContaining([
                        expect.objectContaining({
                            userId: 'editor-id',
        ...
        describe('Comment validation with trim', () => {
            it('should trim comment and validate max 500 characters on create', async () => {
                ...
                campaignUserRepo.findOne.mockResolvedValueOnce(null) // No existing membership

                const qb = authUserRepo.createQueryBuilder()
                qb.getOne.mockResolvedValue({ id: 'target-user', email: 'target@example.com' })

                campaignUserRepo.create.mockImplementation((data: any) => ({ ...data, id: 'new-membership-id' }))
                campaignUserRepo.save.mockImplementation((activity: any) => Promise.resolve(activity))
                ...
                expect(campaignUserRepo.create).toHaveBeenCalledWith(
                    expect.objectContaining({
            ...
            it('should return 400 with validation details for invalid role or email', async () => {
                ...
                campaignUserRepo.findOne.mockResolvedValue({
                    id: 'membership-admin',
                    campaign_id: 'campaign-1',
                    user_id: 'test-user-id',
                    role: 'admin'
                ...
                expect(campaignUserRepo.save).not.toHaveBeenCalled()
            })
        })
    })
    ...
    describe('Rate Limiting', () => {
        ...
        it('should have separate limits for read and write', async () => {
            const { dataSource, campaignRepo, authUserRepo } = buildDataSource()
            const mockQB = campaignRepo.createQueryBuilder()
            mockQB.getRawMany.mockResolvedValue([])

            campaignRepo.create.mockImplementation((data: any) => ({ ...data, id: 'new-id' }))
            campaignRepo.save.mockImplementation((activity: any) => Promise.resolve(activity))
            ...
            await request(app).post('/campaigns').send({ name: 'test', description: 'test' }).expect(201)
        })
