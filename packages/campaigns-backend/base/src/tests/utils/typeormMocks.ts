...
export const createMockRepository = <T extends object>(): MockRepository<T> => {
    ...
    return {
        ...
        create: jest.fn().mockImplementation((activity: any) => ({ ...activity, id: 'mock-id' })),
        save: jest.fn().mockImplementation((activity: any) => Promise.resolve({ ...activity, id: activity.id || 'mock-id' })),
        update: jest.fn().mockResolvedValue({ affected: 1 }),
        delete: jest.fn().mockResolvedValue({ affected: 1 }),
        remove: jest.fn().mockImplementation((activity: any) => Promise.resolve(activity))
    }
}
...
export const createMockDataSource = (repositories: RepoMap, options: { isInitialized?: boolean } = {}) => {
    const getRepository = jest.fn((activity: any) => {
        if (typeof activity === 'string') {
            return repositories[activity]
        }
        return repositories[activity?.name] || repositories[activity]
    })
