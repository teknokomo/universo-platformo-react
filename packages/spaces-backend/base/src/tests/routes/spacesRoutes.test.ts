...
describe('spacesRoutes', () => {
    ...
    it('возвращает 400 при попытке создать пространство без имени', async () => {
        ...
        const response = await request(app).post('/api/v1/uniks/unik-1/spaces').send({})
    ...
    it('проксирует GET /spaces/:spaceId/canvases/:canvasId в контроллер', async () => {
        const getSpy = jest.spyOn(CanvasController.prototype, 'getCanvasById').mockImplementation(async (_req, res) => {
            res.json({ ok: true })
        })

        const { app } = createTestServer()

        const response = await request(app).get('/api/v1/uniks/unik-1/spaces/space-1/canvases/canvas-1')
    ...
    it('проксирует PUT /spaces/:spaceId/canvases/:canvasId в контроллер', async () => {
        const updateSpy = jest.spyOn(CanvasController.prototype, 'updateCanvas').mockImplementation(async (_req, res) => {
            res.json({ updated: true })
        })

        const { app } = createTestServer()

        const response = await request(app).put('/api/v1/uniks/unik-1/spaces/space-1/canvases/canvas-1').send({ name: 'Updated' })
    ...
    it('проксирует DELETE /spaces/:spaceId/canvases/:canvasId в контроллер', async () => {
        const deleteSpy = jest.spyOn(CanvasController.prototype, 'deleteCanvas').mockImplementation(async (_req, res) => {
            res.json({ removed: true })
        })

        const { app } = createTestServer()

        const response = await request(app).delete('/api/v1/uniks/unik-1/spaces/space-1/canvases/canvas-1')
    ...
    it('проксирует GET /spaces/:spaceId/canvases/:canvasId/streaming в контроллер', async () => {
        ...
        const response = await request(app).get('/api/v1/uniks/unik-1/spaces/space-1/canvases/canvas-1/streaming')
