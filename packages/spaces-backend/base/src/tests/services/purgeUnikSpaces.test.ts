                ...
                switch (context.table) {
                    ...
                    case 'spaces_canvases': {
                        ...
                        if (clause.includes('space.unik_id')) {
                            ...
                            return state.spaces_canvases
                                .filter((row) => state.spaces.some((space) => space.id === row.space_id && space.unik_id === unikId))
                                .map((row) => ({ canvasId: row.canvas_id }))
                        }
                        return []
                    }
            ...
            async execute() {
                ...
                switch (context.operation) {
                    ...
                    case 'delete:chat_message_feedback':
                        state.chat_message_feedback = state.chat_message_feedback.filter((row) => !ids.includes(row.canvas_id))
                        return { affected: 1 }
...
describe('cleanupCanvasStorage', () => {
    ...
    it('запускает удаление для каждого холста и логирует ошибки', async () => {
        const remove = jest.fn().mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error('failed'))
        const warn = jest.fn()
