...
export class CanvasService {
    ...
    private async ensureCanvasLinkedToDefaultSpace(
        ...
        if (!unikId) {
            this.deps.logger.warn(`[spaces-backend]: Unable to ensure default Space relation for ${canvas.id}: unikId not provided`)
            return
        }
        ...
        try {
            ...
            if (existingSpace && existingSpace.unikId !== unikId) {
                ...
                this.deps.logger.warn(
                    `[spaces-backend]: Space ${existingSpace.id} belongs to unik ${existingSpace.unikId}, expected ${unikId}. Creating a dedicated default space.`
                )
                existingSpace = null
            }
            ...
            if (!existingSpace) {
                ...
                try {
                    ...
                    if (!spaceIdConflict) {
                        this.deps.logger.warn(
                            `[spaces-backend]: Failed to reuse space id ${canvas.id} for unik ${unikId}: ${
            ...
            this.deps.logger.warn(
                `[spaces-backend]: Unable to ensure Space/Canvas relation for ${canvas.id}: ${
        ...
        try {
            ...
            if (rawChatbotConfig != null) {
                try {
                    ...
                    this.deps.logger.warn(
                        '[spaces-backend] Failed to parse chatbotConfig for canvas %s: %s',
                        canvasId,
            ...
            try {
                ...
                this.deps.logger.warn(
                    '[spaces-backend] Failed to parse flowData for canvas %s: %s',
                    canvasId,
    ...
    async deleteCanvas(canvasId: string, scope?: CanvasScope): Promise<any> {
        try {
            ...
            try {
                ...
                this.deps.logger.error(`[spaces-backend]: Error deleting file storage for canvas ${canvasId}: ${cleanupError}`)
            }
