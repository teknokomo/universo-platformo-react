...
const parseJsonArray = (value: string | null, logger: { warn: (msg: string, meta?: any) => void } = console): string[] => {
    if (!value) return []
...
const deleteChatflowArtifacts = async (manager: EntityManager, canvasIds: string[]) => {
    ...
    await Promise.all(
        CANVAS_TABLES.map(({ table, column }) =>
            manager.createQueryBuilder().delete().from(table).where(`${column} IN (:...ids)`, { ids: canvasIds }).execute()
        )
    )
}

const collectSpaceIds = async (manager: EntityManager, unikId: string, explicitSpaceIds?: string[]): Promise<string[]> => {
    const query = manager
        .createQueryBuilder()
...
const filterDeletableCanvasIds = async (manager: EntityManager, candidateIds: string[]): Promise<string[]> => {
...
export const purgeSpacesForUnik = async (
    ...
    await manager.createQueryBuilder().delete().from('spaces_canvases').where('space_id IN (:...ids)', { ids: targetSpaceIds }).execute()

    await manager.createQueryBuilder().delete().from('spaces').where('id IN (:...ids)', { ids: targetSpaceIds }).execute()
    ...
    await manager.createQueryBuilder().delete().from('canvases').where('id IN (:...ids)', { ids: deletableCanvasIds }).execute()
...
export const cleanupCanvasStorage = async (
    ...
    const removals = await Promise.allSettled(canvasIds.map((canvasId) => Promise.resolve().then(() => removeFolder(canvasId))))
