...
1. Verify initialization order in `packages/universo-react-core-backend/src/index.ts`:
...
2. Check lazy router mounting in `packages/universo-react-core-backend/src/routes/index.ts`:
...
2. Verify using centralized `@universo-react/utils/rate-limiting`:

```typescript
// ✅ CORRECT: Import from centralized package
import { createRateLimiter } from '@universo-react/utils/rate-limiting'
