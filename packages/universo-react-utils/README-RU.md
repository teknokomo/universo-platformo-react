...
-   `@universo-react/database` владеет жизненным циклом Knex runtime и фабриками executor-ов.
-   `@universo-react/types` поставляет общие domain и enum contracts.
-   Backend packages используют `@universo-react/utils/database` как нейтральный SQL-first seam.
