...
import SourceDocDialog from '../dialog/SourceDocDialog'
import { MultiDropdown } from '../dropdown/MultiDropdown'
import { StyledButton } from '../button/StyledButton'
import StatsCard from '../cards/StatsCard'
import Feedback from '../extended/Feedback'
...
const ViewMessagesDialog = ({ show, dialogProps, onCancel }) => {
    ...
    const getCanvasMessagesApi = useApi(api.canvasMessages.getCanvasMessages)
    const getCanvasMessagesAscendingApi = useApi(api.canvasMessages.getCanvasMessagesAscending)
    const getStatsApi = useApi(api.feedback.getStatsFromCanvas)
    const getStoragePathFromServer = useApi(api.canvasMessages.getStoragePath)
    let storagePath = ''
    ...
    const deleteMessages = async (hardDelete) => {
        ...
        try {
            ...
            await api.canvasMessages.deleteCanvasMessages(targetCanvasId, obj)
            enqueueSnackbar({
    ...
    const clearChat = async (chatmsg) => {
        ...
        if (isConfirmed) {
            try {
                ...
                await api.canvasMessages.deleteCanvasMessages(canvasId, obj)
                const description =
                    chatmsg.sessionId && chatmsg.memoryType
    ...
    const agentReasoningArtifacts = (artifacts) => {
        ...
        for (let i = 0; i < newArtifacts.length; i++) {
            ...
            if (artifact && (artifact.type === 'png' || artifact.type === 'jpeg')) {
                ...
                newArtifacts[
                    i
                ].data = `${baseURL}/api/v1/get-upload-file?canvasId=${canvasId}&chatId=${selectedChatId}&fileName=${data.replace(
                    'FILE-STORAGE::',
                    ''
                )}`
            }
