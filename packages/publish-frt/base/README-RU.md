...
## Миграция на TypeScript (январь 2025)

Этот пакет был мигрирован с JavaScript/JSX на TypeScript/TSX с архитектурными улучшениями для лучшей поддерживаемости, типобезопасности и переиспользуемости кода.

### Обзор миграции

**Статус**: Основные публикаторы (AR.js и PlayCanvas) полностью мигрированы на TypeScript  
**Дата миграции**: Январь 2025  
**Подход**: Стратегия Copy-First Migration с инкрементальным рефакторингом

### Что было мигрировано

#### Основные публикаторы

-   ✅ **ARJSPublisher**: Полностью мигрирован на TypeScript с декомпозицией компонентов

    -   `ARJSPublisher.tsx` - Основной компонент публикатора (~250 строк)
    -   `MarkerSettings.tsx` - UI конфигурации маркера
    -   `TimerSettings.tsx` - UI конфигурации таймера
    -   `LibrarySettings.tsx` - Выбор источника библиотек
    -   `ARPreviewPane.tsx` - Компонент предпросмотра маркера

-   ✅ **PlayCanvasPublisher**: Полностью мигрирован на TypeScript
    -   `PlayCanvasPublisher.tsx` - Основной компонент публикатора
    -   Интегрирован с системой шаблонов для поддержки MMOOMM

#### Инфраструктура

-   ✅ **Конфигурация TypeScript**: Двойная система сборки (CommonJS + ES Modules)
-   ✅ **Интеграция TanStack Query**: Централизованное управление серверным состоянием
-   ✅ **Пользовательские хуки**: React-хуки для публикаций
-   ✅ **Система типов**: Комплексные интерфейсы и типы TypeScript
-   ✅ **Общие компоненты**: Переиспользуемые UI-компоненты с полной типобезопасностью

#### Вспомогательные компоненты

-   ✅ **ColyseusSettings.tsx** - Конфигурация мультиплеерного сервера
-   ✅ **GameModeSelector.tsx** - UI выбора игрового режима
-   ✅ **QRCodeSection.tsx** - Генерация и отображение QR-кодов
-   ✅ **svgToPng.ts** - Утилита конвертации SVG в PNG

### Архитектурные улучшения

#### 1. Интеграция TanStack Query

Все управление серверным состоянием теперь использует TanStack Query для консистентного кэширования, состояний загрузки и обработки ошибок:
...
#### 2. Пользовательские хуки публикации

Переиспользуемые хуки инкапсулируют общую логику публикации:

-   **`useVersionResolution`**: Разрешает идентификаторы групп версий из данных потока
-   **`usePublicationLinks`**: Управляет загрузкой и кэшированием ссылок публикации
-   **`usePublicationSettings`**: Обрабатывает загрузку/сохранение настроек с автосохранением (задержка 500мс)
-   **`usePublicationState`**: Управляет операциями создания/удаления публикаций
-   **`useCanvasData`**: Получает и кэширует данные canvas/flow
...
#### 3. Типобезопасные настройки

Все настройки публикации теперь строго типизированы:
...
Новые общие компоненты с полной поддержкой TypeScript:

-   **`PublicationToggle`**: Переключатель публичного/приватного состояния с состояниями загрузки
-   **`PublicationSettingsCard`**: Консистентный контейнер настроек с обработкой ошибок
-   **`AsyncStatusBar`**: Индикаторы загрузки и прогресса
-   **`FieldError`**: Унифицированное отображение ошибок
-   **`NetworkStatusBanner`**: Обнаружение офлайн-режима и функциональность повтора

#### 5. Декомпозиция компонентов

Большие монолитные компоненты были разделены на сфокусированные, поддерживаемые части:

**До**: `ARJSPublisher.jsx` (~900 строк)  
**После**:

-   `ARJSPublisher.tsx` (~250 строк) - Координатор
-   `MarkerSettings.tsx` - Конфигурация маркера
-   `TimerSettings.tsx` - Настройки таймера
-   `LibrarySettings.tsx` - Выбор библиотек
-   `ARPreviewPane.tsx` - Отображение предпросмотра

### Система сборки

#### Двойной вывод сборки

Пакет теперь поддерживает как CommonJS, так и ES Modules:

```json
{
    "main": "dist/index.js", // CommonJS
    "module": "dist/esm/index.js", // ES Modules
    "types": "dist/index.d.ts", // Определения типов
    "exports": {
        ".": {
...
```bash
pnpm run build:cjs    # Компиляция в CommonJS
pnpm run build:esm    # Компиляция в ES Modules
pnpm run build:assets # Копирование статических ресурсов
pnpm run build        # Полная сборка (все вышеперечисленное)
```

#### Конфигурация TypeScript

Две отдельные конфигурации TypeScript:

-   `tsconfig.json` - Вывод CommonJS (`dist/`)
-   `tsconfig.esm.json` - Вывод ES Modules (`dist/esm/`)

Обе конфигурации исключают `packages/flowise-ui` для предотвращения конфликтов с JSX-кодом из Flowise.

### Оптимизации производительности

-   **React.memo**: Применен к часто перерисовываемым компонентам
-   **useMemo**: Используется для дорогостоящих вычислений (генерация QR-кода, форматирование ссылок)
-   **useCallback**: Мемоизированные функции обратного вызова, передаваемые дочерним компонентам
-   **Ленивая загрузка**: Динамические импорты для тяжелых зависимостей (генерация ZIP)
-   **Кэширование запросов**: TanStack Query сокращает избыточные вызовы API

### Улучшения доступности

-   **Семантический HTML**: Заменены `div` с `onClick` на правильные элементы `button`
-   **ARIA-атрибуты**: Добавлены `aria-label`, `aria-live`, `aria-pressed` где необходимо
-   **Навигация с клавиатуры**: Полная поддержка клавиатуры для всех интерактивных элементов
-   **Управление фокусом**: Видимые индикаторы фокуса для всех элементов управления
-   **Поддержка программ чтения с экрана**: Правильная маркировка и живые регионы для динамического контента

### Обработка ошибок

-   **Централизованные уведомления**: Утилита `notifications.ts` для консистентных сообщений об ошибках
-   **Обнаружение сети**: Автоматический баннер офлайн-режима с функциональностью повтора
-   **Валидация**: Валидация ввода с понятными сообщениями об ошибках
-   **Логика повтора**: Встроенный повтор для неудачных операций с экспоненциальной задержкой
-   **Границы ошибок**: Graceful обработка ошибок в дереве компонентов

### Что остается в JSX

Следующие компоненты остаются в JSX и не являются частью этой миграции:

-   Функции чатбота (`features/chatbot/`)
-   Компоненты диалогов (`features/dialog/`)
-   Совместное использование кода API (`features/api/`)
-   Демо экспортера AR.js (`features/arjs/ARJSExporter.jsx`)

Эти компоненты корректно работают с инфраструктурой TypeScript и могут быть мигрированы в будущих итерациях при необходимости.

### Критические изменения

#### Для потребителей

1. **Пути импорта**: Без изменений - все экспорты остаются прежними
2. **Пропсы компонентов**: Полностью обратно совместимы
3. **API-контракты**: Нет изменений в публичных API

#### Для разработчиков

1. **Требования к типам**: Весь новый код должен использовать TypeScript
2. **Строгий режим**: Включены `noImplicitAny`, `strictNullChecks`
3. **Использование хуков**: Предпочитайте пользовательские хуки прямым вызовам API

### Преимущества миграции

-   ✅ **Типобезопасность**: Обнаружение ошибок во время компиляции вместо выполнения
-   ✅ **Лучшая поддержка IDE**: Полное автодополнение и встроенная документация
-   ✅ **Улучшенная поддерживаемость**: Меньшие, сфокусированные компоненты
-   ✅ **Переиспользуемость кода**: Общие хуки и компоненты
-   ✅ **Производительность**: Оптимизированные перерисовки и кэширование
-   ✅ **Доступность**: Интерфейсы, соответствующие WCAG 2.1
-   ✅ **Опыт разработчика**: Более быстрая разработка с инструментами TypeScript

### Тестирование

Миграция включает:

-   Модульные тесты для утилитарных функций (`timerConfig`, `base58Validator`)
-   Проверка типов через `tsc --noEmit`
-   Валидация ESLint с правилами TypeScript
-   Ручное smoke-тестирование всех рабочих процессов публикации

### Будущая работа

-   [ ] Мигрировать оставшиеся JSX-компоненты (chatbot, dialog, api features)
-   [ ] Добавить интеграционные тесты для рабочих процессов публикации
-   [ ] Реализовать E2E-тесты с Playwright
-   [ ] Добавить Storybook для документации компонентов
-   [ ] Профилирование производительности и оптимизация
-   [ ] Аудит доступности с автоматизированными инструментами

### Документация

-   [Документация пользовательских хуков](./src/hooks/publication/README.md)
-   [Документация общих компонентов](./src/components/shared/README.md)
-   [Определения типов](./src/types/)
-   [Требования миграции](./.kiro/specs/publish-frt-typescript-migration/requirements.md)
-   [Дизайн миграции](./.kiro/specs/publish-frt-typescript-migration/design.md)
...
-   **UPDLProcessor**: Центральный класс для обработки потоков UPDL (мигрировал из `packages/flowise-server/src/utils/buildUPDLflow.ts`).
...
-   **Независимость**: Отсутствие прямых импортов из `packages/flowise-server`, что обеспечивает полную модульную независимость.
...
Это фронтенд-приложение тесно интегрировано с основной системой публикации ботов, расположенной в `packages/flowise-ui/src/views/publish/bots/`:
