...
**Мигрировано из:** `packages/flowise-core-frontend/base/src/views/publish/` и `packages/flowise-core-frontend/base/src/views/canvases/`  
...
**Реализованное решение:** Единый глобальный `QueryClient` теперь создается в корне UI (`packages/flowise-core-frontend/base/src/index.jsx`). Все функции публикации используют этот общий клиент напрямую, устраняя избыточные провайдеры и штормы сетевых запросов.

#### Глобальный провайдер QueryClient

**Расположение:** `packages/flowise-core-frontend/base/src/index.jsx`
...
Все ключи i18n, связанные с публикацией, мигрированы из `packages/flowise-core-frontend/base/src/i18n/locales/{en,ru}/views/canvases.json` в `packages/publish-frontend/base/src/i18n/locales/{en,ru}/main.json`.
...
1. **Несовместимость CommonJS/ESM:** Прямые импорты компонентов `publish-frontend` в `flowise-ui` не работают из-за того, что Vite ожидает ESM, а TypeScript производит CommonJS.
   - **Влияние:** Пока нельзя использовать `import { PublishDialog } from 'publish-frontend'` в основном UI
...
- [ ] Включить прямые импорты `publish-frontend` в `flowise-ui`
- [ ] Удалить оригинальные файлы из `packages/flowise-core-frontend/base` после подтверждения стабильности
...
✅ **Статус сборки:** И `publish-frontend`, и `flowise-ui` собираются успешно  
...
Экспортируется из `@packages/publish-frontend/base/src/api`
...
```javascript
// Автоматически включены в packages/flowise-core-frontend/base/src/index.jsx
...
### Сборка
```bash
# Собрать только publish-frontend
pnpm --filter publish-frontend build

# Собрать publish-frontend + flowise-ui
...
**Система типов**: UPDL типы импортируются из пакета `@universo/publish-backend`, обеспечивая централизованные определения типов и согласованность между frontend и backend компонентами.
...
#### Конфигурация сервера (packages/flowise-core-backend/base/src/index.ts)
...
-   **`Handlers`**: Специализированные процессоры для различных типов UPDL нод теперь инкапсулированы внутри каждого шаблона (например, `builders/templates/quiz/arjs/handlers/`). MMOOMM handlers теперь находятся внутри `@universo/template-mmoomm`. Это делает каждый шаблон самодостаточным, сохраняя `publish-frontend` компактным.
...
-   **UPDLProcessor**: Центральный класс для обработки UPDL flow (мигрирован из `packages/flowise-core-backend/base/src/utils/buildUPDLflow.ts`)
-   **Импорт типов**: UPDL типы импортированы из пакета `@universo/publish-backend`
...
-   **Совместное использование типов**: UPDL типы централизованы в пакете `@universo/publish-backend` и импортируются frontend
-   **Сервисный слой**: Backend предоставляет `FlowDataService` для управления данными flow
-   **Независимость**: Нет прямых импортов из `packages/flowise-core-backend/base` - полная модульная независимость
