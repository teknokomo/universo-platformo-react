...
// Helper function moved here to avoid external dependency
const getUniqueNodeId = (nodeData, existingNodes) => {
    const ids = existingNodes.map((node) => node.id)
    let index = 1
    let newId = `${nodeData.name}_${index}`
    while (ids.includes(newId)) {
        index++
        newId = `${nodeData.name}_${index}`
    }
    return newId
}
