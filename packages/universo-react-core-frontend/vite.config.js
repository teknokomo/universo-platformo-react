...
const supportedLanguagesPath = require.resolve('@universo-react/i18n/supported-languages.json')
const supportedLanguages = JSON.parse(fs.readFileSync(supportedLanguagesPath, 'utf-8'))
const frontendRootDir = resolve(__dirname)
const backendRootDir = resolve(__dirname, '../universo-react-core-backend')
...
export default defineConfig(async ({ mode }) => {
    ...
    return {
        ...
        optimizeDeps: {
            ...
            exclude: ['@universo-react/auth-frontend', 'react-i18next', 'i18next'],
        ...
        resolve: {
            ...
            dedupe: [
                ...
                'react-i18next',
                'i18next',
                '@universo-react/auth-frontend',
                '@universo-react/template-mui',
                '@universo-react/store',
                '@universo-react/i18n'
            ],
            alias: [
                ...
                // Force browser version of @universo-react/utils (only bare import)
                { find: /^@universo-react\/utils$/, replacement: resolve(__dirname, '../universo-react-utils/src/index.browser.ts') },
                {
                    find: /^@universo-react\/utils\/env$/,
                    replacement: resolve(__dirname, '../universo-react-utils/src/env/index.browser.ts')
                },
                {
                    find: /^@universo-react\/utils\/validation\/codename$/,
                    replacement: resolve(__dirname, '../universo-react-utils/src/validation/codename.ts')
                },
                { find: /^@universo-react\/utils\/vlc$/, replacement: resolve(__dirname, '../universo-react-utils/src/vlc/index.ts') },
                {
        ...
        build: {
            ...
            target: 'es2022',
            commonjsOptions: {
                include: [/@universo-react\//, /node_modules/],
