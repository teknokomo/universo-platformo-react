...
✅ **Современный пакет** — `@universo-react/core-frontend`
...
-   **Аутентификация**: `@universo-react/auth-frontend` — вход, сессия, защищённые маршруты
-   **Профиль**: `@universo-react/profile-frontend` — управление профилем пользователя
-   **Приложения**: `@universo-react/applications-frontend` — CRUD приложений
-   **Метахабы**: `@universo-react/metahubs-frontend` — управление метахабами
-   **API клиент**: `@universo-react/api-client` — централизованный API-слой на базе Axios
-   **UI компоненты**: `@universo-react/template-mui` — общая MUI тема, лейаут, навигация
...
```
packages/universo-react-core-frontend/
...
Префикс `@/` разрешается в директорию `src/`. Все импорты пакетов workspace используют область видимости `@universo-react/` через PNPM workspace resolution.
...
-   [universo-core-backend](../universo-react-core-backend/README-RU.md) — Express бэкенд-сервер
-   [universo-template-mui](../universo-react-template-mui/README-RU.md) — Общие UI компоненты и тема
-   [universo-i18n](../universo-react-i18n/README-RU.md) — Система интернационализации
-   [auth-frontend](../universo-react-auth-frontend/README-RU.md) — UI аутентификации
