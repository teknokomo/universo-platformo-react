...
describe('WorkspaceAccessService', () => {
    ...
    describe('role validation', () => {
        ...
        it('throws when user role is insufficient', async () => {
            ...
            await expect(service.requireUnikRole(req, 'user-1', 'unik-1', { allowedRoles: ['owner'] })).rejects.toMatchObject({
                status: 403
            })
        })
        ...
        it('throws when database contains invalid role', async () => {
            ...
            await expect(service.requireUnikRole(req, 'user-1', 'unik-1')).rejects.toMatchObject({
                status: 500,
        ...
        it('allows access when no role restrictions specified', async () => {
            ...
            await expect(service.requireUnikRole(req, 'user-1', 'unik-1')).resolves.toEqual(membership)
        })
        ...
        it('allows higher roles to access lower role operations (hierarchy)', async () => {
            ...
            await expect(service.requireUnikRole(req1, 'user-1', 'unik-1', { allowedRoles: ['admin'] })).resolves.toEqual(ownerMembership)
            ...
            await expect(service.requireUnikRole(req2, 'user-2', 'unik-1', { allowedRoles: ['editor'] })).resolves.toEqual(adminMembership)
            ...
            await expect(service.requireUnikRole(req3, 'user-3', 'unik-1', { allowedRoles: ['member'] })).resolves.toEqual(editorMembership)
        })
    })
    ...
    describe('error handling', () => {
        it('throws 403 when user not found in membership', async () => {
            ...
            await expect(service.requireUnikRole(req, 'user-1', 'unik-1')).rejects.toMatchObject({ status: 403 })
        })
    })
})
