...
```typescript
import { createAuthUserProvisioningService, createGlobalUsersRoutes, createGlobalAccessService } from '@universo-react/admin-backend'
import { getPermissionService } from '@universo-react/auth-backend'
...
-   `@universo-react/admin-frontend` - Фронтенд UI для админ-панели
-   `@universo-react/auth-backend` - Аутентификация и guard'ы доступа
