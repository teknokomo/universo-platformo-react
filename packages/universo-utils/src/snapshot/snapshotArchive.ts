...
export function computeSnapshotHash(snapshot: Record<string, unknown>): string {
    const normalized = normalizePublicationSnapshotForHash(snapshot as PublicationSnapshotHashInput)
 ...
 */
export function validateSnapshotEnvelope(raw: unknown): MetahubSnapshotTransportEnvelope {
    // 0. Type guard
