...
interface CatalogSystemFieldSeedDefinition extends CatalogSystemFieldDefinition {
    name: VersionedLocalizedContent<string>
    description: VersionedLocalizedContent<string>
}

export interface CatalogSystemFieldToggleValidationResult {
    normalized: CatalogSystemFieldState[]
    errors: string[]
}
...
const SYSTEM_FIELD_REGISTRY: readonly CatalogSystemFieldSeedDefinition[] = [
    {
        key: 'app.published',
...
const SYSTEM_FIELD_DEFINITION_MAP = new Map<CatalogSystemFieldKey, CatalogSystemFieldSeedDefinition>(
    SYSTEM_FIELD_REGISTRY.map((definition) => [definition.key, definition])
)

const normalizeStatesMap = (states?: CatalogSystemFieldState[] | null): Map<CatalogSystemFieldKey, boolean> => {
    const map = new Map<CatalogSystemFieldKey, boolean>()
...
export const getCatalogSystemFieldDefinitions = (): CatalogSystemFieldDefinition[] =>
    SYSTEM_FIELD_REGISTRY.map(({ name: _name, description: _description, ...definition }) => ({ ...definition }))

export const getCatalogSystemFieldDefinition = (key: CatalogSystemFieldKey): CatalogSystemFieldDefinition | undefined => {
...
export const getDefaultCatalogSystemFieldStates = (): CatalogSystemFieldState[] =>
    SYSTEM_FIELD_REGISTRY.map((definition) => ({ key: definition.key, enabled: definition.defaultEnabled }))

export const validateCatalogSystemFieldToggleSet = (
    states?: CatalogSystemFieldState[] | null
...
export const deriveApplicationLifecycleContract = (states?: CatalogSystemFieldState[] | null): ApplicationLifecycleContract => {
    const validationResult = validateCatalogSystemFieldToggleSet(states)
    if (validationResult.errors.length > 0) {
        throw new Error(validationResult.errors.join('; '))
    }

    const enabledMap = new Map<CatalogSystemFieldKey, boolean>(validationResult.normalized.map((state) => [state.key, state.enabled]))
...
export const derivePlatformSystemFieldsContract = (states?: CatalogSystemFieldState[] | null): PlatformSystemFieldsContract => {
    const validationResult = validateCatalogSystemFieldToggleSet(states)
    if (validationResult.errors.length > 0) {
        throw new Error(validationResult.errors.join('; '))
    }
...
export const resolvePlatformSystemFieldsContractFromConfig = (config?: unknown): PlatformSystemFieldsContract => {
    ...
    const rawStates =
        rawSystemFields && typeof rawSystemFields === 'object'
            ? (rawSystemFields as { fields?: CatalogSystemFieldState[] }).fields
            : undefined
