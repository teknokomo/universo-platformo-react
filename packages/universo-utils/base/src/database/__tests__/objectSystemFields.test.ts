...
describe('objectSystemFields', () => {
    it('exposes a unique registry derived from canonical system field names', () => {
        const definitions = getObjectSystemComponents()
    ...
    it('returns localized seed rows with default enabled state', () => {
        const rows = getObjectSystemComponentSeedInputs()
    ...
    it('builds shared seed records and reserved codenames from the same registry', () => {
        const seedInput = getObjectSystemComponentSeedInputs().find((row) => row.key === 'app.deleted')
        const record = buildObjectSystemComponentSeedRecord(seedInput!)
        const reserved = getReservedObjectSystemFieldCodenames()
    ...
    it('rejects invalid toggle combinations', () => {
        const result = validateObjectSystemFieldToggleSet([
