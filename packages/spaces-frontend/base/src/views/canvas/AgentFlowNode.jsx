// Universo Platformo | AgentFlow Node Component
// Compact node rendering for AgentFlow canvas (Flowise 3.x style)
// Adapted from Flowise 3.0.12 AgentFlowNode.jsx
...
import { MainCard, AGENTFLOW_ICONS } from '@flowise/template-mui'
...
const AgentFlowNode = ({ data }) => {
    ...
    const getAnchorPosition = (index) => {
        ...
        const outputPosition = spacing * (index + 1)
    ...
    useEffect(() => {
        ...
        const componentNode = canvas.componentNodes?.find?.((nd) => nd.name === data.name) || canvas.componentNodes?.[data.name]
        if (componentNode) {
            if (!data.version) {
                            ...
                            {data.color && !data.icon ? (
                                ...
                                >
                                    <img
                                        ...
                                        src={`/api/v1/node-icon/${data.name}`}
                                        alt={data.name}
                                    />
                                </div>
                            )}
                                        ...
                                        <Box key={`model-${index}`} sx={{ display: 'flex', gap: 1, mt: 1 }}>
                                            ...
                                            >
                                                <img
                                                    ...
                                                    src={`/api/v1/node-icon/${item.model}`}
                                                    alt={item.model}
                                                />
