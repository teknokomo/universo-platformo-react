...
// const
import { initNode, initializeDefaultNodeData } from '../../utils/genericHelper'
import { showHideInputParams } from '../../utils/showHideInputParams'
import { flowContext, FLOWISE_CREDENTIAL_ID } from '@flowise/store'
...
export const ConfigInput = ({ data, inputParam, disabled = false, arrayIndex = null, parentParamForArray = null }) => {
    ...
    useEffect(() => {
        const loadComponentData = async () => {
            ...
            try {
                const node = await api.nodes.getSpecificNode(nodeName)

                if (!node) return

                // Initialize component node with basic data
                const componentNodeData = cloneDeep(initNode(node, `${node.name}_0`))
                ...
                console.error('[ConfigInput] Error loading component data:', error)
            }
        }
    ...
    useEffect(() => {
        ...
        // If the main model value changed, we need to reload the node definition from API
        // Otherwise just update the component data with new config values
        const updateComponentData = async () => {
            // When main value (model name) changes, reload from API
            if (hasMainValueChanged) {
                const nodeName = currentInputValues.mainValue
                if (!nodeName) return
                ...
                try {
                    ...
                    const freshComponentData = cloneDeep(initNode(node, `${node.name}_0`))
                    ...
                    // Check if we have existing config for this new model
                    const currentValue = getCurrentInputValue()
                    if (hasArrayConfig() || data.inputs[`${inputParam.name}Config`]) {
                        const configData = getConfigData()
                        if (configData && configData[inputParam.name] === currentValue) {
                            // Config matches, use it
                            freshComponentData.inputs = { ...configData, [inputParam.name]: currentValue }
                        } else {
                            // No matching config, use defaults
                            const defaultInput = initializeDefaultNodeData(freshComponentData.inputParams)
                            freshComponentData.inputs = { ...defaultInput, [inputParam.name]: currentValue }
                        }
                    } else {
                        // No config at all, set the model name
                        freshComponentData.inputs = {
                            ...freshComponentData.inputs,
                            [inputParam.name]: currentValue
                        }
                    }
                    ...
                    freshComponentData.inputParams = showHideInputParams({
                        ...freshComponentData,
                        inputs: freshComponentData.inputs
                    })

                    const credential = freshComponentData.inputs.credential || freshComponentData.inputs[FLOWISE_CREDENTIAL_ID]
                    freshComponentData.credential = credential ? credential : undefined

                    setSelectedComponentNodeData(freshComponentData)
                    ...
                    console.error('[ConfigInput] Error reloading component data:', error)
                }
                return
            }

            // If only config changed (not main value), just update from current selectedComponentNodeData
