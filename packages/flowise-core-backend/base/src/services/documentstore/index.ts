...
import { DocumentStore, DocumentStoreFileChunk, UpsertHistory, DocumentStoreDTO } from '@flowise/docstore-backend'
 ...
 * Delegates to @flowise/docstore-backend service
 */
 ...
 * Delegates DB operations to @flowise/docstore-backend service.
...
const deleteDocumentStore = async (storeId: string, unikId?: string) => {
    try {
        ...
        // Delete files from storage (requires flowise-components, cannot be moved to docstore-backend)
        await removeFilesFromStorage(DOCUMENT_STORE_BASE_FOLDER, entity.id)

        // Delegate DB deletion (chunks, upsert history, store) to docstore-backend
