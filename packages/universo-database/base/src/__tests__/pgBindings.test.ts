...
describe('convertPgBindings', () => {
    ...
    it('handles mixed order and repeated refs ($1 AND $2 OR $1 → 3 bindings from 2 params)', () => {
        const result = convertPgBindings(
            'SELECT * FROM t WHERE a = $1 AND b = $2 OR c = $1',
            ['x', 'y']
        )
        expect(result).toEqual({
    ...
    it('throws on out-of-range parameter $5 with 1 param', () => {
        expect(() => convertPgBindings('SELECT $5', ['only-one'])).toThrow(
            'Binding $5 references index 4 but only 1 params provided'
        )
    })

    it('throws on $0 (zero-indexed)', () => {
        expect(() => convertPgBindings('SELECT $0', ['a'])).toThrow(
            'Binding $0 references index -1 but only 1 params provided'
        )
    })

    it('throws on mixed ?/$N placeholders', () => {
        expect(() =>
            convertPgBindings('SELECT ? FROM t WHERE id = $1', ['a', 'b'])
