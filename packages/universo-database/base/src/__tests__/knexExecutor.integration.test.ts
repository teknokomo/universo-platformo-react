...
describeIntegration('Executor Integration (requires PG)', () => {
    ...
    it('pool executor handles $1-style params via conversion', async () => {
        ...
        const rows = await exec.query<{ id: number; name: string }>(
            'SELECT * FROM _test_exec WHERE id = $1',
            [1]
        )
    ...
    it('pool executor handles queries without parameters', async () => {
        ...
        const rows = await exec.query<{ name: string }>(
            'SELECT name FROM _test_exec WHERE id = 99'
        )
    ...
    it('RLS executor runs queries on pinned connection', async () => {
        ...
        try {
            ...
            const rows = await exec.query<{ name: string }>(
                'SELECT name FROM _test_exec WHERE id = $1',
                [2]
            )
    ...
    it('pool executor transaction rolls back on error', async () => {
        ...
        const rows = await exec.query<{ id: number }>(
            'SELECT id FROM _test_exec WHERE id IN ($1, $2)',
            [10, 11]
        )
