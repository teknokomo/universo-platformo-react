...
import { exportRuntimeReportCsv, runRuntimeReport } from '../api'
...
describe('runtime report API helpers', () => {
    ...
    it('exports a saved report through the runtime reports endpoint with CSRF protection', async () => {
        const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
            ...
            expect(url).toBe('http://localhost:3000/api/v1/applications/app-1/runtime/reports/export?workspaceId=workspace-1')
        ...
        await expect(blob.text()).resolves.toBe('Progress\r\n75\r\n')
        const reportRequest = fetchMock.mock.calls[1]?.[1] as RequestInit
        expect(reportRequest.method).toBe('POST')
        expect(reportRequest.body).toBe(JSON.stringify({ reportCodename: 'LearnerProgress', limit: 10, offset: 20, locale: 'en' }))
        expect(new Headers(reportRequest.headers).get('X-CSRF-Token')).toBe('csrf-token')
    })
})
