...
export class SpacesApi {
    ...
    async create(
        ...
        const response = await this.client.post<Space>(
            `/unik/${unikId}/spaces`,
            payload
        )
        return response.data
    ...
    async update(
        ...
        const response = await this.client.put<Space>(
            `/unik/${unikId}/spaces/${spaceId}`,
            payload
        )
        return response.data
...
export const spaceQueryKeys = {
    ...
    list: (unikId: string) =>
        [...spaceQueryKeys.lists(), { unikId }] as const,
    details: () => [...spaceQueryKeys.all, 'detail'] as const,
    detail: (unikId: string, spaceId: string) =>
        [...spaceQueryKeys.details(), { unikId, spaceId }] as const,
} as const
