...
export class AssistantsApi {
     ...
     */
    async getOpenAIAssistant(
        unikId: string,
        assistantId: string,
        credentialId: string
    ): Promise<OpenAIAssistant> {
        const response = await this.client.get<OpenAIAssistant>(
            `/unik/${unikId}/openai-assistants/${assistantId}`,
    ...
    async update(
        ...
        const response = await this.client.put<Assistant>(
            `/unik/${unikId}/assistants/${id}`,
            payload
        )
        return response.data
    ...
    async delete(
        ...
        await this.client.delete(
            `/unik/${unikId}/assistants/${id}`,
            isDeleteBoth ? { params: { isDeleteBoth: 'true' } } : undefined
        )
    }
    ...
    async getVectorStore(
        ...
        const response = await this.client.get<VectorStore>(
    ...
    async listVectorStores(
        ...
        const response = await this.client.get<VectorStore[]>(
            `/unik/${unikId}/openai-assistants-vector-store`,
            { params: { credential: credentialId } }
        )
        return response.data
    }
    ...
    async createVectorStore(
        ...
        const response = await this.client.post<VectorStore>(
            `/unik/${unikId}/openai-assistants-vector-store`,
            payload,
            { params: { credential: credentialId } }
        )
    ...
    async updateVectorStore(
        ...
        const response = await this.client.put<VectorStore>(
            `/unik/${unikId}/openai-assistants-vector-store/${vectorStoreId}`,
            payload,
            { params: { credential: credentialId } }
        )
    ...
    async uploadFilesToVectorStore(
        ...
        const response = await this.client.post<OpenAIFile[]>(
            `/unik/${unikId}/openai-assistants-vector-store/${vectorStoreId}`,
            formData,
            {
                params: { credential: credentialId },
     ...
     */
    async uploadFilesToAssistant(
        unikId: string,
        credentialId: string,
        formData: FormData
    ): Promise<OpenAIFile[]> {
        const response = await this.client.post<OpenAIFile[]>(
            `/unik/${unikId}/openai-assistants-file/upload`,
            formData,
            {
                params: { credential: credentialId },
                headers: { 'Content-Type': 'multipart/form-data' },
    ...
    async getChatModels(unikId: string): Promise<AssistantComponent[]> {
        const response = await this.client.get<AssistantComponent[]>(
            `/unik/${unikId}/assistants/components/chatmodels`
        )
        return response.data
    }
    ...
    async getDocStores(unikId: string): Promise<AssistantComponent[]> {
        const response = await this.client.get<AssistantComponent[]>(
            `/unik/${unikId}/assistants/components/docstores`
        )
        return response.data
    }
    ...
    async getTools(unikId: string): Promise<AssistantComponent[]> {
        const response = await this.client.get<AssistantComponent[]>(
            `/unik/${unikId}/assistants/components/tools`
        )
        return response.data
    }
...
export const assistantQueryKeys = {
    ...
    list: (unikId: string, type?: string) =>
        [...assistantQueryKeys.lists(), { unikId, type }] as const,
    
    details: () => [...assistantQueryKeys.all, 'detail'] as const,
    detail: (unikId: string, id: string) =>
        [...assistantQueryKeys.details(), { unikId, id }] as const,
    
    // OpenAI API queries
    openai: () => [...assistantQueryKeys.all, 'openai'] as const,
    openaiList: (unikId: string, credentialId: string) =>
        [...assistantQueryKeys.openai(), 'list', { unikId, credentialId }] as const,
    ...
    chatModels: (unikId: string) =>
        [...assistantQueryKeys.components(), 'chatModels', { unikId }] as const,
    docStores: (unikId: string) =>
        [...assistantQueryKeys.components(), 'docStores', { unikId }] as const,
    tools: (unikId: string) =>
        [...assistantQueryKeys.components(), 'tools', { unikId }] as const,
} as const
