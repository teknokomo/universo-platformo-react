...
import type { Credential, CredentialComponent, CreateCredentialPayload, UpdateCredentialPayload } from '../types/credential'
...
export class CredentialsApi {
    ...
    async getAll(unikId: string): Promise<Credential[]> {
        const response = await this.client.get<Credential[]>(`/unik/${unikId}/credentials`)
     ...
     */
    async getByName(unikId: string, componentCredentialName: string): Promise<Credential[]> {
        const response = await this.client.get<Credential[]>(`/unik/${unikId}/credentials`, {
    ...
    async getAllComponents(): Promise<CredentialComponent[]> {
        const response = await this.client.get<CredentialComponent[]>('/components-credentials')
        return response.data
    }
    ...
    async getById(unikId: string, id: string): Promise<Credential> {
        const response = await this.client.get<Credential>(`/unik/${unikId}/credentials/${id}`)
        return response.data
    }
    ...
    async getComponentSchema(name: string): Promise<CredentialComponent> {
        const response = await this.client.get<CredentialComponent>(`/components-credentials/${name}`)
        return response.data
    }
     ...
     */
    async create(unikId: string, payload: CreateCredentialPayload): Promise<Credential> {
        const response = await this.client.post<Credential>(`/unik/${unikId}/credentials`, payload)
        return response.data
    }
     ...
     */
    async update(unikId: string, id: string, payload: UpdateCredentialPayload): Promise<Credential> {
        const response = await this.client.put<Credential>(`/unik/${unikId}/credentials/${id}`, payload)
        return response.data
    }
...
export const credentialQueryKeys = {
    ...
    list: (unikId: string) => [...credentialQueryKeys.lists(), { unikId }] as const,

    /** Credential list filtered by component name */
    listByName: (unikId: string, name: string) => [...credentialQueryKeys.lists(), { unikId, name }] as const,

    /** All detail queries */
    details: () => [...credentialQueryKeys.all, 'detail'] as const,

    /** Specific credential detail */
    detail: (unikId: string, id: string) => [...credentialQueryKeys.details(), { unikId, id }] as const,

    /** All component schema queries */
    components: () => [...credentialQueryKeys.all, 'components'] as const,

    /** Specific component schema */
    component: (name: string) => [...credentialQueryKeys.components(), { name }] as const
} as const
