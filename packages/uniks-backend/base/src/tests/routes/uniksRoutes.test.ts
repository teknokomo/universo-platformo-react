...
const { purgeSpacesForUnik, cleanupCanvasStorage, createSpacesRoutes } = require('@universo/spaces-backend') as {
...
describe('uniks routes (TypeORM)', () => {
    ...
    const buildIndividualApp = (userId: string, repos: { unikRepo: any; membershipRepo: any }, options?: { transaction?: jest.Mock }) => {
    ...
    it('forbids member addition without owner rights', async () => {
        const unikRepo = createMockRepository<any>()
        const membershipRepo = createMockRepository<any>()
        membershipRepo.findOne.mockResolvedValue({ role: 'editor' })

        const { app } = buildCollectionApp('user-7', { unikRepo, membershipRepo })
        const response = await request(app).post('/members').send({ unik_id: 'unik-9', user_id: 'member-1', role: 'editor' })

        expect(response.status).toBe(403)
    })

    it('updates unik by editor (editor allowed)', async () => {
        const unikRepo = createMockRepository<any>()
        const membershipRepo = createMockRepository<any>()
        membershipRepo.findOne.mockResolvedValue({ role: 'editor' })
    ...
    it('deletes unik by owner with cascading cleanup', async () => {
        const unikRepo = createMockRepository<any>()
        const membershipRepo = createMockRepository<any>()
        ...
        expect(cleanupCanvasStorage).toHaveBeenCalledWith(['canvas-1'], removeFolderFromStorage, { source: 'Uniks' })
        purgeSpacesForUnik.mockReset()
        cleanupCanvasStorage.mockReset()
    })

    it('does not call storage cleanup if helper did not delete canvases', async () => {
        const unikRepo = createMockRepository<any>()
        const membershipRepo = createMockRepository<any>()
        membershipRepo.findOne.mockResolvedValue({ role: 'owner' })
        const managerUnikRepo = {
    ...
    it('returns 404 if unik not found inside transaction', async () => {
        const unikRepo = createMockRepository<any>()
        const membershipRepo = createMockRepository<any>()
        membershipRepo.findOne.mockResolvedValue({ role: 'owner' })
    ...
    it('connects spaces routes inside createUniksRouter', async () => {
        const unikRepo = createMockRepository<any>()
        const membershipRepo = createMockRepository<any>()
        ...
        expect(createSpacesRoutes).toHaveBeenCalledWith(
            expect.any(Function),
            expect.objectContaining({ canvasService: expect.any(Object) })
        )
    })

    it('proxies canvas routes with unikId and spaceId parameters', async () => {
