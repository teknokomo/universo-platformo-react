...
-   `src/platform/migrations/` exports the native SQL definitions consumed by `@universo-react/migrations-platform`.
...
-   `@universo-react/auth-backend` supplies authenticated request context and RLS middleware.
-   `@universo-react/database` supplies the shared Knex runtime and executor factories.
-   `@universo-react/migrations-platform` registers the package's native SQL platform definitions.
