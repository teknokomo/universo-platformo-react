...
2. Инициализировать shared Knex singleton из `@universo-react/database`.
3. Провалидировать и выполнить registered platform migrations через `@universo-react/migrations-platform`.
...
7. Смонтировать роутеры `/api/v1` и отдать frontend bundle из `@universo-react/core-frontend`.

## Key Integrations

-   `@universo-react/database` предоставляет `getKnex()`, `destroyKnex()` и executor factories.
-   `@universo-react/migrations-platform` отвечает за startup validation и unified platform migration execution.
-   `@universo-react/auth-backend` предоставляет Passport setup, auth routes и RLS-aware auth middleware.
-   `@universo-react/metahubs-backend`, `@universo-react/applications-backend`, `@universo-react/start-backend`, `@universo-react/admin-backend` и `@universo-react/profile-backend` предоставляют domain routers, которые монтирует core server.
...
-   Канонический database runtime основан на Knex и шарится через `@universo-react/database`.
