# Universo Core Backend (@universo-react/core-backend)
...
2. Initialize the shared Knex singleton from `@universo-react/database`.
3. Validate and run registered platform migrations through `@universo-react/migrations-platform`.
...
7. Mount `/api/v1` routers and serve the frontend bundle from `@universo-react/core-frontend`.

## Key Integrations

-   `@universo-react/database` provides `getKnex()`, `destroyKnex()`, and executor factories.
-   `@universo-react/migrations-platform` owns startup validation and unified platform migration execution.
-   `@universo-react/auth-backend` provides Passport setup, auth routes, and RLS-aware auth middleware.
-   `@universo-react/metahubs-backend`, `@universo-react/applications-backend`, `@universo-react/start-backend`, `@universo-react/admin-backend`, and `@universo-react/profile-backend` provide domain routers mounted by the core server.
...
Application realtime WebSocket upgrades are stricter than HTTP CORS. Configure `APPLICATION_REALTIME_WS_ORIGINS` with an explicit comma-separated origin list, for example `http://127.0.0.1:3100,http://localhost:3100`. Wildcard origins are rejected for WebSocket upgrades.
...
-   The canonical database runtime is Knex-based and shared through `@universo-react/database`.
