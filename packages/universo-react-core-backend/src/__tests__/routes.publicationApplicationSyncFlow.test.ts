...
import { buildRegisteredSystemAppSchemaGenerationPlan } from '@universo-react/migrations-platform'
...
jest.mock('@universo-react/auth-backend', () => {
...
jest.mock('@universo-react/metahubs-backend', () => {
...
jest.mock('@universo-react/start-backend', () => {
    const expressModule = require('express') as typeof import('express')
...
jest.mock('@universo-react/profile-backend', () => {
    const expressModule = require('express') as typeof import('express')
...
describe('core route composition publication -> application sync flow', () => {
    ...
    it('routes publication bootstrap into application sync through the shared runtime-source seam', async () => {
        ...
        const metahubsModule = jest.requireMock('@universo-react/metahubs-backend') as {
            loadPublishedPublicationRuntimeSource: jest.Mock
            createMetahubsServiceRoutes: jest.Mock
        }
        const applicationsModule = jest.requireMock('@universo-react/applications-backend') as {
        ...
        const authModule = jest.requireMock('@universo-react/auth-backend') as {
