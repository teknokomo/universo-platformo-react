# Сервис аутентификации (@universo-react/auth-backend)

Производственный сервис аутентификации для серверной аутентификации Universo Platformo. Построен на Passport.js + Supabase и монтируется внутри `packages/universo-react-core-backend` по пути `/api/v1/auth`.
...
-   `@universo-react/database` - Общий Knex runtime и `createRlsExecutor()` для pinned request execution
...
-   PNPM 12 workspace environment (12.4.1 закреплён в корне workspace)
...
Используется из `packages/universo-react-core-backend`.
...
-   [Фронтенд аутентификации](../universo-react-auth-frontend/README-RU.md)
-   [RLS Integration Pattern](../../../memory-bank/rls-integration-pattern.md)
-   [Интеграция с core backend](../universo-react-core-backend/README-RU.md)
