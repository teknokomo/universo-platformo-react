# Authentication Service (@universo-react/auth-backend)

Production-grade authentication service powering Universo Platformo's server-side authentication. Built with Passport.js + Supabase and mounted inside `packages/universo-react-core-backend` at `/api/v1/auth`.
...
```typescript
import { createEnsureAuthWithRls } from '@universo-react/auth-backend'
import { getKnex } from '@universo-react/database'
...
```typescript
import type { RequestWithDbContext } from '@universo-react/auth-backend'
...
-   `@universo-react/database` - Shared Knex runtime plus `createRlsExecutor()` for pinned request execution
...
pnpm --filter @universo-react/auth-backend build

# Development mode with watch
pnpm --filter @universo-react/auth-backend dev

# Lint TypeScript
pnpm --filter @universo-react/auth-backend lint
```
...
Consumed by `packages/universo-react-core-backend`.

### Required Variables

-   `SESSION_SECRET` - Secret key for session signing and encryption
-   `SUPABASE_URL` - Supabase project URL for authentication
-   `SUPABASE_PUBLISHABLE_DEFAULT_KEY` or `SUPABASE_ANON_KEY` - Supabase public client key. New projects should prefer `SUPABASE_PUBLISHABLE_DEFAULT_KEY`; legacy projects may keep `SUPABASE_ANON_KEY`.

### JWT Verification Variables

-   `SUPABASE_JWT_SECRET` - Optional legacy JWT secret for HS256 verification
-   `SUPABASE_JWKS_URL` - Optional explicit JWKS endpoint override for asymmetric JWT verification
-   `SUPABASE_JWT_ISSUER` - Optional explicit issuer override (defaults to `${SUPABASE_URL}/auth/v1`)
...
import { createAuthRouter, passport } from '@universo-react/auth-backend'
...
The third argument is optional, but it is the supported place for registration/onboarding role assignment and rollback cleanup helpers.
...
-   **Dual-mode Verification**: HS256 tokens use `SUPABASE_JWT_SECRET`; asymmetric Supabase tokens use JWKS verification via `jose`
...
4. **Environment**: Configure legacy `SUPABASE_JWT_SECRET` or the JWKS-based Supabase env contract
...
describe('RLS Context', () => {
    it('should apply PostgreSQL session variables', async () => {
        ...
        expect(session.query).toHaveBeenCalledWith("SELECT set_config('request.jwt.claims', $1::text, false)", [
...
-   [Authentication Frontend](../universo-react-auth-frontend/README.md)
-   [RLS Integration Pattern](../../../memory-bank/rls-integration-pattern.md)
-   [Core Backend Integration](../universo-react-core-backend/README.md)
