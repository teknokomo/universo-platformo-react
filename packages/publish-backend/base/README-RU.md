# Publish Service (@universo/publish-backend)
...
Проект структурирован как **workspace package** (`@universo/publish-backend`), обеспечивая чёткое разделение и управление зависимостями внутри монорепозитория:

```
packages/publish-backend/base/
├── package.json              # Конфигурация пакета со scoped именем "@universo/publish-backend"
...
-   **Имя пакета**: `@universo/publish-backend` (scoped имя для организации)
-   **Интеграция**: Используется как зависимость `"@universo/publish-backend": "workspace:*"` в основном сервере.
...
-   **Совместное использование типов**: Выступает в качестве источника истины для UPDL типов (`IUPDLSpace`, `IUPDLObject` и т.д.), которые используются фронтенд-пакетом `@universo/publish-frontend`.
...
-   **Модульность и развязанность**: Полностью независим от бизнес-логики `packages/flowise-core-backend/base`. Больше не содержит код генерации UPDL.
...
**Критическое исправление**: Для решения race conditions, когда маршруты регистрировались до готовности подключения к базе данных, этот пакет использует **асинхронную фабрику маршрутов**, аналогично `@universo/profile-backend`.
...
-   **Развязанность**: Логика `utilBuildUPDLflow` была полностью **удалена** из бэкенда и перенесена в класс `UPDLProcessor` в пакете `publish-frontend`.
