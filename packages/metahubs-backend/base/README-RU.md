...
- Системные таблицы метаданных (`_sys_objects`, `_sys_attributes`, `_sys_migrations`) для runtime-интроспекции
...
- `_sys_objects`: Реестр всех объектов метаданных (каталоги, документы, хабы) с презентацией и конфигурацией.
- `_sys_attributes`: Определения полей с типами данных, правилами валидации и UI-конфигурацией.
- `_sys_migrations`: История применённых миграций схемы.

Эти таблицы позволяют выполнять runtime-интроспекцию и генерацию Server-Driven UI.

#### `_sys_objects` — Реестр объектов метаданных
...
#### `_sys_attributes` — Определения полей

| Колонка | Тип | Описание |
|---------|-----|----------|
| `id` | UUID | Первичный ключ. Совпадает с UUID исходного атрибута из Metahub. |
| `object_id` | UUID FK | Ссылка на `_sys_objects.id` (объект-владелец). |
...
| `target_object_id` | UUID FK | Только для типа `REF`: ссылается на `_sys_objects.id` целевого объекта. Позволяет UI отрисовывать селекторы связей. |
...
#### `_sys_migrations` — История миграций схемы
