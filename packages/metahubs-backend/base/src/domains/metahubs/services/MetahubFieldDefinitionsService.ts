...
type MetahubAttributeRecord = {
    ...
    system: FieldDefinitionSystemMetadata
    version: number
    createdAt: unknown
    updatedAt: unknown
}
 ...
 */
export class MetahubFieldDefinitionsService {
    ...
    private getSystemMetadata(row: Record<string, unknown>): FieldDefinitionSystemMetadata {
        const systemKey =
    ...
    private assertSystemMutationAllowed(
        attribute: ReturnType<MetahubFieldDefinitionsService['mapRowToFieldDefinition']> | null,
    ...
    private assertReservedBusinessCodenameAllowed(
        codename: unknown,
        currentAttribute: ReturnType<MetahubFieldDefinitionsService['mapRowToFieldDefinition']> | null = null
    ): void {
    ...
    private getMaxChildAttributesLimit(validationRules: unknown): number | null {
        ...
        const raw = (validationRules as Record<string, unknown>).maxChildFieldDefinitions
    ...
    private createTableChildLimitError(maxChildFieldDefinitions: number): MetahubDomainError {
        return new MetahubDomainError({
            message: `Maximum ${maxChildFieldDefinitions} child attributes per TABLE`,
            statusCode: 409,
            code: 'TABLE_CHILD_LIMIT_REACHED',
            details: { maxChildFieldDefinitions }
        })
    }

    private createTableFieldDefinitionLimitError(maxTableFieldDefinitions: number): MetahubDomainError {
        return new MetahubDomainError({
            message: `Maximum ${maxTableFieldDefinitions} TABLE attributes per catalog`,
            statusCode: 409,
            code: 'TABLE_ATTRIBUTE_LIMIT_REACHED',
            details: { maxTableFieldDefinitions }
        })
    }

    private createTableDisplayFieldDefinitionForbiddenError(): MetahubDomainError {
        return new MetahubDomainError({
    ...
    private async generateUniqueTableAttributeId(schemaName: string, linkedCollectionId: string, db?: SqlQueryable): Promise<string> {
        ...
        const existingTableAttrs = await queryMany<{ id: string }>(
            ...
            [linkedCollectionId, FieldDefinitionDataType.TABLE]
        )
    ...
    async countTableAttributes(metahubId: string, objectId: string, userId?: string, db?: SqlQueryable): Promise<number> {
        ...
        const result = await queryOne<{ count: number }>(
            ...
            [objectId, FieldDefinitionDataType.TABLE]
        )
        return result ? result.count : 0
    }
    ...
    async findAll(metahubId: string, objectId: string, userId?: string, scope: AttributeScope = 'business', db?: SqlQueryable) {
        ...
        return rows.map((row) => this.mapRowToFieldDefinition(row))
    }
    ...
    private async findAllBusinessMerged(
        ...
        const run = async (runner: SqlQueryable) => {
            ...
            const sharedLinkedCollectionId = await sharedContainerService.findContainerObjectId(
                metahubId,
            ...
            const sharedItems = sharedLinkedCollectionId
                ? (await this.listRootRowsByObjectId(schemaName, sharedLinkedCollectionId, 'business', runner)).map((row) =>
                      this.mapRowToFieldDefinition(row)
                  )
                : []
    ...
    async findChildAttributes(metahubId: string, parentAttributeId: string, userId?: string, db?: SqlQueryable) {
        ...
        return rows.map(this.mapRowToFieldDefinition)
    }
    ...
    async findChildAttributesByParentIds(metahubId: string, parentAttributeIds: string[], userId?: string) {
        const result = new Map<string, ReturnType<typeof this.mapRowToFieldDefinition>[]>()
        ...
        for (const row of rows) {
            const attr = this.mapRowToFieldDefinition(row)
    ...
    async listObjectSystemFieldDefinitions(metahubId: string, objectId: string, userId?: string, db?: SqlQueryable) {
        return this.findAll(metahubId, objectId, userId, 'system', db)
    }

    async listCatalogSystemFieldDefinitions(metahubId: string, linkedCollectionId: string, userId?: string, db?: SqlQueryable) {
        return this.listObjectSystemFieldDefinitions(metahubId, linkedCollectionId, userId, db)
    }
    ...
    async getObjectSystemFieldsSnapshot(
        ...
        const attributes = await this.listObjectSystemFieldDefinitions(metahubId, objectId, userId, db)
    ...
    async getCatalogSystemFieldsSnapshot(
        ...
        return this.getObjectSystemFieldsSnapshot(metahubId, linkedCollectionId, userId, db)
    }
    ...
    private resolveCatalogSystemToggleState(
        ...
        const applyState = (key: CatalogSystemFieldKey, nextEnabled: boolean, origin: 'requested' | 'dependency') => {
            ...
            for (const dependent of getCatalogSystemFieldDefinitionSeedRecords()) {
    ...
    async ensureObjectSystemFieldDefinitions(
        metahubId: string,
        objectId: string,
        userId?: string,
        db?: SqlQueryable,
        optionsOrStates?:
            ...
            | {
                  ...
                  policy?: import('@universo/types').PlatformSystemFieldDefinitionsPolicy
              }
    ) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const qt = qSchemaTable(schemaName, '_mhb_attributes')
        ...
        const seedPlan = resolveCatalogSystemFieldDefinitionSeedPlan(states, policy, existingByKey.keys())
        ...
        return this.listObjectSystemFieldDefinitions(metahubId, objectId, userId, runner)
    }

    async ensureCatalogSystemFieldDefinitions(
        metahubId: string,
        linkedCollectionId: string,
        userId?: string,
        db?: SqlQueryable,
        ...
        return this.ensureObjectSystemFieldDefinitions(metahubId, linkedCollectionId, userId, db, optionsOrStates)
    }
     ...
     */
    async findCatalogReferenceBlockers(metahubId: string, targetLinkedCollectionId: string, userId?: string) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
    ...
    async create(metahubId: string, data: FieldDefinitionCreateInput, userId?: string, db?: SqlQueryable) {
        ...
        if (data.parentAttributeId) {
            ...
            if (!parent) {
                throw new MetahubNotFoundError('Parent field definition', data.parentAttributeId)
            }
            ...
            if (data.dataType === FieldDefinitionDataType.TABLE) {
                throw new MetahubValidationError('Nested TABLE attributes are not allowed')
            }
            const maxChildFieldDefinitions = this.getMaxChildAttributesLimit(parent.validationRules)
            if (typeof maxChildFieldDefinitions === 'number') {
                const childCount = await this.countChildAttributes(metahubId, data.parentAttributeId, userId, db)
                if (childCount >= maxChildFieldDefinitions) {
                    throw this.createTableChildLimitError(maxChildFieldDefinitions)
                }
            }
        }

        if (data.dataType === FieldDefinitionDataType.TABLE) {
            const tableCount = await this.countTableAttributes(metahubId, data.linkedCollectionId, userId, db)
            if (tableCount >= 10) {
                throw this.createTableFieldDefinitionLimitError(10)
            }

            explicitAttributeId = await this.generateUniqueTableAttributeId(schemaName, data.linkedCollectionId, db)
        }

        const sortOrder = data.sortOrder ?? (await this.getNextSortOrder(schemaName, data.linkedCollectionId, data.parentAttributeId, db))
        const now = new Date()
    ...
    async update(metahubId: string, id: string, data: FieldDefinitionMutationInput, userId?: string, db?: DbExecutor | SqlQueryable) {
        ...
        if (current?.system?.isSystem) {
            ...
            const platformSystemFieldDefinitionsPolicy: PlatformSystemFieldDefinitionsPolicy =
                data.platformSystemFieldDefinitionsPolicy ?? (await readPlatformSystemFieldDefinitionsPolicy(runner))
            const policyBlockReason = getPlatformSystemFieldDefinitionMutationBlockReason(
                systemKey ?? null,
            ...
            return updatedSystem ? this.mapRowToFieldDefinition(updatedSystem) : null
        }
        ...
        return updated ? this.mapRowToFieldDefinition(updated) : null
    }
    ...
    async delete(metahubId: string, id: string, userId?: string, db?: DbExecutor | SqlQueryable) {
        ...
        const runDelete = async (tx: SqlQueryable) => {
            ...
            if (attribute?.data_type === FieldDefinitionDataType.TABLE) {
            ...
            if (!deleted) {
                throw new MetahubNotFoundError('Field definition', id)
            }
    ...
    async moveAttribute(metahubId: string, objectId: string, fieldDefinitionId: string, direction: 'up' | 'down', userId?: string) {
        const schemaName = await this.schemaService.ensureSchema(metahubId, userId)
        const qt = qSchemaTable(schemaName, '_mhb_attributes')
        ...
        return this.exec.transaction(async (tx: SqlQueryable) => {
            ...
            if (!current) throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
            ...
            if (neighbor) {
                // Swap
                await tx.query(`UPDATE ${qt} SET sort_order = $1 WHERE id = $2`, [neighbor.sort_order, fieldDefinitionId])
    ...
    async reorderAttribute(
        ...
        return this.exec.transaction(async (tx: SqlQueryable) => {
            ...
            if (isCrossList) {
                ...
                await tx.query(`UPDATE ${qt} SET parent_attribute_id = $1 WHERE id = $2`, [targetParent, fieldDefinitionId])
                ...
                if (targetParent !== null) {
                    ...
                    if (siblingCount && siblingCount.count === 1) {
                        await tx.query(`UPDATE ${qt} SET is_display_attribute = true, is_required = true WHERE id = $1`, [
                            fieldDefinitionId
                        ])
                    }
                }
            ...
            if (!refreshed) throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
            ...
            if (oldOrder !== clampedNew) {
                ...
                const baseParams: unknown[] = [objectId, fieldDefinitionId, ...(targetParent ? [targetParent] : [])]
                ...
                await tx.query(`UPDATE ${qt} SET sort_order = $1 WHERE id = $2`, [clampedNew, fieldDefinitionId])
            }
            ...
            if (!updated) throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
            return this.mapRowToFieldDefinition(updated)
        })
    }
    ...
    async reorderAttributeMergedOrder(
        ...
        return this.exec.transaction(async (tx: SqlQueryable) => {
            ...
            const current = items.find((item) => item.id === fieldDefinitionId)
            if (!current) {
                throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
            }
            ...
            const updated = updatedItems.find((item) => item.id === fieldDefinitionId)
            if (!updated) {
                throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
            }
            return updated
    ...
    private async _validateCrossListTransfer(
        ...
        if (targetParentId !== null) {
            ...
            if (!parentAttr || parentAttr.data_type !== FieldDefinitionDataType.TABLE) {
                throw new MetahubDomainError({
            ...
            const maxChildFieldDefinitions = this.getMaxChildAttributesLimit(parentAttr.validation_rules)
            if (typeof maxChildFieldDefinitions === 'number') {
    ...
    async setDisplayAttribute(
        ...
        const attribute = await this.findById(metahubId, fieldDefinitionId, userId, runner)
        if (!attribute) {
            throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
        }
        if (attribute.system?.isSystem) {
        ...
        if (attribute.dataType === FieldDefinitionDataType.TABLE) {
            throw this.createTableDisplayFieldDefinitionForbiddenError()
        }
        ...
        const applyDisplayAttribute = async (tx: SqlQueryable) => {
            if (attribute.parentAttributeId) {
                ...
                await tx.query(
                    ...
                    [now, userId ?? null, linkedCollectionId]
                )
            }
            ...
            await tx.query(
                `UPDATE ${qt} SET is_display_attribute = true, is_required = true, _upl_updated_at = $1, _upl_updated_by = $2
                 WHERE id = $3`,
                [now, userId ?? null, fieldDefinitionId]
            )
        }
    ...
    async clearDisplayAttribute(
        ...
        const attribute = await queryOne<Record<string, unknown>>(runner, `SELECT * FROM ${qt} WHERE id = $1`, [fieldDefinitionId])
        if (!attribute) {
            throw new MetahubNotFoundError('Field definition', fieldDefinitionId)
        }
    ...
    private mapRowToFieldDefinition = (row: Record<string, unknown>): MetahubAttributeRecord => {
        ...
        return {
            id: row.id as string,
            linkedCollectionId: row.object_id as string,
            codename: getCodenameText(row.codename),
            dataType: row.data_type as FieldDefinitionDataType,
