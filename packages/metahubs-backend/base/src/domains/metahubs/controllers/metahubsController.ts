...
const isManagedMetahubSchemaName = (schemaName: string): boolean =>
  schemaName.startsWith('mhb_') && isManagedDynamicSchemaName(schemaName)
...
export function createMetahubsController(getDbExecutor: () => DbExecutor) {
  ...
  const list = async (req: Request, res: Response) => {
    ...
    const { items: metahubs, total } = await listMetahubsStore(exec, {
      ...
      offset,
      sortBy: (
        sortBy === 'codename' ? 'codename' : sortBy === 'name' ? 'name' : sortBy === 'created' ? 'created' : 'updated'
      ) as 'name' | 'codename' | 'created' | 'updated',
  ...
  const copy = async (req: Request, res: Response) => {
    ...
    const defaultSourceBranch =
      sourceBranches.find((branch) => branch.id === sourceMetahub.defaultBranchId) ?? sourceBranches[0]
    ...
    const nameVlc = buildLocalizedContent(
      ...
      sourceMetahub.name?._primary ?? 'en'
    )
    if (!nameVlc) {
      return res.status(400).json({ error: 'Invalid input', details: { name: ['Name is required'] } })
    }
    ...
    try {
      ...
      if (cleanupFailures.length > 0) {
        ...
        throw new MetahubDomainError({ message: `Copy rollback cleanup failed: ${cleanupFailures.join('; ')}`, statusCode: 500, code: 'COPY_CLEANUP_FAILED' })
      }
      throw error
    }
