...
async function resolveTemplateContext(
  ...
  if (currentTemplateVersionId) {
    ...
    if (currentVersion?.manifestJson) {
      try {
        ...
        throw new MetahubValidationError(`Current template manifest is invalid: ${safeErrorMessage(error)}`, { metahubId: metahub.id })
      }
    }
  }
  ...
  if (!requested || requested.templateId !== template.id) {
    throw new MetahubValidationError('Target template version is not linked to this metahub template', { targetTemplateVersionId, templateId: template.id })
  }
...
export function createMetahubMigrationsController(getDbExecutor: () => DbExecutor) {
  ...
  const apply = async (req: Request, res: Response) => {
    ...
    try {
      ...
      migrationPlan = await buildMigrationPlan(
        exec,
        metahub,
        branch,
        parsed.data.targetTemplateVersionId,
