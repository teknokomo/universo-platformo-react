...
export function createElementsController(createHandler: ReturnType<typeof createMetahubHandlerFactory>) {
  const mkServices = (exec: import('../../../utils').DbExecutor, schemaService: import('../../metahubs/services/MetahubSchemaService').MetahubSchemaService) => {
  ...
  const create = createHandler(async ({ req, res, metahubId, userId, exec, schemaService }) => {
    ...
    const element = await elementsService.create(
      metahubId,
      catalogId,
      { data, sortOrder, createdBy: userId },
      userId
    )
  ...
  const move = createHandler(async ({ req, res, metahubId, userId, exec, schemaService }) => {
    const { catalogId, elementId } = req.params
    const { elementsService } = mkServices(exec, schemaService)
  ...
  const copy = createHandler(async ({ req, res, metahubId, userId, exec, schemaService }) => {
    ...
    const copyOptions = hasRequiredChildTables
      ? { ...requestedOptions, copyChildTables: true }
      : requestedOptions
    ...
    const copied = await elementsService.create(
      metahubId,
      catalogId,
      { data: copiedData, createdBy: userId },
      userId
    )
