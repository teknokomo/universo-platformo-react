...
export function createBranchesController(
  ...
  const getSettingsService = (ctx: MetahubHandlerContext) =>
    new MetahubSettingsService(ctx.exec, ctx.schemaService)
  ...
  const update = createHandler(async (ctx) => {
    const { req, res, metahubId, userId, exec } = ctx
    const { branchId } = req.params
    const branchesService = getService(exec)
  ...
  const setDefault = createHandler(async ({ req, res, metahubId, userId, exec }) => {
    const { branchId } = req.params
    const branchesService = getService(exec)
  ...
  const blockingUsers = createHandler(async ({ req, res, metahubId, userId, exec }) => {
    const { branchId } = req.params
    const branchesService = getService(exec)
  ...
  const remove = createHandler(async ({ req, res, metahubId, userId, exec }) => {
    const { branchId } = req.params
    const branchesService = getService(exec)
