...
const normalizeMultilineUiConfig = (
    ...
    if (nextUiConfig.widget === 'textarea' && dataType !== AttributeDataType.STRING) {
        return {
...
export function createAttributesController(_createHandler: CreateHandler, getDbExecutor: () => DbExecutor) {
    ...
    const copy = async (req: Request, res: Response) => {
        ...
        if (source.targetEntityKind === SET_KIND) {
            ...
            const constantBelongsToSet = await constantsService.belongsToSet(metahubId, source.targetEntityId, copyTargetConstantId, userId)
            if (!constantBelongsToSet) {
                return res.status(400).json({
    ...
    const deleteAttribute = async (req: Request, res: Response) => {
        ...
        try {
            await exec.transaction(async (trx: SqlQueryable) => {
                await attributesService.ensureSequentialSortOrder(metahubId, catalogId, userId, attribute.parentAttributeId ?? null, trx)
