...
import {
  respondSchemaSyncFailure,
  isSchemaSyncFailure,
  syncMetahubSchemaOrThrow,
  isUniqueViolation
} from '../../shared/errorGuards'
...
export function createAttributesController(
    ...
    if (search) {
      ...
      const matchesName = (name: unknown): boolean => {
        ...
        if (typeof name === 'object') {
          if ('locales' in name && (name as Record<string, unknown>).locales && typeof (name as Record<string, unknown>).locales === 'object') {
      ...
      const matchesItem = (item: { codename: string; name: unknown }) => item.codename.toLowerCase().includes(searchLower) || matchesName(item.name)
  ...
  const listChildrenBatch = async (req: Request, res: Response) => {
    ...
    const parentIds = raw.split(',').map((id) => id.trim()).filter(Boolean)
