import type { CatalogSystemFieldKey, CatalogSystemFieldState, PlatformSystemAttributesPolicy } from '@universo/types'
...
export interface CatalogSystemAttributeSeedPlan {
...
const LEGACY_PLATFORM_SYSTEM_ATTRIBUTES_POLICY: PlatformSystemAttributesPolicy = {
...
export function resolvePlatformSystemAttributesPolicyRows(
...
export const getPlatformSystemAttributeMutationBlockReason = (
    key: CatalogSystemFieldKey | null | undefined,
    policy: PlatformSystemAttributesPolicy
): string | null => {
    if (!isPlatformSystemFieldKey(key)) {
...
export async function readPlatformSystemAttributesPolicy(exec: Queryable): Promise<PlatformSystemAttributesPolicy> {
...
export function resolveCatalogSystemAttributeSeedPlan(
    states?: CatalogSystemFieldState[],
    policy: PlatformSystemAttributesPolicy = LEGACY_PLATFORM_SYSTEM_ATTRIBUTES_POLICY,
    existingKeys: Iterable<CatalogSystemFieldKey | string> = []
): CatalogSystemAttributeSeedPlan {
