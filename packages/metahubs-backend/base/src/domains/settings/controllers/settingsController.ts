...
function mergeSettingsWithDefaults(
  dbRows: MetahubSettingRow[],
  registry: readonly RegistryEntry[] = METAHUB_SETTINGS_REGISTRY
) {
...
export function createSettingsController(createHandler: ReturnType<typeof createMetahubHandlerFactory>) {
  ...
  const bulkUpdate = createHandler(async ({ req, res, metahubId, userId, exec, schemaService }) => {
    const settingsService = new MetahubSettingsService(exec, schemaService)
    const languageOptions = await getContentLocaleCodes(exec)
  ...
  const resetByKey = createHandler(async ({ req, res, metahubId, userId, exec, schemaService }) => {
    const settingsService = new MetahubSettingsService(exec, schemaService)
