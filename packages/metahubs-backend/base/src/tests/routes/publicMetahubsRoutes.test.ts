...
const mockComponentsFindAll = jest.fn()
...
jest.mock('../../domains/metahubs/services/MetahubComponentsService', () => ({
    MetahubComponentsService: jest.fn().mockImplementation(() => ({
        findAll: (...args: unknown[]) => mockComponentsFindAll(...args)
    }))
}))
...
const COMPONENT = { id: 'attr-1', codename: 'title', objectCollectionId: 'cat-1' }
const RECORD = { id: 'el-1', objectCollectionId: 'cat-1', data: { title: 'Book' } }
...
describe('publicMetahubsRoutes', () => {
    ...
    // ── GET /:slug/tree-entity/:treeEntityCodename/objects ──
    it('lists objects in a hub', async () => {
        mockObjectsFindAll.mockResolvedValue([LINKED_COLLECTION])

        const res = await request(buildApp()).get('/public/my-project/tree-entity/main-hub/objects')
        expect(res.status).toBe(200)
        expect(res.body.items).toHaveLength(1)
    })

    it('filters objects not belonging to the hub', async () => {
    ...
    it('filters objects that are not object compatible kinds', async () => {
    ...
    // ── GET /:slug/tree-entity/:treeEntityCodename/object/:linkedCollectionCodename ──
    it('returns a object by codename', async () => {
        const res = await request(buildApp()).get('/public/my-project/tree-entity/main-hub/object/products')
        expect(res.status).toBe(200)
        expect(res.body.codename).toBe('products')
    })

    it('returns 404 for object not in hub', async () => {
    ...
    it('returns 404 for object codename that is not object compatible', async () => {
    ...
    // ── GET .../components ─────────────────────────────
    it('lists field definitions for a object', async () => {
        mockComponentsFindAll.mockResolvedValue([COMPONENT])

        const res = await request(buildApp()).get('/public/my-project/tree-entity/main-hub/object/products/components')
        expect(res.status).toBe(200)
        expect(res.body.items).toHaveLength(1)
    })
    ...
    it('lists records with pagination', async () => {
        ...
        const res = await request(buildApp()).get('/public/my-project/tree-entity/main-hub/object/products/records?limit=10&offset=0')
        expect(res.status).toBe(200)
        expect(res.body.items).toHaveLength(1)
    ...
    it('returns a single record', async () => {
        ...
        const res = await request(buildApp()).get('/public/my-project/tree-entity/main-hub/object/products/record/el-1')
        expect(res.status).toBe(200)
    ...
    it('returns 404 for missing record', async () => {
        ...
        const res = await request(buildApp()).get('/public/my-project/tree-entity/main-hub/object/products/record/no-such')
        expect(res.status).toBe(404)
    })
})
