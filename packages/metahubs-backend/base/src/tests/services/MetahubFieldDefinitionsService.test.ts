...
import { MetahubFieldDefinitionsService } from '../../domains/metahubs/services/MetahubFieldDefinitionsService'
import { codenamePrimaryTextSql } from '../../domains/shared/codename'

describe('MetahubFieldDefinitionsService active-row filtering', () => {
    const schemaService = {
    ...
    const service = new MetahubFieldDefinitionsService(mockExec as any, schemaService)
    ...
    const configurablePlatformPolicyRows = [
        ...
        { key: 'platformSystemFieldDefinitionsRequired', value: { _value: true } },
    ...
    it('rejects platform system toggles when global platform defaults are enforced', async () => {
        ...
        mockExecQuery.mockImplementation(async (sql: string) => {
            if (sql.includes('FROM admin.cfg_settings')) {
                return [
                    { key: 'platformSystemFieldDefinitionsConfigurable', value: { _value: false } },
                    { key: 'platformSystemFieldDefinitionsRequired', value: { _value: true } },
                    { key: 'platformSystemFieldDefinitionsIgnoreMetahubSettings', value: { _value: true } }
                ]
            }
    ...
    it('seeds disabled system-field states when explicit catalog states are provided', async () => {
        ...
        await service.ensureCatalogSystemFieldDefinitions('metahub-1', 'catalog-1', 'user-1', undefined, [
    ...
    it('reads system rows back through the provided transaction runner', async () => {
        ...
        const result = await service.ensureCatalogSystemFieldDefinitions('metahub-1', 'catalog-1', 'user-1', { query: trxQuery } as any)
    ...
    it('supports the object-scoped system attribute adapter with the provided transaction runner', async () => {
        ...
        const result = await service.ensureObjectSystemFieldDefinitions('metahub-1', 'object-1', 'user-1', { query: trxQuery } as any)
    ...
    it('keeps legacy catalog wrappers aligned with object-scoped system attribute reads', async () => {
        ...
        const objectScoped = await service.listObjectSystemFieldDefinitions('metahub-1', 'catalog-1', 'user-1')
        const legacyCatalogScoped = await service.listCatalogSystemFieldDefinitions('metahub-1', 'catalog-1', 'user-1')
