...
import { useStorageDetails } from '../api/useStorageDetails'

/**
 * Storage Board Page
 *
 * Displays analytics dashboard for a storage with:
 * - Real-time statistics (containers, slots, members)
 * - Documentation slots
 * - Activity and slot charts (demo data)
 *
 * Layout:
...
const StorageBoard = () => {
    ...
    // Fetch storage details with TanStack Query
    const {
        data: storage,
        isLoading,
        error,
        isError
    } = useStorageDetails(storageId || '', {
    ...
    // Error state
    if (isError || !storage) {
        const errorMessage = error instanceof Error ? error.message : t('board.error', 'Failed to load storage data')

        return (
            <Stack spacing={3} sx={{ maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto', width: '100%', p: 2 }}>
                <EmptyListState
                    image={APIEmptySVG}
                    imageAlt='Error loading storage'
                    title={t('board.error', 'Failed to load storage data')}
                />
    ...
    // Use current slot count as trend data (shows actual value in chart)
    const slotsCount = storage.slotsCount ?? 0
    const slotsData = Array(30).fill(slotsCount)
    ...
    return (
        <Stack spacing={2} sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' }, mx: 'auto' }}>
            ...
            <Box sx={{ px: { xs: 1.5, md: 2 } }}>
                <ViewHeader
                    title={storage.name}
                    description={storage.description || t('board.defaultDescription', 'Storage analytics and statistics')}
                    search={false}
                />
            </Box>
            ...
            <Box sx={{ width: '100%', maxWidth: { sm: '100%', md: '1700px' } }}>
                <Grid container spacing={2} columns={12}>
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <StatCard
                            title={t('board.stats.containers.title')}
                            value={storage.containersCount ?? 0}
                            interval={t('board.stats.containers.interval')}
                            description={t('board.stats.containers.description')}
                            data={containersData}
                        />
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6} lg={3}>
                        <StatCard
                            title={t('board.stats.slots.title')}
                            value={storage.slotsCount ?? 0}
                            interval={t('board.stats.slots.interval')}
                            description={t('board.stats.slots.description')}
                            data={slotsData}
                        />
                    </Grid>
                    ...
                    {/* Slots Chart (Demo Data) */}
                    <Grid item xs={12} md={6}>
                        <PageViewsBarChart title={t('board.charts.slots.title')} description={t('board.charts.slots.description')} />
                    </Grid>
                </Grid>

                {/* Back Button */}
                <Box display='flex' justifyContent='center' sx={{ mt: 2 }}>
                    <Button variant='text' startIcon={<ArrowBackRoundedIcon />} onClick={() => navigate('/storages')}>
                        {t('actions.backToList', 'Back to Storages')}
                    </Button>
                </Box>
            </Box>
        </Stack>
