...
describe('PlayCanvas Editor artifact metadata', () => {
    ...
    it('keeps full upstream UI bootstrap wired to the upstream editor without the hosted entity fallback', () => {
        ...
        try {
            ...
            expect(source).toContain('const createHostedAssetObserver = (input = {}) => {')
            expect(source).toContain("input.meta && typeof input.meta === 'object' && !Array.isArray(input.meta)")
            expect(source).toContain("editorInstance.method('assets:list', () => hostedAssetObservers);")
            expect(source).toContain("editorInstance.method('assets:get', (id) =>")
            expect(source).toContain("editorInstance.emit('assets:add', asset);")
