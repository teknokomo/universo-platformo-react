...
describe('PlayCanvas Editor artifact metadata', () => {
    ...
    it('keeps the attribute parser TypeScript lib URL compatible with the workspace compiler', () => {
        const viteConfigPath = path.join(packageRoot, 'vendor', 'playcanvas-editor', 'vite.config.mjs')
        const viteConfig = fs.readFileSync(viteConfigPath, 'utf8')

        expect(viteConfig).toContain('ensureAttributeParserTypeScriptLibCompatibility')
        expect(viteConfig).toContain('lib.es2022.sharedmemory.d.ts')
    })
    ...
    it('keeps full upstream UI bootstrap wired to the upstream editor without the hosted entity fallback', () => {
        ...
        try {
            ...
            expect(source).toContain('const normalizeCompatibilityMaterialData = (value) => {')
            expect(source).toContain(
            ...
            expect(source).toContain('__universoMmoommVisualMaterialSignature')
            expect(source).toContain('__universoMmoommVisualMaterialOwned')
            expect(source).toContain('const installMmoommVisualMaterialRenderHook = (editorInstance, engineEntity, entity) => {')
            expect(source).toContain(
                "for (const methodName of ['_onMaterialLoad', '_onMaterialRemove', '_onMaterialUnload', '_onSetMeshes'])"
            )
            expect(source).toContain("for (const callbackName of ['_onAssetLoad', '_onAssetAdd', '_onAssetRemove', '_onAssetUnload'])")
            expect(source).toContain('reference[callbackName] = wrappedCallback;')
            expect(source).toContain('applyMmoommVisualMaterialToEngineEntity(editorInstance, latestEntity);')
            expect(source).toContain('const resourceId = entity?.resource_id || entity?.id;')
            expect(source).toContain('if (!app?.root) return null;')
            expect(source).toContain("if (typeof app.root.findByGuid === 'function') {")
            expect(source).toContain('app.root.findByName(String(name))')
            expect(source).toContain('marker.lastSerializedMmoommVisualMaterialEntityCount = entities.filter((entity) =>')
            expect(source).toContain('applyMmoommVisualMaterialToEngineEntity(editorInstance, entity)')
            expect(source).toContain('if (!scene) {')
            expect(source).toContain('const readCompatibilityTokenClaims = (token) => {')
            expect(source).toContain("encodedPayload.replace(/-/g, '+').replace(/_/g, '/')")
            expect(source).toContain('const isUsableFullBootAccessToken = (token) => {')
            expect(source).toContain('const refreshFullBootAccessToken = async (force = false) => {')
            expect(source).toContain("claims?.mode === 'universo-full-upstream-ui'")
            expect(source).toContain('claims.expiresAt - 30000 > Date.now()')
            expect(source).toContain('const existingToken =')
            expect(source).toContain("typeof marker.compatibilityConfig?.accessToken === 'string'")
            expect(source).toContain('if (!force && isUsableFullBootAccessToken(existingToken)) {')
            ...
            expect(source).toContain('const mergeScenePayloadSnapshots = (...payloads) => {')
            expect(source).toContain('const mergeSceneMetadataSnapshots = (...metadataSnapshots) => {')
            expect(source).toContain('return mergeScenePayloadSnapshots(...payloads);')
            expect(source).toContain('metadata: mergeSceneMetadataSnapshots(merged.metadata, next.metadata)')
            expect(source).toContain(
                "mergeSceneMetadataSnapshots(fallbackPayload?.metadata, sceneMetadata, { savedBy: 'universo-playcanvas-editor-bridge' })"
            )
            expect(source).toContain('const cleanPayloadSnapshot = cloneScenePayloadSnapshot(payload) || payload;')
            expect(source).toContain('marker.lastCleanLoadedScenePayload = cleanPayloadSnapshot;')
            ...
            expect(source).toContain('const cleanLoadedScenePayload =')
            expect(source).toContain('const loadedScenePayload = readLoadedScenePayload();')
            expect(source).toContain('mergeScenePayloadSnapshots(cleanLoadedScenePayload, loadedScenePayload)')
            expect(source).toContain('const loadedScenePayload = readLoadedScenePayload(marker.lastLoadedScene);')
            expect(source).toContain('rememberScenePayloadEntities(loadedScenePayload);')
            expect(source).toContain('installHostedAssetAdapter(window.editor, loadedScenePayload);')
            expect(source).toContain('if (marker.fullBootMode === true) {')
            expect(source).toContain('void loadFullBootAssets();')
            expect(source).toContain('const applyBootstrapCompatibilityDescriptor = (descriptor) => {')
            expect(source).toContain('applyBootstrapCompatibilityDescriptor(descriptor);')
            expect(source).toContain('const createFullBootConfigResponseBody = (mode) => {')
            expect(source).toContain("mode === 'universo-compatibility-rest-minimal'")
            expect(source).toContain('marker.restCompatibilityConfig || window.config?.universoRestCompatibilityConfig || null')
            expect(source).toContain('const isUsableRestCompatibilityConfig = (compatibilityConfig) => {')
            expect(source).toContain('claims.origin === window.location.origin')
            expect(source).toContain('if (isUsableRestCompatibilityConfig(compatibilityConfig)) return compatibilityConfig;')
            expect(source).toContain("mode === 'universo-full-upstream-ui'")
            expect(source).toContain("window.config?.mode === 'universo-full-upstream-ui' ? window.config : null")
            expect(source.indexOf('if (/\\\\/config$/.test(url.pathname)) {')).toBeLessThan(
                source.indexOf('if (!window.config?.project?.id) return null;')
            )
            expect(source).toContain('assetsEndpoint = new URL(restConfig.endpoints.assets, window.location.href);')
            ...
            expect(source).toContain('const createFullBootFetchRequest = (input, init) => {')
            expect(source).toContain('if (input instanceof Request) return init === undefined ? input : new Request(input, init);')
            expect(source).toContain('const fullBootRequest = createFullBootFetchRequest(input, init);')
            expect(source).toContain("const requestMethod = String(fullBootRequest?.method || init?.method || 'GET').toUpperCase();")
            expect(source).toContain('const createRewrittenFullBootFetchRequest = async (request, url, init) => {')
            expect(source).toContain('requestInit.body = await request.clone().arrayBuffer();')
            expect(source).toContain('const rewrittenInput = await createRewrittenFullBootFetchRequest(')
            expect(source).toContain('window.fetch = async (input, init) => {')
            expect(source).toContain("const normalizedMethod = String(method || 'GET').toUpperCase();")
            expect(source).toContain("if (projectAssetsPath && !(projectAssetsCollectionPath && normalizedMethod === 'GET')) {")
            expect(source).toContain("if (requestMethod === 'GET' && url.pathname === '/api/projects/' + numericProjectId + '/assets') {")
            expect(source).toContain('if (isCompatibilityRequest && !isCompatibilityWrite) {')
            expect(source).toContain("(typeof window.config?.accessToken === 'string' && window.config.accessToken) ||")
            ...
            expect(source).toContain('Date.now() < (marker.hydrationDirtySuppressionUntil || 0)')
            expect(source).toContain('marker.suppressedHydrationEntityRemovalIds')
            expect(source).toContain('const consumeSuppressedHydrationEntityRemoval = (entity) => {')
            ...
            expect(source).toContain('const createHostedAssetObserver = (input = {}, editorInstance = null) => {')
            expect(source).toContain("input.meta && typeof input.meta === 'object' && !Array.isArray(input.meta)")
            expect(source).toContain('const hostedAssetApiAssets = [];')
            expect(source).toContain('const canUseNativeAssets =')
            expect(source).toContain('Never replace them with a plain object')
            expect(source).toContain('const getAssetIdentityKeys = (asset) => {')
            expect(source).toContain('const getSceneLocalAssets = () =>')
            expect(source).toContain('const getFullBootStorageAssets = () =>')
            expect(source).toContain('const rememberSceneLocalAssets = (assets) => {')
            expect(source).toContain('const mergeFullBootAssetSources = () => {')
            expect(source).toContain('const completeFullBootAssetProgress = () => {')
            expect(source).toContain('const scheduleFullBootAssetProgressCompletion = () => {')
            expect(source).toContain("const assetsPanel = window.editor.call('layout.assets');")
            expect(source).toContain('fullBootAssetProgressRetryTimer = window.setTimeout(retry, 250);')
            expect(source).toContain('attempts >= 600')
            expect(source).toContain('const installMergedFullBootAssets = () => {')
            expect(source).toContain("marker.fullBootAssetLoadState !== 'ready'")
            expect(source).toContain("window.editor.call('assets:progress', 1);")
            expect(source).toContain('marker.fullBootAssetProgressCompleted = true;')
            expect(source).toContain("methodName === 'assets:progress'")
            expect(source).toContain('const forceFullBootAssetProgress =')
            expect(source).toContain('const callArgs = forceFullBootAssetProgress ? [1, ...args.slice(1)] : args;')
            expect(source).toContain('marker.fullBootAssetProgressOverrideCount =')
            expect(source).toContain("methodName === 'assets:clear'")
            expect(source).toContain('marker.fullBootAssetClearSuppressedCount =')
            expect(source).toContain('const resolvePendingFullBootAssetDocuments = (editorInstance, assetsOverride = null) => {')
            expect(source).toContain('document.__universoFullBootAssetResolved === true')
            expect(source).toContain("if (typeof document._setType === 'function' && !document.type) document._setType('json0');")
            expect(source).not.toContain("queueMicrotask(() => document.emit('load'));")
            expect(source).toContain(
                'marker.fullBootAssetDocumentsResolved = (Number(marker.fullBootAssetDocumentsResolved) || 0) + resolved;'
            )
            expect(source).toContain('const loadFullBootAssetForEditor = (editorInstance, uniqueId, callback) => {')
            expect(source).toContain("if (methodName === 'loadAsset' && marker.fullBootMode === true && typeof args[1] === 'function') {")
            expect(source).toContain('marker.fullBootAssetLoadResolved = (Number(marker.fullBootAssetLoadResolved) || 0) + 1;')
            expect(source).toContain('const getFullBootAssetIdentityKeys = () => {')
            expect(source).toContain('const sceneIdentityKeys = new Set(getSceneLocalAssets().flatMap(getAssetIdentityKeys));')
            expect(source).toContain('const selectSceneLocalAssets = (fallbackAssets, editorAssets) => {')
            expect(source).toContain('const fullBootAssetKeys = getFullBootAssetIdentityKeys();')
            expect(source).toContain('...getSceneLocalAssets().flatMap(getAssetIdentityKeys),')
            expect(source).toContain('...(Array.isArray(fallbackAssets) ? fallbackAssets.flatMap(getAssetIdentityKeys) : [])')
            expect(source).toContain('const assets = selectSceneLocalAssets(fallbackPayload?.assets, editorAssets);')
            expect(source).toContain('The loaded scene payload is authoritative for embedded assets')
            expect(source).toContain("editorInstance.method('assets:loaded', () => true);")
            expect(source).toContain('installHostedAssetAdapter(editorInstance, marker.lastCleanLoadedScenePayload);')
            expect(source).toContain('installHostedAssetAdapter(editorInstance, fallbackPayload);')
            expect(source).toContain('installHostedAssetAdapter(window.editor, assets);')
            expect(source).toContain('marker.fullBootAssetOrigin =\n      sceneAssets.length > 0')
            expect(source).toContain('marker.fullBootStorageAssetsById = Object.fromEntries')
            expect(source).toContain('return marker.fullBootAssetsPromise.then(() => installMergedFullBootAssets());')
            expect(source).toContain('const maxAttempts = 2;')
            expect(source).toContain("marker.fullBootAssetLoadState = 'loading';")
            expect(source).toContain('marker.fullBootAssetLoadAttempts = attempt;')
            expect(source).toContain('if (response.status === 401) {')
            expect(source).toContain('const refreshedToken = await refreshFullBootAccessToken(true);')
            expect(source).toContain('marker.fullBootAssetLoadRetries = (Number(marker.fullBootAssetLoadRetries) || 0) + 1;')
            expect(source).toContain("throw createLoadError('Full-boot asset request returned invalid JSON');")
            expect(source).toContain("throw createLoadError('Full-boot asset request returned an invalid items payload');")
            expect(source).toContain("throw createLoadError('Full-boot asset request returned duplicate asset identifiers', false);")
            expect(source).toContain('error?.retryable === false')
            expect(source).toContain('const trackedPromise = loadPromise')
            expect(source).toContain("marker.fullBootAssetLoadState = 'failed';")
            expect(source).toContain('marker.fullBootAssetsById = null;')
            expect(source).toContain('if (marker.fullBootAssetsPromise === trackedPromise) marker.fullBootAssetsPromise = null;')
            expect(source).toContain('const parseCanonicalAssetDocumentId = (value) => {')
            expect(source).toContain('const explicitEditorDocumentId = parseCanonicalAssetDocumentId(asset?.editorDocumentId);')
            expect(source).toContain(
                "const stableAssetId = typeof asset?.stableAssetId === 'string' && asset.stableAssetId.trim() ? asset.stableAssetId.trim() : null;"
            )
            expect(source).toContain('const embeddedAssetId = stableAssetId ? parseCanonicalAssetDocumentId(asset?.id) : null;')
            expect(source).toContain('rememberScenePayloadEntities(payload);')
            expect(source.indexOf('rememberScenePayloadEntities(payload);')).toBeLessThan(source.indexOf('markClean();'))
            ...
            expect(source).toContain('return { status: 200, bodyPromise: loadFullBootAssets() };')
            expect(source).toContain('body = await Promise.resolve(synthetic.bodyPromise || synthetic.body);')
            expect(source).toContain('Promise.resolve(cloudApiResponse.bodyPromise || cloudApiResponse.body)')
            ...
            expect(source).toContain('native.setRequestHeader(restConfig.auth.headerName, accessToken);')
    ...
    it('rejects direct and split-argument install or network source commands in scripts', () => {
        ...
        try {
            const samples = [
                ...
                ['corepack-prepare.mjs', 'corepack prepare pnpm@12.4.1 --activate'],
                ...
                ['split-corepack-prepare.mjs', "spawnSync('corepack', ['prepare', 'pnpm@12.4.1', '--activate'])"],
