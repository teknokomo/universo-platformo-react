...
Route guard component that blocks navigation when metahub migrations are pending. Uses `MigrationGuardShell` from `@universo-react/migration-guard-shared` for common guard logic.
...
The guard delegates common logic to `MigrationGuardShell` from `@universo-react/migration-guard-shared`:
...
```typescript
// StructuredBlocker type (from @universo-react/types):
