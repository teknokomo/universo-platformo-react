...
This package (`@universo-react/metahubs-frontend`) provides the React UI for metahub management, including the **Metahub Migration Guard** system and the migrations management page.
...
Route guard that checks migration status before allowing access to metahub pages. Uses `MigrationGuardShell` from `@universo-react/migration-guard-shared` for common guard logic.
...
## Key Types (from `@universo-react/types`)
...
-   `@universo-react/migration-guard-shared` — `MigrationGuardShell`, `MIGRATION_STATUS_QUERY_OPTIONS`
-   `@universo-react/types` — type definitions
-   `@universo-react/utils` — `extractAxiosError`
