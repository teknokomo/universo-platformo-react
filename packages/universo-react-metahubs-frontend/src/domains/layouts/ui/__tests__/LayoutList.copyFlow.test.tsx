...
vi.mock('@universo-react/template-mui', async (importOriginal) => {
    const actual = await importOriginal<typeof import('@universo-react/template-mui')>()
    return {
...
describe('LayoutList copy flow entry', () => {
    ...
    it('enables adaptive embedded header controls for scoped layout lists', async () => {
    ...
    it('creates the first scoped layout without seeding legacy runtime behavior config from the entity form', async () => {
        const queryClient = new QueryClient({
            defaultOptions: {
                queries: { retry: false },
                mutations: { retry: false }
    ...
    it('creates manual global layouts without pre-seeding dashboard config', async () => {
        ...
        expect(payload.data.scopeEntityId).toBeUndefined()
        expect(payload.data.config).toBeUndefined()
    })
})
