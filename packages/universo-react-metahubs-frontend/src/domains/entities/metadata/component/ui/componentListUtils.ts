...
import type { ComponentDefinitionDataType, ComponentDefinitionValidationRules } from '../../../../../types'
...
export type ObjectTab = 'components' | 'system' | 'records' | 'settings'
...
export type ComponentFormValues = {
    ...
    validationRules?: ComponentDefinitionValidationRules
...
export const extractResponseMaxChildComponents = (error: unknown): number | undefined => {
    const data = extractResponseData(error)
    const max = data?.maxChildComponents
    return typeof max === 'number' && Number.isFinite(max) ? max : undefined
}

export const sanitizeComponentUiConfig = (
    dataType: ComponentDefinitionDataType,
