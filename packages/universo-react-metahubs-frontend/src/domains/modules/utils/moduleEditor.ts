...
import { MODULE_LIFECYCLE_EVENTS, resolveAllowedModuleCapabilities, type ModuleCapability, type ModuleRole } from '@universo-react/types'

export interface ModuleRoleGuidance {
    summary: string
    entryPoints: string[]
    allowedCapabilities: ModuleCapability[]
}

const BASE_MODULE_EDITOR_COMPLETIONS: Completion[] = [
    {
        label: 'ExtensionModule',
        type: 'class',
        detail: 'SDK base class',
        info: 'Base class for all embedded metahub modules.'
    },
    {
        label: 'SharedLibraryModule',
        type: 'class',
        detail: 'SDK base class',
    ...
    },
    {
        ...
        type: 'property',
        detail: 'rpc.client',
        info: 'Calls a server-side module method from client code.',
...
const LIFECYCLE_COMPLETIONS: Completion[] = MODULE_LIFECYCLE_EVENTS.map((eventName) => ({
...
export const MODULE_EDITOR_COMPLETIONS: Completion[] = [...BASE_MODULE_EDITOR_COMPLETIONS, ...LIFECYCLE_COMPLETIONS]

const moduleEditorCompletionSource = completeFromList(MODULE_EDITOR_COMPLETIONS)

export const buildModuleEditorExtensions = (ariaLabel = 'Module source code') => [
    javascript({ typescript: true }),
    autocompletion({ override: [moduleEditorCompletionSource] }),
    EditorView.contentAttributes.of({ 'aria-label': ariaLabel })
]

export const getModuleEditorTheme = (mode: PaletteMode) => (mode === 'dark' ? vscodeDark : vscodeLight)

export const getModuleRoleGuidance = (moduleRole: ModuleRole): ModuleRoleGuidance => {
    switch (moduleRole) {
        case 'lifecycle':
            return {
                summary:
                    'Lifecycle modules are optimized for before/after entity events and usually validate, enrich, or veto record mutations.',
                entryPoints: MODULE_LIFECYCLE_EVENTS.map((eventName) => `@OnEvent('${eventName}')`),
                allowedCapabilities: resolveAllowedModuleCapabilities(moduleRole)
            }
        case 'widget':
            return {
                summary:
