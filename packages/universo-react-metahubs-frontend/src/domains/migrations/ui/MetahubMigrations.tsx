...
import type { PaginationActions, PaginationState } from '@universo-react/template-mui'
...
const MetahubMigrations = () => {
    ...
    const rows = useMemo<MigrationDisplayRow[]>(() => {
        return (migrationsQuery.data?.items ?? []).map((item) => {
            ...
            // Schema column: for baseline show "0 → N" (from schema version zero), otherwise "N → M"
