...
-   **Пакет**: `@universo-react/metahubs-frontend`
-   **Версия**: `0.1.0`
...
-   **Виджеты на основе Сущностей**: Добавление первого экрана автоматически создаёт отдельную запись Объекта, а дублирование копирует связанную запись. В дополнительных настройках можно выбрать совместимый источник, создать отдельную модель и увидеть предупреждение перед совместным использованием записи.
...
### 🧭 Редактирование виджета трактовочной сети

-   Редактор макетов метахаба настраивает канонический виджет `interpretationNetworkWorkspace` через пользовательские элементы управления, а не через сырой JSON.
-   Семантика данных Матрицы остаётся в `matrixMode`. Доступность представлений хранится в `allowedMatrixViews`, а начальное представление задаётся через `defaultMatrixView`.
-   `templatePanel.showInStructureList` и `templatePanel.showInMatrix` по умолчанию включены и задают каноническое размещение вкладки шаблонов. Если выключить оба флага, шаблоны остаются данными рабочего пространства, но опубликованное приложение не показывает интерфейс шаблонов.
-   Общий контракт из `@universo-react/types` считает `table`, `horizontalRows` и `verticalTree` равноправными представлениями Матрицы, требует хотя бы одно и ограничивает `verticalTree` иерархическими ячейками.
-   Эти настройки принадлежат метахабу, потому что публикуются с каждой конфигурацией. Переопределения конкретного развёртывания принадлежат Панели приложения, а пользовательские Структуры, ячейки, Материалы, Связи и шаблоны таблиц принадлежат опубликованному рабочему пространству.
-   Интерфейс и валидация локализованы на английском и русском языках и не показывают пользователю ID виджетов, ключи конфигурации, UUID или устаревшие продуктовые названия.
...
pnpm --filter @universo-react/metahubs-frontend dev
```
...
import {
  ...
  RecordList,
  metahubsDashboard
} from '@universo-react/metahubs-frontend'

// Импорт i18n ресурсов
import { metahubsTranslations } from '@universo-react/metahubs-frontend'
...
│   ├── view-preferences/  # Ключи хранения и помощники для настроек просмотра
...
```tsx
import { MetahubList } from '@universo-react/metahubs-frontend'
...
```tsx
import { MetahubBoard } from '@universo-react/metahubs-frontend'
...
```tsx
import { StandardEntityCollectionPage } from '@universo-react/metahubs-frontend'
...
```tsx
import { StandardEntityChildCollectionPage } from '@universo-react/metahubs-frontend'
...
```tsx
import { FieldDefinitionList, RecordList } from '@universo-react/metahubs-frontend'
...
```tsx
import { TemplateSelector } from '@universo-react/metahubs-frontend'
...
```tsx
import { ColumnsContainerEditorDialog } from '@universo-react/metahubs-frontend'
...
### Настройки виджета рабочей области трактовочной сети

Экран настройки макета включает отдельный редактор виджета рабочей области трактовочной сети. Автор метахаба может управлять `allowedMatrixViews` и `defaultMatrixView` без редактирования сырого JSON виджета. Редактор сохраняет общие правила наследования виджета и настройки видимости по сущностям, которые используются в других виджетах макета, а согласованность проверяет общий контракт `@universo-react/types`.

### MetahubMigrationGuard

Компонент-гард маршрутов, блокирующий навигацию при наличии незавершённых миграций. Использует `MigrationGuardShell` из `@universo-react/migration-guard-shared` для общей логики гарда.
...
-   [`@universo-react/template-mui`](../universo-react-template-mui/README-RU.md) - UI компоненты
-   [`@universo-react/types`](../universo-react-types/README-RU.md) - Общие типы
