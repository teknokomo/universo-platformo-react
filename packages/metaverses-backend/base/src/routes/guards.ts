...
const baseGuards = createAccessGuards<MetaverseRole, MetaverseUser>({
    ...
    getMembership: async (ds: DataSource, userId: string, metaverseId: string, queryRunner?: QueryRunner) => {
        const repo = getManager(ds, queryRunner).getRepository(MetaverseUser)
