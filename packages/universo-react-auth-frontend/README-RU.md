# Фронтенд аутентификации (@universo-react/auth-frontend)
...
```tsx
import { createAuthClient, AuthProvider } from '@universo-react/auth-frontend'
...
```tsx
import { AuthView } from '@universo-react/auth-frontend'
;<AuthView
    labels={{
...
```tsx
import { LoginForm } from '@universo-react/auth-frontend'
;<LoginForm
    client={authClient}
    labels={{
        submit: 'Войти',
...
```tsx
import { useSession } from '@universo-react/auth-frontend'
...
```tsx
import { useAuthError } from '@universo-react/auth-frontend'
...
pnpm --filter @universo-react/auth-frontend build

# Режим разработки с отслеживанием
pnpm --filter @universo-react/auth-frontend dev

# Проверить TypeScript
pnpm --filter @universo-react/auth-frontend lint
```
...
```tsx
// App.tsx
import { createAuthClient, AuthProvider, useAuth } from '@universo-react/auth-frontend'
...
function AppRoutes() {
    ...
    return (
        <Routes>
            ...
            <Route path='/*' element={isAuthenticated ? <AuthenticatedApp /> : <Navigate to='/auth' />} />
        </Routes>
    )
}
```
...
```tsx
// api/client.ts
import { createAuthClient } from '@universo-react/auth-frontend'
...
;<AuthView onLogin={handleLogin} onRegister={handleRegister} errorMapper={translateError} labels={authLabels} />
```
...
-   [Бэкенд сервиса аутентификации](../universo-react-auth-backend/README-RU.md)
-   [Пакет API-клиента](../universo-react-api-client/README-RU.md)
-   [Шаблон Material-UI](../universo-react-template-mui/README-RU.md)
