import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { defineConfig, mergeConfig } from 'vitest/config'

import baseConfig from '../../tools/testing/frontend/vitest.base.config'
import { loadTsconfigAliases } from '../../tools/testing/frontend/loadTsconfigAliases'

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const srcDir = path.resolve(__dirname, 'src')
const templateMuiSrcDir = path.resolve(__dirname, '../universo-template-mui/src')
const tsconfigAliases = loadTsconfigAliases(path.resolve(__dirname, 'tsconfig.json'), __dirname)

export default mergeConfig(
    baseConfig,
    defineConfig({
        root: __dirname,
        resolve: {
            alias: {
                '@universo/template-mui/components/dialogs': path.resolve(templateMuiSrcDir, 'components/dialogs/index.ts'),
                '@universo/template-mui': path.resolve(templateMuiSrcDir, 'index.ts'),
                ...tsconfigAliases,
                '@': srcDir
            }
        },
        test: {
            globals: true,
            include: ['src/**/*.{test,spec}.{ts,tsx,js,jsx}']
        },
        esbuild: {
            loader: 'tsx',
            include: [/src\/.*\.[jt]sx?$/, /tools\/testing\/frontend\/.*\.[jt]sx?$/],
            jsx: 'automatic'
        }
    })
)
