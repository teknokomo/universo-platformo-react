...
const AssistantDialog = ({ show, dialogProps, onCancel, onConfirm, setError }) => {
    ...
    const getAssistantByIdApi = useApi((...args) => api.assistants.getById(...args))
    const getOpenAIAssistantApi = useApi((...args) => api.assistants.getOpenAIAssistant(...args))
    ...
    useEffect(() => {
        if (getAssistantByIdApi.data) {
            setAssistantId(getAssistantByIdApi.data.id)
            setAssistantIcon(getAssistantByIdApi.data.iconSrc)
            setAssistantCredential(getAssistantByIdApi.data.credential)

            const assistantDetails = JSON.parse(getAssistantByIdApi.data.details)
    ...
    }, [getAssistantByIdApi.data])

    useEffect(() => {
        if (getOpenAIAssistantApi.data) {
            syncData(getOpenAIAssistantApi.data)
        }
    }, [getOpenAIAssistantApi.data])

    useEffect(() => {
        if (getOpenAIAssistantApi.error) {
            let errMsg = 'Internal Server Error'
            let error = getOpenAIAssistantApi.error
    ...
    }, [getOpenAIAssistantApi.error])

    useEffect(() => {
        if (getAssistantByIdApi.error) {
            let errMsg = ''
            const error = getAssistantByIdApi.error
            if (error?.response?.data) {
                errMsg = typeof error.response.data === 'object' ? error.response.data.message : error.response.data
            }
            enqueueSnackbar({
    ...
    useEffect(() => {
        ...
        if (dialogProps.type === 'EDIT' && dialogProps.data) {
            ...
            getAssistantByIdApi.request(unikId, dialogProps.assistantId)
    ...
    const addNewAssistant = async () => {
        ...
        try {
            ...
            const createResp = await api.assistants.create(unikId, obj)
            if (createResp) {
                enqueueSnackbar({
    ...
    const saveAssistant = async () => {
        ...
        try {
            ...
            const saveResp = await api.assistants.update(unikId, assistantId, obj)
            if (saveResp) {
                enqueueSnackbar({
    ...
    const onSyncClick = async () => {
        setLoading(true)
        try {
            const getResp = await api.assistants.getOpenAIAssistant(unikId, openAIAssistantId, assistantCredential)
            if (getResp) {
                syncData(getResp)
                enqueueSnackbar({
    ...
    const uploadFormDataToVectorStore = async (formData) => {
        ...
        try {
            ...
            const uploadResp = await api.assistants.uploadFilesToVectorStore(unikId, vectorStoreId, assistantCredential, formData)
            if (uploadResp) {
                enqueueSnackbar({
                ...
                const uploadedFiles = uploadResp
    ...
    const uploadFormDataToCodeInterpreter = async (formData) => {
        setLoading(true)
        try {
            const uploadResp = await api.assistants.uploadFilesToAssistant(unikId, assistantCredential, formData)
            if (uploadResp) {
                enqueueSnackbar({
                    message: t('assistants:messages.fileUploaded'),
                    options: {
    ...
    const deleteAssistant = async (isDeleteBoth) => {
        ...
        try {
            await api.assistants.delete(unikId, assistantId, isDeleteBoth)
            enqueueSnackbar({
        ...
        if (toolType === 'code_interpreter') {
            ...
            try {
                const vectorStoreId = toolResources.file_search?.vector_store_ids?.length
                    ...
                    : ''
                await api.assistants.deleteFilesFromVectorStore(unikId, vectorStoreId, assistantCredential, [fileId])
            } catch (error) {
                console.error(error)
            }
        }
