...
import { DocStoreInputHandler } from '@flowise/docstore-frontend'
...
import { ViewMessagesDialog } from '@flowise/template-mui'
import { CanvasConfigurationDialog, ViewLeadsDialog, ConfirmDialog, PromptGeneratorDialog, ExpandTextDialog } from '@flowise/template-mui'
...
// import { api } from '@universo/api-client' // Replaced import nodesApi from '@/api/nodes' // Duplicate removed
// import { api } from '@universo/api-client' // Replaced import documentstoreApi from '@/api/documentstore' // Duplicate removed

// Const
import { baseURL } from '@flowise/template-mui'
import { SET_CANVAS, closeSnackbar as closeSnackbarAction, enqueueSnackbar as enqueueSnackbarAction } from '@flowise/store'

// Utils
import { initNode } from '@universo/utils/ui-utils/genericHelper'
...
const CustomAssistantConfigurePreview = () => {
    ...
    const getByIdApi = useApi((...args) => api.assistants.getById(...args))
    const getChatModelsApi = useApi((...args) => api.assistants.getChatModels(...args))
    const getDocStoresApi = useApi((...args) => api.assistants.getDocStores(...args))
    const getToolsApi = useApi((...args) => api.assistants.getTools(...args))
    const getCanvasApi = useApi((...args) => api.canvases.getCanvas(...args))
    const getAllApi = useApi((...args) => api.assistants.getAll(...args))
        ...
        if (checkMandatoryFields()) {
            ...
            try {
                ...
                if (!customAssistantCanvasId) {
                    saveResp = await api.canvases.createCanvas(unikId, spaceId, saveObj)
                } else {
                    saveResp = await api.canvases.updateCanvas(unikId, customAssistantCanvasId, saveObj, { spaceId })
                }
                ...
                if (saveResp.data) {
                    ...
                    await api.assistants.update(unikId, customAssistantId, {
    ...
    const addDocStore = async (toolAgentId) => {
        const docStoreVSNode = await api.nodes.getSpecificNode('documentStoreVS')
        const retrieverToolNode = await api.nodes.getSpecificNode('retrieverTool')
    ...
    const handleDeleteFlow = async () => {
        ...
        if (isConfirmed && customAssistantId) {
            try {
                await api.assistants.delete(unikId, customAssistantId)
                if (customAssistantCanvasId) {
                    await api.canvases.deleteCanvas(unikId, customAssistantCanvasId, { spaceId: resolveSpaceId() })
                }
                navigate(-1)
            } catch (error) {
                enqueueSnackbar({
    ...
    const generateDocStoreToolDesc = async (storeId) => {
        ...
        try {
            ...
            const resp = await api.documentStore.generateDocStoreToolDesc(storeId, { selectedChatModel: selectedChatModelObj })
    ...
    useEffect(() => {
        if (getChatModelsApi.data) {
            ...
            if (customAssistantId) {
                ...
                getByIdApi.request(unikId, customAssistantId)
            }
        }
    ...
    useEffect(() => {
        if (getByIdApi.data) {
            ...
            try {
                const assistantDetails = JSON.parse(getByIdApi.data.details)
