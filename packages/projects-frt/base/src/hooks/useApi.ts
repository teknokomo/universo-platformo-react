...
export function useApi<T, TArgs extends any[] = any[]>(apiFunc: ApiFunc<T, TArgs>): UseApiResponse<T, TArgs> {
    ...
    const request = useCallback(async (...args: TArgs): Promise<T | null> => {
        setLoading(true)
        setError(null)
        try {
