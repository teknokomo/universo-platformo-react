...
const renderWithProviders = (ui: React.ReactElement, { route = '/Projects/test-Project-id/members' } = {}) => {
                ...
                <SnackbarProvider maxSnack={3}>
                    <I18nextProvider i18n={i18n}>
                        <MemoryRouter initialEntries={[route]}>
                            <Routes>
                                <Route path='/Projects/:ProjectId/members' element={ui} />
                            </Routes>
                        </MemoryRouter>
                    </I18nextProvider>
                </SnackbarProvider>
...
describe('ProjectMembers', () => {
    ...
    describe('Component rendering with happy-dom', () => {
        it('should render component with translated UI elements', () => {
            renderWithProviders(<ProjectMembers />)
        ...
        it('should render toolbar with search and controls', () => {
            const { container } = renderWithProviders(<ProjectMembers />)
    ...
    describe('Loading state', () => {
        it('should display skeleton loader while fetching members', () => {
            vi.mocked(ProjectsApi.listProjectMembers).mockImplementation(
                () =>
                    new Promise((resolve) =>
                        setTimeout(
                            () =>
    ...
    describe('Error handling', () => {
        it('should display error message when API fails', async () => {
            vi.mocked(ProjectsApi.listProjectMembers).mockRejectedValue(new Error('Network error'))
    ...
    describe('Search functionality', () => {
        ...
        it('should update search field on user input', async () => {
            const { user } = renderWithProviders(<ProjectMembers />)
    ...
    describe('Navigation', () => {
        it('should handle missing ProjectId parameter', () => {
            vi.mocked(ProjectsApi.listProjectMembers).mockImplementation(() => new Promise(() => {}))

            renderWithProviders(<ProjectMembers />, { route: '/Projects//members' })

            // Should not call API without valid ProjectId
            expect(ProjectsApi.listProjectMembers).not.toHaveBeenCalled()
        })
    })
})
