...
describe('DDL Diff Utilities', () => {
    describe('calculateSchemaDiff', () => {
        ...
        describe('when comparing with existing snapshot', () => {
            it('should ignore enumeration entities for physical DDL diff', () => {
                const snapshot = createTestSnapshot({
                    'enum-entity-1111-2222-333344445555': {
                        ...
                        kind: 'enumeration' as SchemaSnapshot['entities'][string]['kind'],
            ...
            it('does not generate physical FK changes for REF fields targeting sets', () => {
                const snapshot = createTestSnapshot()
                const entities: EntityDefinition[] = [
                    createTestEntity({
                        fields: [
                            {
                                ...
                                targetEntityKind: 'set' as EntityDefinition['fields'][number]['targetEntityKind']
                            }
                        ]
                    })
                ]
