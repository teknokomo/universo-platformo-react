...
```bash
pnpm add @universo-react/schema-ddl
```
...
```typescript
import { createDDLServices, generateSchemaName } from '@universo-react/schema-ddl'
...
```typescript
import { generateSchemaName, generateTableName, generateColumnName, buildFkConstraintName, isValidSchemaName } from '@universo-react/schema-ddl'
...
```typescript
import { calculateSchemaDiff, ChangeType } from '@universo-react/schema-ddl'
...
| `generateColumnName(fieldId)`                | Генерирует имя колонки с префиксом attr\_                                     |
