...
export const createMockRepository = <T extends object>(): MockRepository<T> => {
    ...
    return {
        ...
        create: jest.fn().mockImplementation((slot: any) => ({ ...slot, id: 'mock-id' })),
        save: jest.fn().mockImplementation((slot: any) => Promise.resolve({ ...slot, id: slot.id || 'mock-id' })),
        update: jest.fn().mockResolvedValue({ affected: 1 }),
        delete: jest.fn().mockResolvedValue({ affected: 1 }),
        remove: jest.fn().mockImplementation((slot: any) => Promise.resolve(slot))
    }
}
...
export const createMockDataSource = (repositories: RepoMap, options: { isInitialized?: boolean } = {}) => {
    const getRepository = jest.fn((slot: any) => {
        if (typeof slot === 'string') {
            return repositories[slot]
        }
        return repositories[slot?.name] || repositories[slot]
    })
