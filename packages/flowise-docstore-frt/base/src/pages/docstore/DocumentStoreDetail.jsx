...
import DocumentLoaderListDialog from './DocumentLoaderListDialog'
...
import DocumentStoreStatus from './DocumentStoreStatus'
...
const DocumentStoreDetails = () => {
    ...
    const getSpecificDocumentStore = useApi((...args) => api.documentStore.getSpecificDocumentStore(...args))
    ...
    const showVectorStore = (id) => {
        // Navigate to vector store configuration for ALL loaders
        navigate(`/unik/${unikId}/document-stores/vector/${id}/all`)
    }
    ...
    const deleteVectorStoreDataFromStore = async (storeId) => {
        try {
            await api.documentStore.deleteVectorStoreDataFromStore(unikId, storeId)
        } catch (error) {
            console.error(error)
        }
    }
    ...
    const onDocStoreDelete = async (type, file, removeFromVectorStore) => {
        ...
        if (type === 'STORE') {
            ...
            try {
                const deleteResp = await api.documentStore.deleteDocumentStore(unikId, storeId)
            ...
            try {
                const deleteResp = await api.documentStore.deleteLoaderFromStore(unikId, storeId, file.id)
                setBackdropLoading(false)
                if (deleteResp.data) {
                    enqueueSnackbar({
    ...
    const onStoreRefresh = async (storeId) => {
        ...
        if (isConfirmed) {
            ...
            setBackdropLoading(true)
            try {
                const resp = await api.documentStore.refreshLoader(unikId, storeId)
                if (resp.data) {
                    enqueueSnackbar({
                    ...
                    <Stack flexDirection='column' sx={{ gap: 3 }}>
                        ...
                        >
                            {(documentStore?.status === 'STALE' || documentStore?.status === 'UPSERTING') && (
                                <IconButton onClick={onConfirm} size='small' color='primary' title={t('document-store:actions.refresh')}>
                                    <IconRefresh />
                                </IconButton>
                            )}
                            <StyledButton
                                                ...
                                                {documentStore?.loaders &&
                                                    ...
                                                    documentStore?.loaders.map((loader, index) => (
                                                        <LoaderRow
                                                            ...
                                                            onChunkUpsert={() =>
                                                                navigate(
                                                                    `/unik/${unikId}/document-stores/vector/${documentStore.id}/${loader.id}`
                                                                )
                                                            }
