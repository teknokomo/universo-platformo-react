                    ...
                    >
                                    ...
                                    <TimelineContent>
                                        {history.result.numAdded !== undefined && history.result.numAdded > 0 && (
                                            <Box sx={{ fontWeight: 500 }}>
                                                {t('vector-store:upsertResult.added')}: {history.result.numAdded}
                                            </Box>
                                        )}
                                        {history.result.numUpdated !== undefined && history.result.numUpdated > 0 && (
                                            <Box sx={{ fontWeight: 500 }}>
                                                {t('vector-store:upsertResult.updated')}: {history.result.numUpdated}
                                            </Box>
                                        )}
                                        {history.result.numSkipped !== undefined && history.result.numSkipped > 0 && (
                                            <Box sx={{ fontWeight: 500 }}>
                                                {t('vector-store:upsertResult.skipped')}: {history.result.numSkipped}
                                            </Box>
                                        )}
                                        {history.result.numDeleted !== undefined && history.result.numDeleted > 0 && (
                                            <Box sx={{ fontWeight: 500 }}>
                                                {t('vector-store:upsertResult.deleted')}: {history.result.numDeleted}
                                            </Box>
                                        )}
                                        <Button
                                            size='small'
                                        ...
                                        >
                                            {t('vector-store:upsertHistory.details')}
                                        </Button>
                                    </TimelineContent>
                                </TimelineItem>
                            ))}
                        {upsertHistory.length === 0 && (
                            <Stack sx={{ alignItems: 'center', justifyContent: 'center' }} flexDirection='column'>
                                ...
                                <div>{t('vector-store:upsertHistory.empty')}</div>
                            </Stack>
                        )}
                    </Timeline>
                </Box>
