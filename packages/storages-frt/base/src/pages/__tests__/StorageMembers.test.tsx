...
vi.mock('../../api/storages', () => ({
    ...
    inviteStorageMember: vi.fn(),
    updateStorageMemberRole: vi.fn(),
    removeStorageMember: vi.fn()
}))
...
const renderWithProviders = (ui: React.ReactElement, { route = '/storages/test-storage-id/members' } = {}) => {
                ...
                <SnackbarProvider maxSnack={3}>
                    <I18nextProvider i18n={i18n}>
                        <MemoryRouter initialEntries={[route]}>
                            <Routes>
                                <Route path='/storages/:storageId/members' element={ui} />
                            </Routes>
                        </MemoryRouter>
                    </I18nextProvider>
                </SnackbarProvider>
...
describe('StorageMembers', () => {
    beforeEach(() => {
        vi.clearAllMocks()
        // Default mock: return empty list with proper structure
        vi.mocked(storagesApi.listStorageMembers).mockResolvedValue({
            data: [],
            pagination: {
                total: 0,
                limit: 20,
    ...
    describe('Component rendering with happy-dom', () => {
        it('should render component with translated UI elements', () => {
            renderWithProviders(<StorageMembers />)
        ...
        it('should render toolbar with search and controls', () => {
            const { container } = renderWithProviders(<StorageMembers />)
    ...
    describe('Loading state', () => {
        it('should display skeleton loader while fetching members', () => {
            vi.mocked(storagesApi.listStorageMembers).mockImplementation(
                () =>
                    new Promise((resolve) =>
                        setTimeout(
                            () =>
    ...
    describe('Error handling', () => {
        it('should display error message when API fails', async () => {
            vi.mocked(storagesApi.listStorageMembers).mockRejectedValue(new Error('Network error'))
    ...
    describe('Search functionality', () => {
        ...
        it('should update search field on user input', async () => {
            const { user } = renderWithProviders(<StorageMembers />)
    ...
    describe('Navigation', () => {
        it('should handle missing storageId parameter', () => {
            vi.mocked(storagesApi.listStorageMembers).mockImplementation(() => new Promise(() => {}))

            renderWithProviders(<StorageMembers />, { route: '/storages//members' })

            // Should not call API without valid storageId
            expect(storagesApi.listStorageMembers).not.toHaveBeenCalled()
        })
    })
})
