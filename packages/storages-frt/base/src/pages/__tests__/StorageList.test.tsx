            ...
            <QueryClientProvider client={queryClient}>
                <SnackbarProvider maxSnack={3}>
                    <I18nextProvider i18n={i18n}>
                        <MemoryRouter initialEntries={['/storages']}>{ui}</MemoryRouter>
                    </I18nextProvider>
                </SnackbarProvider>
            </QueryClientProvider>
        )
...
describe('StorageList', () => {
    ...
    describe('Loading State', () => {
        it('should display loading skeletons while fetching data', () => {
            ...
            vi.mocked(storagesApi.listStorages).mockImplementation(() => new Promise(() => {}))

            renderWithProviders(<StorageList />)
    ...
    describe('Error State', () => {
        it('should display error message when API fails', async () => {
            // Mock API to reject
            vi.mocked(storagesApi.listStorages).mockRejectedValue(new Error('Network error'))
    ...
    describe('Empty State', () => {
        it('should display empty state when no storages exist', async () => {
            // Mock API to return empty list
            vi.mocked(storagesApi.listStorages).mockResolvedValue({
                data: [],
                total: 0,
                limit: 20,
                offset: 0
            ...
            await waitFor(() => {
                expect(screen.getByText(/no storages/i) || screen.getByText(/get started/i)).toBeInTheDocument()
            })
        })
    ...
    describe('Success State - List Rendering', () => {
        const mockStorages = [
            {
                id: 'storage-1',
                name: 'Test Storage 1',
                description: 'First test storage',
                role: 'admin',
                containersCount: 5,
                slotsCount: 20,
                membersCount: 3,
        ...
        it('should render storage cards with names', async () => {
            renderWithProviders(<StorageList />)

            await waitFor(() => {
                expect(screen.getByText('Test Storage 1')).toBeInTheDocument()
                expect(screen.getByText('Test Storage 2')).toBeInTheDocument()
            })
        })

        it('should display storage descriptions', async () => {
            renderWithProviders(<StorageList />)

            await waitFor(() => {
                expect(screen.getByText('First test storage')).toBeInTheDocument()
                expect(screen.getByText('Second test storage')).toBeInTheDocument()
            })
        })

        it('should show role chips for storages', async () => {
            renderWithProviders(<StorageList />)
        ...
        it('should display containers and slots counts', async () => {
            renderWithProviders(<StorageList />)
    ...
    describe('Search Functionality', () => {
        ...
        it('should call API with search parameter when typing', async () => {
            const { user } = renderWithProviders(<StorageList />)

            await waitFor(() => {
                expect(screen.getByText('Production Storage')).toBeInTheDocument()
            })
            ...
            if (searchInput) {
                ...
                await waitFor(
                    () => {
                        const calls = vi.mocked(storagesApi.listStorages).mock.calls
    ...
    describe('Pagination', () => {
        const generateMockStorages = (count: number) =>
    ...
    describe('Create Storage', () => {
        beforeEach(() => {
            ...
            vi.mocked(storagesApi.createStorage).mockResolvedValue({
                id: 'new-storage',
                name: 'New Storage',
    ...
    describe('Edge Cases', () => {
        it('should handle storages without descriptions', async () => {
            vi.mocked(storagesApi.listStorages).mockResolvedValue({
                data: [
                    {
                        id: 'no-desc',
        ...
        it('should handle storages without role', async () => {
            vi.mocked(storagesApi.listStorages).mockResolvedValue({
                data: [
                    {
                        id: 'no-role',
        ...
        it('should handle storages with zero counts', async () => {
            vi.mocked(storagesApi.listStorages).mockResolvedValue({
                data: [
                    {
                        id: 'zero-counts',
