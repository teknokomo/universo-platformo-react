...
const StorageMembers = () => {
    ...
    const { user } = useAuth()
    const { t, i18n } = useTranslation(['storages', 'roles', 'common', 'flowList'])
    ...
    const paginationResult = usePaginated<StorageMember, 'email' | 'role' | 'created'>({
        queryKeyFn: (params) => storagesQueryKeys.membersList(storageId!, params),
        queryFn: (params) => storagesApi.listStorageMembers(storageId!, params),
        initialLimit: 20,
        sortBy: 'created',
        sortOrder: 'desc',
        enabled: !!storageId
    ...
    const memberColumns = [
        ...
        },
        {
            ...
            align: 'center',
            render: (row: StorageMember) => {
                const roleKey = (row.role || 'member') as StorageRole
    ...
    ] satisfies TableColumn<StorageMember>[]

    const createMemberContext = useCallback(
        (baseContext: Partial<ActionContext<StorageMember, MemberData>>): ActionContext<StorageMember, MemberData> => ({
            ...baseContext,
            ...
            helpers: {
                ...
                openDeleteDialog: (member: StorageMember) => {
    ...
    if (!storageId) {
        return (
            <MainCard
                ...
                shadow={false}
            >
                <EmptyListState image={APIEmptySVG} imageAlt='Invalid storage' title={tc('errors.connectionFailed')} />
            </MainCard>
        )
    }
    ...
    return (
        ...
        >
                    ...
                    {isLoading && members.length === 0 ? (
                        ...
                        <>
                            {view === 'card' ? (
                                ...
                                >
                                    {members.map((member: StorageMember) => {
                                                ...
                                                headerAction={
                                                    descriptors.length > 0 ? (
                                                        <Box onClick={(e) => e.stopPropagation()}>
                                                            <BaseEntityMenu<StorageMember, MemberFormData>
                                                                entity={member}
                                                                entityKind='member'
                                ...
                                <Box sx={{ mx: { xs: -1.5, md: -2 } }}>
                                    <FlowListTable
                                        ...
                                        renderActions={(row: StorageMember) => {
                                            // Owner role cannot be edited or removed
                                            if (row.role === 'owner') {
                                                return null
                                            }
            ...
            <ConfirmDeleteDialog
                ...
                title={removeDialogState.member?.userId === user?.id ? tc('confirmSelfRemove') : tc('members.confirmRemove')}
                description={
