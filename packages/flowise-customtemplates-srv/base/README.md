...
```bash
pnpm add @flowise/customtemplates-srv
```
...
```typescript
import { createCustomTemplatesService } from '@flowise/customtemplates-srv'
...
**Note**: The `unik_id` column, FK constraint, and index are added later by the `AddUniksAndLinked` migration from `@universo/uniks-srv`. This follows the same pattern as `tool`, `credential`, `assistant`, and other Flowise tables.
