...
Фронтенд-приложение для управления метахабами, разделами и сущностями в экосистеме Universo Platformo. Metahubs Frontend предоставляет комплексные UI-процессы для управления трёхуровневой архитектурой Метахабы → Разделы → Сущности с полной изоляцией данных и безопасностью.
...
- **Иерархическая организация**: Трёхуровневая архитектура (Метахабы → Разделы → Сущности)
- **Полная изоляция данных**: Сущности и разделы из разных метахабов полностью разделены
- **Ролевой доступ**: Пользовательские роли и разрешения для контроля доступа к метахабным
...
- **Разделы**: Логические группировки внутри метахабов (например, "Веб-сервисы", "Мобильные приложения")
- **Сущности**: Отдельные активы, принадлежащие конкретным разделам внутри метахабов
...
// Список метахабов с возможностями управления
function MetahubsPage() {
  return <MetahubList />
}

// Панель управления и аналитика метахаба
...
function MetahubData() {
  ...
  return <div>Найдено {metahubs?.length} метахабов</div>
}
```
...
│   │   ├── MetahubList.tsx   # Основной компонент списка
│   │   ├── MetahubBoard.tsx  # Компонент панели управления
│   │   └── MetahubActions.ts # Определения действий
...
// Получить конкретную метахаб
const metahub = await metahubsApi.getMetahub(id)

// Создать новую метахаб
...
### Операции в контексте метахаба
```typescript
// Получить разделы для конкретной метахаба
const sections = await metahubsApi.getMetahubSections(metahubId)

// Получить сущности для конкретной метахаба  
const entities = await metahubsApi.getMetahubEntities(metahubId)

// Связать раздел с метахаба
...
- **Трёхуровневая модель**: Метахабы → Разделы → Сущности
- **Изоляция данных**: Строгие границы контекста между метахабами
...
```typescript
// Всегда поддерживать контекст метахаба
...
test('отображает список метахабов', () => {
  render(<MetahubList />)
  expect(screen.getByText('Метахабы')).toBeInTheDocument()
})
```
