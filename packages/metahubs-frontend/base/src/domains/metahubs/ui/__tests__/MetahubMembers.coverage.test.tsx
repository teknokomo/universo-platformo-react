...
describe('MetahubMembers (coverage)', () => {
    it('executes invite/edit/remove flows and related callbacks', async () => {
        const { default: MetahubMembers } = await import('../MetahubMembers')

        render(<MetahubMembers />)
        ...
        const confirmDeleteButton = await screen.findByText('confirm-delete')
        await act(async () => {
