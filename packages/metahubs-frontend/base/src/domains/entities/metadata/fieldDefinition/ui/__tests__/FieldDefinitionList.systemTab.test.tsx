...
vi.mock('../../hooks/mutations', () => ({
    useCreateFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useCopyFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useUpdateFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useDeleteFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useMoveFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useReorderFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useToggleFieldDefinitionRequired: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useSetDisplayFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false }),
    useClearDisplayFieldDefinition: () => ({ mutateAsync: vi.fn(), isPending: false })
}))

vi.mock('../api', () => ({
    listAttributes: vi.fn(),
    listFieldDefinitionsDirect: vi.fn(),
...
vi.mock('./FieldDefinitionActions', () => ({
    __esModule: true,
...
vi.mock('./NestedFieldDefinitionList', () => ({
    __esModule: true,
    default: () => null
}))

vi.mock('./dnd', () => ({
    FieldDefinitionDndProvider: ({ children }: { children: React.ReactNode }) => <>{children}</>,
    FieldDefinitionDndContainerRegistryProvider: ({ children }: { children: React.ReactNode }) => <>{children}</>,
...
describe('FieldDefinitionList system tab', () => {
    beforeEach(() => {
        vi.clearAllMocks()
        usePaginatedMock.mockReturnValue({
            data: [systemFieldDefinition],
            isLoading: false,
            error: null,
            meta: {
                totalAll: 1,
    ...
    it('renders the dedicated system view without the create action', async () => {
        render(
            <MemoryRouter initialEntries={['/metahub/metahub-1/entities/catalog/instance/catalog-1/system']}>
                <Routes>
                    <Route
                        path='/metahub/:metahubId/entities/:kindKey/instance/:linkedCollectionId/system'
    ...
    it('keeps system redirects inside the standard catalog entity route tree', async () => {
        render(
            <MemoryRouter initialEntries={['/metahub/metahub-1/entities/catalog/instance/catalog-1/field-definitions?tab=system']}>
                <Routes>
                    <Route
                        path='/metahub/:metahubId/entities/:kindKey/instance/:linkedCollectionId/field-definitions'
                        element={<FieldDefinitionList />}
                    />
                </Routes>
            </MemoryRouter>
    ...
    it('renders shared rows as read-only and keeps local rows editable in merged views', async () => {
        ...
        render(
            <MemoryRouter initialEntries={['/metahub/metahub-1/entities/catalog/instance/catalog-1/field-definitions']}>
                <Routes>
                    <Route
                        path='/metahub/:metahubId/entities/:kindKey/instance/:linkedCollectionId/field-definitions'
                        element={<FieldDefinitionList />}
