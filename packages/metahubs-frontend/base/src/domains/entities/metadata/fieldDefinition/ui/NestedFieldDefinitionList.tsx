...
import type { FieldDefinitionDataType } from '../../../../../types'
import {
    FieldDefinition,
...
import { metahubsQueryKeys, invalidateFieldDefinitionsQueries } from '../../../../shared'
import {
    useCreateChildFieldDefinition,
...
import { useContainerRegistry, useFieldDefinitionDndState } from './dnd'
...
const getDisplayAttributeFlag = (row: FieldDefinitionDisplay): boolean =>
    Boolean((row as { isDisplayAttribute?: boolean }).isDisplayAttribute)
...
function localizeTableValidationError(
    ...
    if (code === 'NESTED_TABLE_FORBIDDEN') {
        return t('fieldDefinitions.tableValidation.nestedTableNotAllowed', 'Nested TABLE fieldDefinitions are not allowed')
    }
    if (code === 'TABLE_CHILD_LIMIT_REACHED') {
        const max = extractResponseMaxChildAttributes(error)
        return t('fieldDefinitions.tableValidation.maxChildAttributes', 'Maximum {{max}} child fieldDefinitions per TABLE', {
            max: max ?? maxChildAttributesFallback ?? '—'
        })
    }
    if (code === 'TABLE_ATTRIBUTE_LIMIT_REACHED') {
        const max = extractResponseMaxTableAttributes(error)
        return t('fieldDefinitions.tableValidation.maxTableAttributes', 'Maximum {{max}} TABLE fieldDefinitions per catalog', {
            max: max ?? '—'
        })
    }
    if (code === 'TABLE_DISPLAY_ATTRIBUTE_FORBIDDEN') {
        return t('fieldDefinitions.tableValidation.tableCannotBeDisplay', 'TABLE fieldDefinitions cannot be set as the display attribute')
    }
    return null
}
...
interface NestedFieldDefinitionListProps {
    metahubId: string
    treeEntityId?: string
...
const NestedFieldDefinitionList = ({
    metahubId,
    treeEntityId,
    linkedCollectionId,
    parentAttributeId,
    ...
    searchFilter,
    onRefresh
}: NestedFieldDefinitionListProps) => {
    const codenameConfig = useCodenameConfig()
    const preferredVlcLocale = useMetahubPrimaryLocale()
    const attributeCodenameScope = useSettingValue<string>('entity.catalog.attributeCodenameScope') ?? 'per-level'
    const allowedAttributeTypesSetting = useSettingValue<string[]>('entity.catalog.allowedAttributeTypes')
    const allowAttributeCopySetting = useSettingValue<boolean>('entity.catalog.allowAttributeCopy')
    const allowAttributeDeleteSetting = useSettingValue<boolean>('entity.catalog.allowAttributeDelete')
    const allowDeleteLastDisplayAttributeSetting = useSettingValue<boolean>('entity.catalog.allowDeleteLastDisplayAttribute')
    ...
    const { dialogs, openCreate, openEdit, openCopy, openDelete, close } = useListDialogs<FieldDefinition>()
    ...
    const createChildAttributeMutation = useCreateChildFieldDefinition()
    ...
    const queryKey = useMemo(
        () => ['metahubs', 'childAttributes', metahubId, linkedCollectionId, parentAttributeId] as const,
        [metahubId, linkedCollectionId, parentAttributeId]
    )
    ...
    const { data, isLoading, error } = useQuery({
        ...
        queryFn: async () => {
            if (treeEntityId) {
                return fieldDefinitionsApi.listChildFieldDefinitions(metahubId, treeEntityId, linkedCollectionId, parentAttributeId)
            }
            return fieldDefinitionsApi.listChildFieldDefinitionsDirect(metahubId, linkedCollectionId, parentAttributeId)
        },
        enabled: !!metahubId && !!linkedCollectionId && !!parentAttributeId
    })
    ...
    const { data: globalCodenamesData } = useQuery({
        ...
        enabled: isGlobalScope && !!metahubId && !!linkedCollectionId
    })
    ...
    const { activeContainerId, overContainerId, pendingTransfer, activeFieldDefinition: dndActiveAttr } = useFieldDefinitionDndState()
    ...
    const invalidateChildQueries = useCallback(async () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['metahubs', 'childAttributesForElements', metahubId, linkedCollectionId] })
        ...
        if (treeEntityId) {
            await invalidateFieldDefinitionsQueries.all(queryClient, metahubId, treeEntityId, linkedCollectionId)
        } else {
            queryClient.invalidateQueries({ queryKey: metahubsQueryKeys.fieldDefinitionsDirect(metahubId, linkedCollectionId) })
        }
        onRefresh?.()
    }, [queryClient, queryKey, treeEntityId, metahubId, linkedCollectionId, onRefresh])

    const allowedChildDataTypes = useMemo(() => {
        const baseChildTypes = TABLE_CHILD_DATA_TYPES as unknown as FieldDefinitionDataType[]
        if (!Array.isArray(allowedAttributeTypesSetting)) return baseChildTypes
        const allowedSet = new Set(
            allowedAttributeTypesSetting.filter(
                (item): item is FieldDefinitionDataType =>
                    typeof item === 'string' && baseChildTypes.includes(item as FieldDefinitionDataType)
            )
        )
    ...
    const childDataTypeOptions = useMemo(
        () =>
            allowedChildDataTypes.map((dt) => ({
                value: dt,
                label: t(`fieldDefinitions.dataTypeOptions.${dt.toLowerCase()}`, dt)
            })),
        [allowedChildDataTypes, t]
    )

    const canDeleteChildAttribute = useCallback(
        (attribute: FieldDefinitionDisplay | FieldDefinition | null | undefined): boolean => {
    ...
    const childActionDescriptors: ActionDescriptor<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>[] = useMemo(
        () => [
            {
                id: 'edit',
                ...
                icon: <EditIcon />,
                order: 10,
                onSelect: (ctx: ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>) => {
            ...
            },
            {
                ...
                order: 20,
                group: 'reorder',
                enabled: (ctx: ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>) => {
                ...
                onSelect: async (ctx: ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>) => {
                    try {
                        const contextExtras = ctx as ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload> &
            ...
            },
            {
                ...
                labelKey: 'fieldDefinitions.actions.clearDisplayAttribute',
                icon: <StarIcon />,
                order: 51,
                group: 'flags',
                visible: (ctx: ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>) => {
            ...
            },
            {
                ...
                group: 'danger',
                enabled: (ctx: ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>) => {
                    const raw = childAttributeMap.get(ctx.entity.id) ?? (ctx.entity as unknown as FieldDefinition)
                    return canDeleteChildAttribute(raw)
                },
                onSelect: (ctx: ActionContext<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>) => {
                    const attr = childAttributeMap.get(ctx.entity.id) ?? (ctx.entity as unknown as FieldDefinition)
                    openDelete(attr)
                }
            }
        ],
    ...
    const localizedDefaults = useMemo(() => {
        ...
        return {
            ...
            dataType: defaultDataType as FieldDefinitionDataType,
    ...
    const validate = useCallback(
        (values: GenericFormValues) => {
            ...
            if (!normalized) errors.codename = t('fieldDefinitions.validation.codenameRequired', 'Codename is required')
            else if (!isValidCodenameForStyle(normalized, codenameConfig.style, codenameConfig.alphabet, codenameConfig.allowMixed))
                errors.codename = t('fieldDefinitions.validation.codenameInvalid')
    ...
    const buildTabs = useCallback(
            ...
            const tabs: TabConfig[] = [
                {
                    ...
                    content: (
                        <FieldDefinitionFormFields
                            ...
                            dataTypeHelperText={
                                isEditMode
                                    ? t('fieldDefinitions.edit.typeChangeDisabled', 'Data type cannot be changed after creation')
                                    : undefined
                            }
        ...
        [preferredVlcLocale, t, tc, metahubId, linkedCollectionId, childDataTypeOptions, childAttributes.length]
    )
    ...
    const handleCreate = async (data: GenericFormValues) => {
        ...
        try {
            ...
            if (!codename) {
                setDialogError(t('fieldDefinitions.validation.codenameRequired'))
                return
            }
            ...
            const normalizedUiConfig = sanitizeChildAttributeUiConfig(dataType as FieldDefinitionDataType, targetEntityKind, uiConfig)
            const payload: FieldDefinitionLocalizedPayload & {
                ...
                dataType: dataType as FieldDefinitionDataType,
                ...
                },
                {
                    onError: (e: unknown) => {
                        if (extractResponseCode(e) === 'TABLE_CHILD_LIMIT_REACHED') {
                            ...
                            const localizedMsg =
                                localizeTableValidationError(e, t, maxChildAttributesLimit) || msg || t('fieldDefinitions.createError')
            ...
            const msg = extractResponseMessage(e) ?? (e instanceof Error ? e.message : '')
            const localizedMsg = localizeTableValidationError(e, t, maxChildAttributesLimit) || msg || t('fieldDefinitions.createError')
            setDialogError(localizedMsg)
        }
    }
    ...
    const handleUpdate = async (data: GenericFormValues) => {
        ...
        try {
            ...
            const codename = normalizeCodenameForStyle(rawCodename, codenameConfig.style, codenameConfig.alphabet)
            if (!codename) {
                setEditDialogError(t('fieldDefinitions.validation.codenameRequired'))
                return
            }
            const codenamePayload = ensureLocalizedContent(codenameValue, namePrimaryLocale, codename)
            ...
            updateChildAttributeMutation.mutate(
                {
                    ...
                    linkedCollectionId,
                    parentAttributeId,
                    fieldDefinitionId: currentAttribute.id,
                    data: payload
                },
                {
                    onError: (e: unknown) => {
    ...
    const handleCopy = (data: GenericFormValues) => {
        ...
        const codename = normalizeCodenameForStyle(rawCodename, codenameConfig.style, codenameConfig.alphabet)
        if (!codename) {
            setCopyDialogError(t('fieldDefinitions.validation.codenameRequired', 'Codename is required'))
            return
        }
        const codenamePayload = ensureLocalizedContent(codenameValue, namePrimaryLocale, codename)
        ...
        copyChildAttributeMutation.mutate(
            ...
            },
            {
                onError: (e: unknown) => {
                    const msg = extractResponseMessage(e) ?? (e instanceof Error ? e.message : '')
                    setCopyDialogError(msg || t('fieldDefinitions.copyError', 'Failed to copy attribute'))
    ...
    const editInitialValues = useMemo(() => {
        ...
        return {
            ...
            dataType: attr.dataType as FieldDefinitionDataType,
    ...
    const copyInitialValues = useMemo(() => {
        ...
        return {
            ...
            codename: null,
            codenameTouched: false,
            dataType: source.dataType as FieldDefinitionDataType,
    ...
    const tableData = useMemo(
        () => filteredChildAttributes.map((attr) => toFieldDefinitionDisplay(attr, i18n.language)),
            ...
            <Box>
                <Stack direction='row' justifyContent='space-between' alignItems='center' sx={{ mb: 1 }}>
                    <Typography variant='caption' color='text.secondary'>
                        {t('fieldDefinitions.childAttributes', 'Child Attributes')} ({childAttributes.length})
                    </Typography>
                    <Tooltip
                        title={
                            isChildLimitReached && maxChildAttributesLimit !== null
                                ? t(
                                      'fieldDefinitions.typeSettings.table.maxChildAttributesReached',
                                      'Maximum {{max}} child fieldDefinitions reached',
                                      {
                    ...
                    (() => {
                        ...
                        if (pendingTransfer) {
                            if (pendingTransfer.fromContainerId === containerId) {
                                ...
                                const ghost = toFieldDefinitionDisplay(dndActiveAttr, i18n.language)
                            ...
                            <FlowListTable<FieldDefinitionDisplay>
                                ...
                                emptyStateMessage={
                                    isInvalidDropTarget && maxChildAttributesLimit !== null
                                        ...
                                        : t('fieldDefinitions.noChildAttributes', 'No child fieldDefinitions yet')
                                }
                                renderActions={(row: FieldDefinitionDisplay) => {
                                    ...
                                    return (
                                        <BaseEntityMenu<FieldDefinitionDisplay, FieldDefinitionLocalizedPayload>
                                            entity={toFieldDefinitionDisplay(originalAttribute, i18n.language)}
                ...
                <EntityFormDialog
                    ...
                    title={t('fieldDefinitions.createChildDialog.title', 'Add Child Attribute')}
                    nameLabel={tc('fields.name', 'Name')}
                ...
                <EntityFormDialog
                    ...
                    mode='edit'
                    title={t('fieldDefinitions.editChildDialog.title', 'Edit Child Attribute')}
                    nameLabel={tc('fields.name', 'Name')}
                    descriptionLabel={tc('fields.description', 'Description')}
                ...
                <EntityFormDialog
                    ...
                    mode='copy'
                    title={t('fieldDefinitions.copyTitle', 'Copy Attribute')}
                    saveButtonText={t('fieldDefinitions.copy.action', 'Copy')}
                    savingButtonText={t('fieldDefinitions.copy.actionLoading', 'Copying...')}
                    cancelButtonText={tc('actions.cancel', 'Cancel')}
                ...
                <ConfirmDeleteDialog
                    ...
                    title={t('fieldDefinitions.deleteDialog.title', 'Delete Attribute')}
                    description={t('fieldDefinitions.deleteChildDialog.message', 'Are you sure you want to delete this child attribute?')}
                    ...
                    onConfirm={() => {
                        ...
                        const fieldDefinitionId = dialogs.delete.item.id
                        close('delete')
...
export default NestedFieldDefinitionList
