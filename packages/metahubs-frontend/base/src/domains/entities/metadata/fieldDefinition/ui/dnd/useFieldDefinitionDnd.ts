...
interface UseFieldDefinitionDndOptions {
...
export function useFieldDefinitionDnd({
    containers,
    allowCrossListRootChildren,
    allowCrossListBetweenChildren,
    onReorder,
    onValidateTransfer
}: UseFieldDefinitionDndOptions) {
    ...
    const findFieldDefinition = useCallback(
        (itemId: string): FieldDefinition | undefined => {
    ...
    const handleDragEnd = useCallback(
        async (event: DragEndEvent) => {
            ...
            const fieldDef = findFieldDefinition(String(active.id))
            if (!fieldDef) return
        ...
        [containers, findContainer, findFieldDefinition, isCrossListAllowed, onReorder]
    )
    ...
    const activeFieldDefinition = activeId ? findFieldDefinition(activeId) : undefined
