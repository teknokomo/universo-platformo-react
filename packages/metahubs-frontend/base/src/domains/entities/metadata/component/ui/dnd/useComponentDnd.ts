...
interface UseComponentDndOptions {
...
export function useComponentDnd({
    ...
    useEffect(() => {
        ...
        let cancelled = false
        const { fieldDef, activeId: xferId, targetParentId, sourceParentId, targetItemCount, newSortOrder } = deferredCrossTransfer

        const execute = async () => {
            if (onValidateTransferRef.current) {
                const allowed = await onValidateTransferRef.current(fieldDef, targetParentId, targetItemCount)
    ...
    const findComponent = useCallback(
        (itemId: string): Component | undefined => {
    ...
    const isCrossListAllowed = useCallback(
        (sourceContainer: ContainerInfo, targetContainer: ContainerInfo): boolean => {
            ...
            const sourceIsRoot = sourceContainer.parentComponentId === null
            const targetIsRoot = targetContainer.parentComponentId === null
    ...
    const handleDragEnd = useCallback(
        async (event: DragEndEvent) => {
            ...
            const fieldDef = findComponent(String(active.id))
            if (!fieldDef) return
            ...
            if (activeContainer.id === overContainer.id) {
                ...
                const transferFieldDef = fieldDef
                const targetParentId = overContainer.parentComponentId
                const sourceParentId = activeContainer.parentComponentId
                const targetItems = overContainer.items
        ...
        [containers, findContainer, findComponent, isCrossListAllowed, onReorder]
    )
    ...
    const activeComponent = activeId ? findComponent(activeId) : undefined
