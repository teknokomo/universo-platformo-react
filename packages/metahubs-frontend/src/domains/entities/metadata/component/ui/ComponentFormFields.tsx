...
import type { VersionedLocalizedContent, ComponentDefinitionDataType, EntityKind, EnumPresentationMode } from '@universo/types'
...
const getObjectComponentDefaultValidationRules = (dataType: ComponentDefinitionDataType): Partial<ComponentDefinitionValidationRules> => {
    const defaults = getDefaultValidationRules(dataType)
    if (dataType !== 'STRING') return defaults
    return {
        ...defaults,
        maxLength:
            typeof (defaults as ComponentDefinitionValidationRules).maxLength === 'number' &&
            (defaults as ComponentDefinitionValidationRules).maxLength > 0
                ? (defaults as ComponentDefinitionValidationRules).maxLength
                : STRING_DEFAULT_MAX_LENGTH
    }
}

export type ComponentFormFieldsProps = {
    ...
    displayComponentHelper?: string
    displayComponentLocked?: boolean
    dataTypeOptions: Array<{ value: ComponentDefinitionDataType; label: string }>
    ...
    /** When true, hides the display component switch (moved to Presentation tab) */
    hideDisplayComponent?: boolean
...
const ComponentFormFields = ({
    ...
    const dataType = (values.dataType as ComponentDefinitionDataType | undefined) ?? 'STRING'
    ...
    useEffect(() => {
        if (displayComponentLocked && !isDisplayComponent) {
            setValue('isDisplayComponent', true)
        }
    }, [displayComponentLocked, isDisplayComponent, setValue])

    useEffect(() => {
        if (!isDisplayComponent || isRequired) return
        setValue('isRequired', true)
    }, [isDisplayComponent, isRequired, setValue])
            ...
            case 'REF':
                return (
                    <TargetEntitySelector
                        ...
                        excludeObjectCollectionId={currentObjectCollectionId}
                        disabled={isLoading}
            ...
            case 'TABLE':
                return (
                    <Stack spacing={2}>
                        <Stack direction='row' spacing={2}>
                            <TextField
                                label={t('components.typeSettings.table.minRows', 'Min rows')}
                                type='number'
                                size='small'
                                fullWidth
                                disabled={isLoading}
                            ...
                            />
                            <TextField
                                label={t('components.typeSettings.table.maxRows', 'Max rows')}
                                type='number'
                                size='small'
                                fullWidth
                                disabled={isLoading}
                        ...
                        <TextField
                            label={t('components.typeSettings.table.maxChildComponents', 'Max child components')}
                            type='number'
                            size='small'
                            fullWidth
                            disabled={isLoading}
                            value={validationRules.maxChildComponents ?? ''}
                            onChange={(e) =>
    ...
    return (
        <Stack spacing={2}>
            ...
            <FormControl fullWidth disabled={isLoading || dataTypeDisabled}>
                <InputLabel id='component-data-type-label'>{dataTypeLabel}</InputLabel>
                <Select
                    labelId='component-data-type-label'
                    label={dataTypeLabel}
                    value={dataType}
                    onChange={(event) => {
                        const newType = event.target.value as ComponentDefinitionDataType
                        setValue('dataType', newType)
                        // Reset validationRules to defaults for new type
                        setValue('validationRules', getObjectComponentDefaultValidationRules(newType))
                    }}
                >
            ...
            <FormControlLabel
                ...
                label={requiredLabel}
                disabled={isLoading || isDisplayComponent}
            />
            {/* Display component toggle (hidden when Presentation tab handles it) */}
            {!hideDisplayComponent && (
                <Box>
                    <FormControlLabel
                        control={
                            <Switch
                                checked={isDisplayComponent}
                                onChange={(event) => setValue('isDisplayComponent', event.target.checked)}
                            />
                        }
                        label={displayComponentLabel}
                        disabled={isLoading || displayComponentLocked}
                    />
                    {displayComponentHelper && <FormHelperText sx={{ mt: -0.5, ml: 7 }}>{displayComponentHelper}</FormHelperText>}
                </Box>
            )}
            <Divider />
            <CodenameField
...
export type PresentationTabFieldsProps = {
    ...
    isRequired?: boolean
    forceDisplayComponentWhenLocked?: boolean
}
...
export const PresentationTabFields = ({
    ...
    useEffect(() => {
        if (!displayComponentLocked || !forceDisplayComponentWhenLocked || isDisplayComponent) return
        setValue('isDisplayComponent', true)
    }, [displayComponentLocked, forceDisplayComponentWhenLocked, isDisplayComponent, setValue])
            ...
            <Box>
                <FormControlLabel
                    control={
                        <Switch
                            checked={isDisplayComponent}
                            onChange={(_, checked) => setValue('isDisplayComponent', checked)}
                            disabled={isLoading || displayComponentLocked}
                        />
                    }
                    label={displayComponentLabel}
                ...
                <>
                    ...
                    <Box>
                        <FormControlLabel
                            ...
                            label={t('components.presentation.multilineEditor', 'Use multiline editor')}
                        />
                ...
                <>
                    ...
                    <Box>
                        <FormControlLabel
                            ...
                            label={t('components.typeSettings.table.showTitle', 'Show table title')}
                        />
                    </Box>
                </>
            )}
            ...
            {isEnumRef && (
                <>
                    ...
                    {!targetEntityId && (
                        <Alert severity='info'>
                            {t(
                                'components.presentation.enumTargetRequired',
                    ...
                    <FormControl fullWidth disabled={isLoading || !targetEntityId}>
                        ...
                        >
                            <MenuItem value='select'>{t('components.presentation.enumModeOptions.select', 'Input field')}</MenuItem>
                            <MenuItem value='radio'>{t('components.presentation.enumModeOptions.radio', 'Radio field')}</MenuItem>
                            <MenuItem value='label'>{t('components.presentation.enumModeOptions.label', 'Label field')}</MenuItem>
                        </Select>
                        <FormHelperText>
                            {t(
                                'components.presentation.enumModeHelper',
                    ...
                    <Box>
                        <FormControlLabel
                            ...
                            label={t('components.presentation.allowEmpty', 'Can be empty')}
                        />
                        <FormHelperText sx={{ mt: -0.5, ml: 7 }}>
                            {t(
                                'components.presentation.allowEmptyHelper',
                    ...
                    <FormControl fullWidth disabled={isLoading || !targetEntityId || isLoadingEnumValues}>
                        <InputLabel id='enum-default-value-label'>
                            {t('components.presentation.defaultEnumValue', 'Default value')}
                        </InputLabel>
                        <Select
                            size='medium'
                            labelId='enum-default-value-label'
                            label={t('components.presentation.defaultEnumValue', 'Default value')}
                        ...
                        <FormHelperText>
                            {isLoadingEnumValues
                                ...
                                : t(
                                      'components.presentation.defaultEnumValueHelper',
                    ...
                    {enumPresentationMode === 'label' && !defaultEnumValueId && (
                        <FormControl fullWidth disabled={isLoading || !targetEntityId}>
                            <InputLabel id='enum-label-empty-display-label'>
                                {t('components.presentation.labelEmptyDisplay', 'Empty value display')}
                            </InputLabel>
                            <Select
                                size='medium'
                                labelId='enum-label-empty-display-label'
                                label={t('components.presentation.labelEmptyDisplay', 'Empty value display')}
                            ...
                            >
                                <MenuItem value='empty'>
                                    {t('components.presentation.labelEmptyDisplayOptions.empty', 'Empty value')}
                                </MenuItem>
                                <MenuItem value='dash'>{t('components.presentation.labelEmptyDisplayOptions.dash', 'Dash')}</MenuItem>
                            </Select>
                            <FormHelperText>
                                {t(
                                    'components.presentation.labelEmptyDisplayHelper',
                    ...
                    {isRequired && !defaultEnumValueId && (
                        <Alert severity='info'>
                            {t(
                                'components.presentation.requiredWithoutDefault',
                                'This component is required. Users must select a value before saving if default is empty.'
                            )}
                        </Alert>
                    )}
                </>
