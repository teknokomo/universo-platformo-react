    ...
    constructor({ chain, ...rest }: ChainToolInput) {
        super({
            ...
            func: async (input, runManager) => {
                ...
                const customChainHandler = runManager?.handlers.find(
                    (handler): handler is CustomChainHandler => handler instanceof CustomChainHandler
                )
                const sseStreamer = customChainHandler?.sseStreamer
                if (runManager) {
