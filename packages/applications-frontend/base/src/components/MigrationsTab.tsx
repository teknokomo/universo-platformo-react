...
export function MigrationsTab({ applicationId }: MigrationsTabProps) {
    ...
    const { data: rollbackAnalysis, isLoading: isAnalyzing } = useMigrationRollbackAnalysis(applicationId, selectedMigrationId ?? '', {
...
function MigrationRow({ applicationId, migration, isLatest, isExpanded, onToggleExpand, onRollbackClick, t }: MigrationRowProps) {
    ...
    return (
        <>
            <TableRow hover>
                ...
                <TableCell>
                    <Box display='flex' alignItems='center' gap={1}>
                        ...
                        {migration.hasDestructive && (
                            <Chip size='small' label={t('migrations.destructive', 'Destructive')} color='warning' icon={<WarningIcon />} />
                        )}
                    </Box>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                    <Chip size='small' label={`${migration.changesCount} ${t('migrations.changesCount', 'changes')}`} variant='outlined' />
                </TableCell>
...
function RollbackDialog({ open, analysis, isAnalyzing, isRollingBack, error, onConfirm, onCancel, t }: RollbackDialogProps) {
    ...
    return (
        <Dialog open={open} onClose={onCancel} maxWidth='sm' fullWidth>
                            ...
                            <DialogContentText component='div'>
                                ...
                                {hasBlockers && (
                                    <Alert severity='error' sx={{ mt: 2 }}>
                                        <Typography variant='subtitle2'>{t('migrations.cannotRollback', 'Cannot rollback')}</Typography>
                                        <List dense>
                                ...
                                {hasWarnings && !hasBlockers && (
                                    <Alert severity='warning' sx={{ mt: 2 }}>
                                        <Typography variant='subtitle2'>{t('migrations.warnings', 'Warnings')}</Typography>
                                        <List dense>
            ...
            <DialogActions>
                ...
                >
                    {isRollingBack ? t('migrations.rollingBack', 'Rolling back...') : t('migrations.rollback', 'Rollback')}
                </Button>
            </DialogActions>
        </Dialog>
    )
