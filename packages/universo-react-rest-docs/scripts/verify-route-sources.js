...
const metahubsRouterPath = path.join(repoRoot, 'packages/universo-react-metahubs-backend/src/domains/router.ts')
...
const documentedRouteFiles = new Set(
    routeSources
        ...
        .filter((filePath) => filePath.startsWith('packages/universo-react-metahubs-backend/src/domains/'))
)
...
if (missingInDocs.length > 0 || staleInDocs.length > 0) {
    const lines = [
        '[verify-route-sources] Metahubs OpenAPI routeSources are out of sync with packages/universo-react-metahubs-backend/src/domains/router.ts'
    ]
