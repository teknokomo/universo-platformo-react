...
`@universo-react/rest-docs` is a documentation-only package.
...
`@universo-react/rest-docs` derives its OpenAPI inventory from route declarations in packages such as auth, profile, start, admin, applications, and metahubs.
...
-   Run `pnpm --filter @universo-react/rest-docs generate:openapi` whenever route mounts change.
-   Run `pnpm --filter @universo-react/rest-docs validate` before shipping docs changes.
-   Run `pnpm --filter @universo-react/rest-docs build` before starting the standalone Swagger server.
