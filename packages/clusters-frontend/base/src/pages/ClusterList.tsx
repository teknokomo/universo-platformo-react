...
// Type for cluster update/create data
type ClusterData = {
    name: string
    description?: string
}
...
const ClusterList = () => {
    ...
    const [view, setView] = useViewPreference(STORAGE_KEYS.CLUSTER_DISPLAY_STYLE)

    // State management for dialog
    const [isCreating, setCreating] = useState(false)
    const [dialogError, setDialogError] = useState<string | null>(null)

    // Use paginated hook for clusters list
    const paginationResult = usePaginated<Cluster, 'name' | 'created' | 'updated'>({
        queryKeyFn: clustersQueryKeys.list,
    ...
    const handleCreateCluster = async (data: { name: string; description?: string }) => {
        setDialogError(null)
        setCreating(true)
        try {
    ...
    const clusterColumns = useMemo(
        () => [
            ...
            },
            {
                ...
                width: '10%',
                align: 'center',
                render: (row: Cluster) => (row.role ? <RoleChip role={row.role} /> : '—')
            },
            {
                id: 'domains',
                label: tc('table.domains', 'Domains'),
                width: '10%',
                align: 'center',
    ...
    const createClusterContext = useCallback(
        (baseContext: any) => ({
            ...
            helpers: {
                ...
                openDeleteDialog: (cluster: Cluster) => {
                    setDeleteDialogState({ open: true, cluster })
                }
            }
        }),
    ...
    return (
        ...
        >
            {error ? (
                ...
                <Stack flexDirection='column' sx={{ gap: 1 }}>
                    ...
                    {isLoading && clusters.length === 0 ? (
                        ...
                        <EmptyListState image={APIEmptySVG} imageAlt='No clusters' title={t('noClustersFound')} />
                    ) : (
                        <>
                            {view === 'card' ? (
                                <Box
                                ...
                                >
                                    {clusters.map((cluster: Cluster) => {
                                        ...
                                        const descriptors = clusterActions.filter((descriptor) => {
                                            if (descriptor.id === 'edit' || descriptor.id === 'delete') {
                                                return cluster.permissions?.manageCluster
                                            }
                                            return true
                                        })
                                                ...
                                                headerAction={
                                                    descriptors.length > 0 ? (
                                                        <Box onClick={(e) => e.stopPropagation()}>
                                                            <BaseEntityMenu<Cluster, ClusterData>
                                                                entity={cluster}
                                                                entityKind='cluster'
                                ...
                                <Box sx={{ mx: { xs: -1.5, md: -2 } }}>
                                    <FlowListTable
                                        ...
                                        renderActions={(row: Cluster) => {
                                            const descriptors = clusterActions.filter((descriptor) => {
                                                if (descriptor.id === 'edit' || descriptor.id === 'delete') {
                                                    return row.permissions?.manageCluster
                                                }
                                                return true
                                            })
                                            ...
                                            return (
                                                <BaseEntityMenu<Cluster, ClusterData>
                                                    ...
                                                    entityKind='cluster'
                                                    descriptors={descriptors}
                                                    // Use clusters namespace for action item labels (edit/delete)
            ...
            <ConfirmDeleteDialog
                ...
                description={t('confirmDeleteDescription', { name: deleteDialogState.cluster?.name || '' })}
                confirmButtonText={tc('actions.delete', 'Delete')}
                deletingButtonText={tc('actions.deleting', 'Deleting...')}
                cancelButtonText={tc('actions.cancel', 'Cancel')}
                onCancel={() => setDeleteDialogState({ open: false, cluster: null })}
                onConfirm={async () => {
