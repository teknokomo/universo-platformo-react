            ...
            <QueryClientProvider client={queryClient}>
                <SnackbarProvider maxSnack={3}>
                    <I18nextProvider i18n={i18n}>
                        <MemoryRouter initialEntries={['/clusters']}>{ui}</MemoryRouter>
                    </I18nextProvider>
                </SnackbarProvider>
            </QueryClientProvider>
        )
...
describe('ClusterList', () => {
    ...
    describe('Loading State', () => {
        it('should display loading skeletons while fetching data', () => {
            ...
            vi.mocked(clustersApi.listClusters).mockImplementation(() => new Promise(() => {}))

            renderWithProviders(<ClusterList />)
    ...
    describe('Error State', () => {
        it('should display error message when API fails', async () => {
            // Mock API to reject
            vi.mocked(clustersApi.listClusters).mockRejectedValue(new Error('Network error'))
    ...
    describe('Empty State', () => {
        it('should display empty state when no clusters exist', async () => {
            ...
            await waitFor(() => {
                expect(screen.getByText(/no clusters/i) || screen.getByText(/get started/i)).toBeInTheDocument()
            })
        })
    ...
    describe('Success State - List Rendering', () => {
        ...
        it('should render cluster cards with names', async () => {
            renderWithProviders(<ClusterList />)
        ...
        it('should display cluster descriptions', async () => {
            renderWithProviders(<ClusterList />)

            await waitFor(() => {
                expect(screen.getByText('First test cluster')).toBeInTheDocument()
                expect(screen.getByText('Second test cluster')).toBeInTheDocument()
            })
        })

        it('should show role chips for clusters', async () => {
            renderWithProviders(<ClusterList />)
        ...
        it('should display domains and resources counts', async () => {
            renderWithProviders(<ClusterList />)
    ...
    describe('Search Functionality', () => {
        ...
        it('should call API with search parameter when typing', async () => {
            const { user } = renderWithProviders(<ClusterList />)
            ...
            if (searchInput) {
                ...
                await waitFor(
                    () => {
                        const calls = vi.mocked(clustersApi.listClusters).mock.calls
    ...
    describe('Pagination', () => {
        const generateMockClusters = (count: number) =>
    ...
    describe('Create Cluster', () => {
        beforeEach(() => {
            ...
            vi.mocked(clustersApi.createCluster).mockResolvedValue({
                id: 'new-cluster',
                name: 'New Cluster',
    ...
    describe('Edge Cases', () => {
        it('should handle clusters without descriptions', async () => {
            vi.mocked(clustersApi.listClusters).mockResolvedValue({
                data: [
                    {
                        id: 'no-desc',
        ...
        it('should handle clusters without role', async () => {
            vi.mocked(clustersApi.listClusters).mockResolvedValue({
                data: [
                    {
                        id: 'no-role',
        ...
        it('should handle clusters with zero counts', async () => {
            vi.mocked(clustersApi.listClusters).mockResolvedValue({
                data: [
                    {
                        id: 'zero-counts',
