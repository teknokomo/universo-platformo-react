...
- **File-Specific Focus:** Maintain each file's intended purpose and avoid mixing contexts:
    ...
    - **`projectbrief.md`:** High-level overview of the project's mission, scope, and strategic goals. It is the foundation and source of truth for requirements. Must start with a `Last Reviewed: YYYY-MM-DD` marker. _(Stable content; update through the Canon Refresh phase, not via routine compression.)_
    - **`productContext.md`:** Explanation of **why** the project exists, problems it solves, target use cases, and desired user experience. Must start with a `Last Reviewed: YYYY-MM-DD` marker. _(Background and rationale; update through the Canon Refresh phase when product direction changes.)_
    ...
    - **`systemPatterns.md`:** Key architecture and design patterns, system structure, and important technical decisions. Document recurring patterns or templates, component relationships, and coding guidelines used in the project. Patterns intended to be preserved through compression must carry the `CRITICAL` marker in their heading. _(Update when significant design decisions or patterns emerge.)_
    - **`techContext.md`:** Technical stack and context — frameworks, libraries, environment setup, constraints, integration points, and any critical technical details or custom modifications that must be preserved. Must start with a `Last Reviewed: YYYY-MM-DD` marker. _(Update through the Canon Refresh phase when the tech stack or platform-level baseline changes.)_
    ...
    - **`currentResearch.md`:** A scratchpad for ongoing deep research findings from external analyses (other AI agents or sources). Keep it in the **600-700 line** target range. Preserve all active/unresolved research regardless of age and the last 3 months in full detail; condense 3-6 month material by about 50% and >6 month resolved history by about 90%, while retaining durable conclusions and links to dedicated `memory-bank/research/` artifacts. Avoid duplicating completed implementation evidence already captured in `progress.md`/`tasks.md`. Keep it English-only, newest-first, summarized, and relevant.

- **Last Reviewed Marker On Canon Files:** `projectbrief.md`, `productContext.md`, and `techContext.md` must each start with a `Last Reviewed: YYYY-MM-DD` blockquote marker plus a short note on what was refreshed. The MB-mode Canon Refresh phase scans this marker to decide whether canon is stale. A marker older than 60 days is a freshness signal, not yet a defect; older than 90 days without an explicit "no change" note is a defect.

- **Canon Refresh Awareness:** When the project undergoes a meaningful structural change (package added or removed, terminology rename, architectural pivot, version bump on the root `package.json`), the canon files (`projectbrief.md`, `productContext.md`, `techContext.md`) must be refreshed in the same session. Do not let canon drift while editing `tasks.md` and `progress.md`. The Canon Refresh phase of MB mode codifies this; routine edits should respect the same intent.
...
- **Avoid Redundancy:** Minimize duplication of information across files. If information is already documented in one memory file, **reference it** in another file rather than duplicating it verbatim. For example, if `projectbrief.md` or `systemPatterns.md` contains a detailed table of core components, other files can briefly mention the fact or just highlight changes, and link or refer to the detailed source:
    ...
    - _Allowed:_ Briefly restating a crucial point for context or linking to another file (e.g., "Active platform-level entity type presets are listed in **projectbrief.md**.").
...
- **Token-Efficient Writing:** Memory bank content is consumed by AI agents. Apply token-efficient patterns when writing or updating files: drop filler phrases ("This is used to", "It is important to note"), drop unnecessary articles in technical bullet lists, prefer compact structures (`{a,b,c}` over `{ a, b, c }`), use `snake_case` for identifiers, and inline constraints next to fields. Detailed guidance lives in the memory-bank compression instructions; the same principles apply to routine edits.
