...
1. КОГДА реорганизуются компоненты шаблона ТОГДА папка компонентов dashboard ДОЛЖНА быть перемещена из `apps/universo-template-mui/base/src/views/dashboard/components` в `apps/universo-template-mui/base/src/components/dashboard`
...
3. КОГДА реструктурируются файлы темы ТОГДА папка theme ДОЛЖНА быть перемещена из `apps/universo-template-mui/base/src/views/dashboard/theme` в `apps/universo-template-mui/base/src/theme`
4. КОГДА организуются общие компоненты ТОГДА папка internals ДОЛЖНА быть правильно перемещена и организована
5. КОГДА перемещаются компоненты auth ТОГДА папка `apps/universo-template-mui/base/src/views/auth` ДОЛЖНА быть перенесена в соответствующее место
6. КОГДА организуется папка shared ТОГДА `apps/universo-template-mui/base/src/views/shared` ДОЛЖНА быть правильно размещена для общих файлов шаблонов
...
5. КОГДА организуются компоненты карточек ТОГДА они ДОЛЖНЫ быть размещены в `apps/universo-template-mui/base/src/components/cards/`
...
1. КОГДА очищаются компоненты ТОГДА вся папка `apps/universo-template-mui/base/src/components/universal` ДОЛЖНА быть удалена
2. КОГДА удаляются тестовые компоненты ТОГДА `apps/universo-template-mui/base/src/components/EntityCard.tsx` ДОЛЖЕН быть удален
...
1. КОГДА интегрируется с основными маршрутами ТОГДА подключение ДОЛЖНО быть сделано напрямую в `packages/ui/src/App.jsx` или правильно в `packages/ui/src/routes/index.jsx`
...
**Пользовательская история:** Как разработчик, я хочу правильно реализовать подход тонкого слоя абстракции, чтобы `apps/universo-templates/` служил простым переключателем тем, а не сложной абстракцией.

#### Критерии приемки

1. КОГДА реализуется слой абстракции ТОГДА `apps/universo-templates/` ДОЛЖЕН быть тонким переключателем, который определяет активный UI пакет
...
4. КОГДА интегрируется ТОГДА основная реализация ДОЛЖНА быть сначала в `apps/universo-template-mui/`, с добавлением абстракции позже
...
2. КОГДА реализуется новая функциональность ТОГДА она ДОЛЖНА быть добавлена в отдельные пакеты в `apps/`
...
1. КОГДА исправляется проблема с `packages/ui/src/routes/index.jsx` ТОГДА весь мусорный/лишний код ДОЛЖЕН быть удален и заменен простым подключением новой структуры
2. КОГДА исправляется интеграция ТОГДА подключение ДОЛЖНО быть сделано правильно в `packages/ui/src/App.jsx` или нормально в `packages/ui/src/routes/index.jsx`
3. КОГДА удаляются промежуточные итерации ТОГДА все компоненты в `packages/ui/src/ui-component`, которые были доработаны для нового дизайна, ДОЛЖНЫ быть проанализированы на предмет удаления
