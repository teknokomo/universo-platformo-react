...
**User Story:** As a developer, I want the quiz functionality to be extracted from the publish-frontend package into a separate template-quiz package, so that the architecture follows the same modular pattern as template-mmoomm.
...
2. WHEN the basic package structure is created THEN the existing working code SHALL be copied from `packages/publish-frontend/base/src/builders/templates/quiz` to preserve functionality and avoid rewriting from scratch
...
5. WHEN the extraction is complete THEN `packages/publish-frontend` SHALL import and use the quiz template from `@universo/template-quiz` package
6. WHEN the integration is verified and working THEN the original quiz code in `packages/publish-frontend/base/src/builders/templates/quiz` SHALL be removed
