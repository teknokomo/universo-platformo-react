...
Данная спецификация описывает масштабный рефакторинг функционала Chatflow в систему Spaces (Пространств) с поддержкой множественных холстов (Canvas). Проект основан на Flowise 2.2.8 с значительными доработками в изолированных приложениях папки `apps/` и модификациями основного функционала в `packages/`. 
...
1. КОГДА выполняется переименование таблицы, ТО таблица `chat_flow` из миграции `packages/server/src/database/migrations/postgres/1693891895163-Init.ts` ДОЛЖНА быть переименована в `spaces`
2. КОГДА принимается решение о структуре, ТО система ДОЛЖНА либо создать новое приложение `apps/spaces-srv` (выглядит как приоритетный вариант, но требует изучения), либо использовать новую миграцию в `apps/space-builder-srv`
3. ЕСЛИ создается `apps/spaces-srv`, ТО функционал Chatflow ДОЛЖЕН постепенно переноситься из основного бэкенда Flowise в изолированное приложение, на данный момент нужно реализовать MVP этого функционала.
4. КОГДА создаются новые entities (Spaces, Canvas), ТО они ДОЛЖНЫ быть правильно зарегистрированы в центральном реестре `packages/server/src/database/entities/index.ts`
5. ЕСЛИ используется подход с новой миграцией в `apps/space-builder-srv`, ТО миграция ДОЛЖНА быть создана по аналогии с `apps/metaverse-srv/base/src/database`
...
1. КОГДА система публикации в `apps/publish-frt` и `apps/publish-srv` формирует ссылку, ТО она ДОЛЖНА использовать UUID Canvas вместо UUID Chatflow
