---
description: How to use the metahub resources workspace as the real entrypoint for layouts, reusable metadata resources, and shared library modules.
---

# Resources Workspace

The Resources page is the real authoring entrypoint for layouts, reusable metadata resources, and shared library modules in a metahub.
...
-   resources/library modules that expose reusable `@shared/<codename>` helpers;
...
3. Switch between Layouts, Components, Constants, Values, and Modules depending on the shared asset you need.
...
## Shared Modules Workflow

1. Open Resources -> Modules to author reusable library helpers.
2. Import those helpers from consumer modules through `@shared/<codename>`.
...
-   Resources -> Modules accepts only reusable library authoring, so shared helpers cannot be created as widget, module, or lifecycle drafts;
-   shared library delete or codename changes fail closed while consumer modules still import `@shared/<codename>`;
