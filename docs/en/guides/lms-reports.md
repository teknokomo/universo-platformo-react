---
description: Catalog-backed LMS report definitions and safe runtime execution.
---
...
They send exactly one saved report reference, either `reportId` or `reportCodename`, and the backend loads the JSON `Definition` from the published `Reports` Catalog in the current workspace.
...
Registrar-only ledger Catalogs are excluded from report target discovery, so reports operate on ordinary runtime record Catalogs and not on internal fact ledgers.
