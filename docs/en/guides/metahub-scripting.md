...
| Imports | `@universo/extension-sdk` is always allowed; consumer scripts may also import Common libraries through `@shared/<codename>`. |
| Roles | `module`, `lifecycle`, `widget`, and `library`. |
| Attachment scopes | `general` in Common for `library`, plus metahub, hub, catalog, set, enumeration, and attribute for executable consumers. |
...
1. Open the Scripts tab in Common for shared libraries, or on the metahub / specific attached entity for consumers.
