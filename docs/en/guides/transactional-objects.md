...
1. `beforePost` lifecycle modules run inside the active transaction.
2. Modules may return declarative posting movements for Ledgers declared in `recordBehavior.posting.targetLedgers`.
...
-   Posting modules can use `ctx.ledger` only with declared capabilities.
...
Those objects are still ordinary Object entities; LMS behavior comes from metadata, modules, layouts, and Ledgers rather than a separate LMS runtime fork.
