...
Add the following variables to `packages/flowise-core-backend/base/.env` (or provide them via CLI flags):
...
When running from source, add the variables to `packages/flowise-core-backend/base/.env`:
