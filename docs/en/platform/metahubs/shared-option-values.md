...
Shared option values live in the Shared Option Values tab of the Resources workspace and belong to the virtual shared enumeration pool instead of one enumeration.
They let one option value definition stay reusable while multiple enumerations inherit the same source row.

## Design-Time Rules

- Create the option value from the Shared Option Values tab when the same meaning must appear in more than one enumeration.
- Keep shared behavior on the shared row and sparse target changes in override rows.
- Use target enumerations to inspect inherited state, but keep the base shared authoring flow in the Shared Option Values tab.
- Use local enumeration values only when one enumeration needs a value that others must not inherit.

## Target Controls

- Exclusions hide the inherited option value from selected enumerations without deleting the shared source.
...
Runtime normalizes duplicated inherited option value ids per target enumeration so runtime metadata and seeded refs stay deterministic.
