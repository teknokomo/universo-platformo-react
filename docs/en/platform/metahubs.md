...
Metahubs are the design-time source of truth for structures, resources-workspace assets, modules, and publication-ready content.
...
-   the dedicated resources workspace, including shared layouts, reusable metadata pools, and reusable modules;
-   modules attached at resources, metahub, or entity scope;
...
| Surface      | Purpose                                                                                              |
| ------------ | ---------------------------------------------------------------------------------------------------- |
| Resources    | Author shared layouts, reusable metadata resources, and reusable modules from one dedicated surface. |
...
Shared layouts stay centralized, reusable metadata pools stay reusable, and resources modules keep the shared `@shared/<codename>` import contract.
...
1. Author the structure, shared layouts, reusable metadata resources, and modules in the metahub.
...
The shared library helper is authored in Resources, the widget consumer module is authored at metahub scope, the widget placement is configured in the shared layout model, and the resulting publication carries all three pieces into the linked application.
...
1. Read [Metahub Modules](../guides/metahub-modules.md) for the module contract.
