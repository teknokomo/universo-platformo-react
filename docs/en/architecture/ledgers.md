...
Runtime Object rows remain the user-editable operational surface, while Ledgers are intended for posting, reporting, projections, and module-controlled fact append flows.
...
Modules may request `posting`, `ledger.read`, and `ledger.write` capabilities.
Capability checks remain fail-closed: a module without the required capability must not read or write Ledger facts.
Runtime modules receive `this.ctx.ledger` with `list`, `facts`, `query`, `append`, and `reverse` methods only through those capability checks.
