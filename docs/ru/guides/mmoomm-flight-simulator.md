...
двумя аутентифицированными пользователями с правом редактирования контента. Оба
пользователя подключаются к одной Colyseus-комнате опубликованного приложения.
Сервер владеет позициями кораблей, выдаёт по одному стабильному кораблю на
пользователя, принимает только намерения движения (`move_to_point`,
`move_to_object`, `stop`) и ищет свободную spawn-позицию, если точка по
