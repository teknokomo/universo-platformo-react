...
Это не захардкоженный вертикальный модуль внутри `packages/apps-template-mui`, а поставляемая демо-среда приходит из канонического двуязычного fixture Orbital Academy.
...
3. Общий MUI-шаблон предоставляет переключение рабочих пространств, рендеринг QR-кодов, просмотр модулей и виджеты статистики.
4. Коммитнутые генератор и контракт снимка поставляют более богатый EN/RU-набор данных Orbital Academy с несколькими классами, модулями, тестами, заполненным прогрессом и двумя маршрутами гостевого доступа.
...
Если у приложения включены рабочие пространства, публичный рантайм привязывает данные к рабочему пространству ссылки доступа или текущей гостевой сессии, а не к произвольному активному рабочему пространству.
...
Поставляемый LMS-набор браузерных тестов покрывает управление рабочими пространствами, негативные сценарии публичных ссылок, раздачу через QR-код, виджет статистики, EN guest journey и RU guest route с локализованными текстами модулей, тестов и ссылок доступа.
