---
description: Catalog-backed определения отчётов LMS и безопасное runtime-выполнение.
---

# Отчёты LMS

![Записи Catalog, используемые определениями отчётов LMS](../.gitbook/assets/entities/catalog-records.png)

Отчёты LMS являются конфигурационными записями в существующем Catalog `Reports`.
...
Они передают ровно одну ссылку на сохранённый отчёт: `reportId` или `reportCodename`, а backend загружает JSON `Definition` из опубликованного Catalog `Reports` в текущем workspace.
...
Registrar-only ledger Catalogs исключаются из поиска target для отчётов, поэтому отчёты работают с обычными runtime record Catalogs, а не с внутренними fact ledgers.
