---
description: Как использовать рабочее пространство ресурсов в метахабе как реальную точку входа для макетов, переиспользуемых ресурсов метаданных и общих библиотечных модулей.
---

# Рабочее пространство ресурсов

Страница Ресурсов является реальной точкой входа для настройки макетов, переиспользуемых ресурсов метаданных и общих библиотечных модулей внутри метахаба.
...
-   модули resources/library, которые отдают переиспользуемые helpers через `@shared/<codename>`;
...
3. Переключайтесь между вкладками «Макеты», «Компоненты», «Константы», «Значения» и «Модули» в зависимости от нужного общего ресурса.
...
## Поток работы с общими модулями

1. Откройте «Ресурсы» -> «Модули», чтобы писать переиспользуемые библиотечные helper-модули.
2. Импортируйте эти helpers из потребляющих модулей через `@shared/<codename>`.
...
-   «Ресурсы» -> «Модули» принимает только проектирование переиспользуемых библиотек, поэтому общие helper-модули нельзя создавать как черновики с ролями widget, module или lifecycle;
-   удаление общей библиотеки или смена её `codename` завершаются закрыто, пока потребляющие модули всё ещё импортируют `@shared/<codename>`;
