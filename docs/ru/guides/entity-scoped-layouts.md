...
## Разреженные настройки зон

Overlay, привязанный к сущности, может также хранить разреженные настройки зоны,
которой владеет тот же шаблон. В нём записываются только явно заданные значения,
поэтому последующее изменение глобальной базы остаётся видимым, пока overlay
сущности намеренно не переопределит эту настройку. Reset одной настройки удаляет
этот ключ и показывает актуальное унаследованное значение, не меняя размещение
виджета и другие настройки.

Общая поверхность `LayoutAuthoringDetails` группирует эти действия с теми же
метаданными размещения, которые используются общими макетами. Backend проверяет
область, права и версию макета до сохранения, поэтому клиент не может записать
настройку в макет другой сущности или
молча перезаписать более новую ревизию.
