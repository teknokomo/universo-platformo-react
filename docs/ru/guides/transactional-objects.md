...
1. Lifecycle-модули `beforePost` выполняются внутри активной транзакции.
2. Модули могут вернуть declarative posting movements только для Регистров, объявленных в `recordBehavior.posting.targetLedgers`.
...
-   Posting modules могут использовать `ctx.ledger` только с объявленными capabilities.
...
Эти объекты остаются обычными Object entities; LMS-поведение задаётся metadata, modules, layouts и Регистрами, а не отдельным LMS runtime fork.
