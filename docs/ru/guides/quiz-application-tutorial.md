---
description: Руководство по созданию приложения-квиза через браузерный сценарий со модулями метахаба, настройкой макета, параметров приложения и проверкой рантайма.
---
...
В конце у вас будет модуль виджета квиза в метахабе, макет с этим виджетом, связанное приложение и проверенное выполнение квиза.

Приложение-квиз намеренно остаётся независимым от шаблона LMS.
В LMS MVP тот же виджет квиза может встраиваться из элемента учебного контента, но сохранённая fixture квиза и это руководство по-прежнему описывают отдельный сценарий квиза на модулях.
...
-   Учитывайте текущий контракт модулей: поддерживаются только встроенные модули и `sdkApiVersion = 1.0.0`.
...
## Шаг 2: Создайте модуль виджета квиза

1. Откройте диалог редактирования метахаба и переключитесь на вкладку Modules.
2. Создайте новый модуль с ролью `widget`.
3. Используйте `quiz-widget` как codename.
4. Оставьте тип источника значением Embedded.
5. Сохраните модуль после вставки исходника виджета квиза или после адаптации стартового шаблона.

![Вкладка модулей metahub](../.gitbook/assets/quiz-tutorial/metahub-modules.png)

Сценарий Playwright в браузере использует этот шаг, чтобы подтвердить, что модуль квиза виден в реальной области Modules на уровне метахаба.
...
3. Установите `moduleCodename` в `quiz-widget`.
...
Именно в этой точке данные публикации, макета и бандлы модулей переходят со стороны метахаба на сторону приложения.
...
Текущий стартовый шаблон ожидает клиентский метод `mount()` и серверный метод `submit()`, поэтому рабочий рантайм одновременно подтверждает и клиентский, и серверный путь выполнения модуля.

## Диагностика проблем

-   Если модуль сохраняется, но не появляется в рантайме, заново опубликуйте метахаб и пересинхронизируйте приложение.
-   Если виджет рендерится без данных, проверьте, что `moduleCodename` виджета в макете точно совпадает с сохранённым кодом модуля.
-   Если вызовы рантайм RPC падают, проверьте, что роль модуля и возможность по-прежнему открывают клиентский RPC-путь.
...
-   [Модули метахаба](metahub-modules.md)
