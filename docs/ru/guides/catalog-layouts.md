description: Как catalog-specific layout наследуются от global layout, хранят sparse overrides для visibility и placement и управляют catalog runtime behavior.
---
...
2. Перейдите в General -> Layouts для глобального контекста или откройте route catalog, который ведёт в его список layout.
...
Пока первый layout catalog не создан, runtime продолжает использовать fallback runtime settings самого catalog.
