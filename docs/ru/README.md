...
- `packages/spaces-backend` — бэкенд для Spaces/Canvases (сущности TypeORM, эндпоинт reorder)
- `packages/spaces-frontend` — фронтенд с вкладками холстов, drag & drop и изолированным HTTP‑клиентом

Ключевые изменения:
- В UI список Пространств теперь загружается из `packages/spaces-frontend`.
- Маршруты Canvas обрабатываются под MinimalLayout через `packages/spaces-frontend/base/src/entry/CanvasRoutes.jsx`, поэтому на экране холста не отображается левое меню.
...
- `/packages/flowise-core-frontend/base/src/shims/globalRequire.ts` сопоставляет устаревшие вызовы CommonJS `require('react')` их ESM-эквивалентам, поэтому такие зависимости, как `prop-types`, больше не падают.
- `/packages/flowise-core-frontend/base/src/shims/useSyncExternalStoreShim.ts` и `useSyncExternalStoreShimWithSelector.ts` реализуют хуки React 18 `useSyncExternalStore` без опоры на CommonJS-бандлы, сохраняя стабильность `react-redux` во время сборки Vite.
- Оба шима импортируются из `packages/flowise-core-frontend/base/src/index.jsx` до рендера приложения, что предотвращает белые экраны из-за отсутствующих CommonJS-хелперов.
