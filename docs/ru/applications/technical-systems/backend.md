# `world-coordinator-backend` — Координатор шардов и миграций — [Статус: Планируется]
...
Интеграции: `multiplayer-backend` (кластер, presence/pubsub), LB/Ingress (sticky WS), `auth-backend` (подпись токенов), `entities-backend` (сериализация минимального состояния).
