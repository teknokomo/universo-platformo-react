...
-   **UI публикации**: Предоставляет пользовательский интерфейс для выбора шаблонов, конфигурирования опций и управления публичными ссылками.
-   **Потребитель шаблонов**: Динамически загружает и использует пакеты шаблонов для генерации опыта.
-   **Модульная архитектура**: Потребляет выделенные пакеты шаблонов (например, `@universo/template-mmoomm`) для специфичной функциональности.
...
Система конструкторов использует **модульную архитектуру на основе шаблонов**, организованную сначала по шаблону, затем по технологии. Система теперь поддерживает как внутренние шаблоны, так и внешние пакеты шаблонов:
...
│     └── handlers/          # Процессоры для викторин
```
...
├── src/playcanvas/
│   ├── builders/            # Конструкторы шаблонов
