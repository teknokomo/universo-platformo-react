...
-   Сервер (`multiplayer-backend`): авторитетный тик-цикл, валидация интентов, расчёт урона/коллизий, рассылка снапшотов
-   Долговременное состояние: Supabase (PostgreSQL); кэш/мастеринг комнат: Redis
-   Доменные сервисы: `entities-backend`, `ships-backend`, `combat-backend`, `economy-backend`, … вызываются из серверной логики
