...
Flowise поддерживает различные переменные окружения для настройки вашего экземпляра. Вы можете указать следующие переменные в файле `.env` внутри папки `packages/server`. Обратитесь к файлу [.env.example](https://github.com/FlowiseAI/Flowise/blob/main/packages/server/.env.example).
...
| LOG\_PATH  | Местоположение хранения файлов логов | String                                           | `Flowise/packages/server/logs` |
...
| SECRETKEY\_PATH               | Локальный путь к файлу, где сохраняется ключ шифрования   | String                      | `Flowise/packages/server` |
