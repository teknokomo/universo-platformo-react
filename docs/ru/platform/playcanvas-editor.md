...
-   **v2.30.4 picker рендерится пустым после обновления**: убедитесь, что `playcanvas/editor@v2.30.4` в bundle и присутствует orchestrator `picker-version-control.ts`. `picker-conflict-manager` тихо no-op-ит, если editor-метод `picker:versioncontrol:hasRetainedDiff` отсутствует — проверьте через `editor.call('picker:versioncontrol:hasRetainedDiff', 'test')` в browser console (должен вернуть boolean, не `undefined`).
...
-   [Тип сущности «Проекты»](./projects-entity-type.md) — пресет типа сущности, владелец связи между экземпляром метахаба и проектом авторинга.
-   [Шаблон метахаба «PlayCanvas»](./playcanvas-template.md) — шаблон, включающий пресет «Проекты».
