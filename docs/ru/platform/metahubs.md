---
description: Объясняет, чем владеют metahubs, как устроены Common section и уровни настроек, и как publications питают applications.
---
...
- разделом Common, включая глобальные layout и catalog-specific layout overlays;
...
| Surface | Purpose |
| --- | --- |
| Common | Позволяет из одной tabbed surface настраивать глобальные layout, catalog-specific layout variants и общее view behavior. |
...
## Область действия Common section

Реальная точка входа для authoring layout теперь находится на tabbed page Common, а legacy ссылки `/layouts` перенаправляются туда.
...
1. Создайте структуру, Common layouts и script-ы в metahub.
...
Скрипт виджета создаётся в metahub, размещение виджета настраивается в модели layout раздела Common, а итоговая publication переносит обе части в связанное application.
Пока у catalog нет первого custom layout, runtime behavior берётся из выбранного global layout; после этого выбранный layout catalog начинает владеть поведением runtime для create/edit/copy surface.
