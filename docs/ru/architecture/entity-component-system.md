...
- Публиковать entity-based разделы, такие как Catalogs v2, через metadata, а не через хардкод меню.
...
- Legacy Catalogs, Sets и Enumerations остаются видимыми до принятия parity.
- Catalogs v2 опирается на те же authoring primitives вместо второго policy layer.
- Browser validation должна покрывать и design-time workspace, и publication/runtime path.
- Advanced visual builder tooling остаётся будущей задачей после закрытия parity.
