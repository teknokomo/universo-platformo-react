...
Блок `config.ledger.fieldRoles` классифицирует эти поля как dimensions, resources, measures, components, period fields, source references или workspace scope.
...
Операционные факты регистров не являются обычными записями Object и по умолчанию не экспортируются как seeded runtime rows.

Объекты также могут хранить `config.recordBehavior`.
...
Runtime Object rows остаются пользовательской операционной поверхностью, а Регистры предназначены для posting, reporting, projections и module-controlled fact append flows.

Транзакционное поведение Объектов принадлежит платформе.
Генерация runtime-схемы добавляет колонки `_app_record_*` и `_app_post*` для Объектов с включенным `recordBehavior`, а также `_app_record_counters` для атомарной нумерации.
API команд записей:
...
Модули могут запрашивать capabilities `posting`, `ledger.read` и `ledger.write`.
Проверки capabilities остаются fail-closed: модуль без нужного capability не должен читать или записывать факты регистра.
Runtime-модули получают `this.ctx.ledger` с методами `list`, `facts`, `query`, `append` и `reverse` только через эти проверки capabilities.
...
Канонический LMS template определяет `ProgressLedger`, `ScoreLedger`, `EnrollmentLedger`, `AttendanceLedger`, `CertificateLedger`, `PointsLedger` и `NotificationLedger`.
Они моделируют факты прогресса, оценивания, назначений, посещаемости, сертификатов, геймификации и уведомлений, сохраняя LMS как конфигурацию, а не runtime fork.
