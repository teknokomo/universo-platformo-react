...
- Embedded-модули могут импортировать `@universo/extension-sdk`, а потребляющие модули могут дополнительно импортировать библиотеки рабочего пространства ресурсов через `@shared/<codename>`; остальные static imports, `require()`, dynamic `import()` и `import.meta` завершают компиляцию с ошибкой.
...
- `@universo/modules-engine` отвечает за компиляцию, разделение бандлов, пул isolate-ов и мониторинг здоровья.
