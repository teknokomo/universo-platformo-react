...
| `LearningResources` | Самостоятельные страницы, структурированные гостевые уроки, ссылки и другие авторские ресурсы |
| `Courses`           | Последовательные курсы из разделов и элементов курса                                          |
| `CourseSections`    | Разделы структуры курса с сохраняемым порядком                                                |
| `CourseItems`       | Элементы курса, привязанные к родительскому курсу, с metadata завершения и доступности        |
| `LearningTracks`    | Учебные траектории из нескольких курсов                                                       |
| `TrackStages`       | Этапы учебных траекторий с сохраняемым порядком                                               |
| `TrackSteps`        | Шаги траектории, привязанные к курсам или ресурсам                                            |
| `Quizzes`           | Определения тестов с таблицами вопросов                                                       |
| `QuizResponses`     | Сохранение ответов по каждому вопросу                                                         |
| `ContentProgress`   | Прогресс учащегося по Learning Resources и публичному гостевому потоку                        |
| `AccessLinks`       | Записи для маршрутизации гостевого доступа                                                    |
| `Enrollments`       | Полиморфные назначения ресурсов, курсов и траекторий                                          |

## Операционные регистры

| Регистр          | Назначение                                                                                       |
| ---------------- | ------------------------------------------------------------------------------------------------ |
| `ProgressLedger` | Append-oriented движения учебного прогресса по учащемуся, учебному элементу, workspace и попытке |
...
-   `TABLE`-поля используются для структурированных content items Learning Resource и вопросов теста.
...
-   Транзакционные LMS-объекты используют общую вкладку Object `behavior` для нумерации, effective dates, lifecycle states, posting targets и posting modules. LMS fixture хранит эти настройки в `config.recordBehavior`.
...
Платформа по-прежнему поддерживает module-backed widgets и QR widgets как общие возможности, но LMS fixture не привязывает глобальные content/statistics/QR widgets в default application layout.
LMS-специфическое поведение задаётся конфигурацией metahub, данными сущностей, модулями на релевантной metadata surface и публичными runtime-ссылками.
