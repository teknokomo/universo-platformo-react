...
-   Every user-facing string introduced with a feature must have `en` and `ru` entries before the feature is considered complete. Labels shared by layout authoring surfaces belong to `@universo-react/i18n` under the common namespace; feature bundles must not duplicate those keys. Consumer tests must load the real locale resources at least once, and browser tests must exercise every supported authoring locale used by the feature.
-   Reuse existing MUI dashboard/app-template primitives before creating new widgets or layouts.
-   Settings and CRUD dialogs must use the shared `StandardDialog` (or the canonical specialized dialog built on it). Their action footer must preserve the shared padding and keyboard/focus behavior; a raw MUI `DialogActions` is allowed only when the shared primitive cannot represent the required interaction and the reason is documented with a focused test.
...
-   When a template is based on a checked-in upstream/reference implementation, preserve its visible geometry, background ownership, and fixed/flow behavior. A refactor must include a browser assertion or screenshot comparison for the reference-sensitive parts; unit tests alone do not establish visual parity.
