...
export class HTMLDocumentGenerator implements IHTMLDocumentGenerator {
    ...
    generateDocument(
        ...
        // Check for additional HTML (like auth screen for multiplayer)
        const additionalHTML = (options as any).additionalHTML || ''
...
${embeddedJavaScript}
    ...
    private getLibrarySourcesForTemplate(options: BuildOptions): string[] {
        // Default PlayCanvas library
        const defaults = ['https://cdn.jsdelivr.net/npm/playcanvas@2.9.0/build/playcanvas.min.js']
        // Allow builders to inject additional libraries (e.g., Colyseus client)
        const extra = Array.isArray((options as any).librarySources) ? (options as any).librarySources : []
        // Merge unique while preserving order (defaults first)
        const merged: string[] = []
        for (const src of defaults.concat(extra)) {
