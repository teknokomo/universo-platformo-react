...
└─ src/
   ...
   │  │        └─ ...
   │  ├─ playcanvas/         # Билдер для PlayCanvas
   │  │  ├─ PlayCanvasBuilder.ts     # Основной класс-билдер PlayCanvas
   │  │  └─ templates/             # Реализации шаблонов для PlayCanvas
   │  │     └─ mmoomm/             # Шаблон Universo MMOOMM
...
-   **`Обработчики (Handlers)`**: Специализированные процессоры для различных типов узлов UPDL теперь инкапсулированы внутри каждого шаблона. Это делает каждый шаблон самодостаточным.

#### Возможности

-   **Максимальная расширяемость**: Легко добавлять новые целевые платформы, создавая новый класс шаблона.
-   **Четкое разделение ответственности**: Высокоуровневые билдеры просты и только управляют процессом, в то время как шаблоны содержат все специфические детали реализации.
