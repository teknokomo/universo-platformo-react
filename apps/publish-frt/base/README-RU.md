...
└─ src/
   ...
   │  └─ libs/             # Локальные библиотеки AR.js и A-Frame для регионов с блокировкой CDN
   ...
   │  │  ├─ AbstractTemplateBuilder.ts # NEW: Абстрактный базовый класс для всех шаблонов
   │  │  ├─ TemplateRegistry.ts       # NEW: Реестр для управления шаблонами
   ...
   │  │  ├─ ARJSBuilder.ts         # Основной класс-билдер AR.js (делегирует шаблонам)
   │  │  └─ templates/             # NEW: Реализации для конкретных шаблонов
   │  │     └─ quiz/               # Шаблон квиза для AR.js
   │  │        ├─ ARJSQuizBuilder.ts   # Билдер для шаблона квиза
   │  │        ├─ handlers/            # Обработчики, специфичные для шаблона квиза
   │  │        │  ├─ ObjectHandler.ts
   │  │        │  └─ ... (другие обработчики)
   │  │        └─ index.ts
   ...
   │  └─ public/           # Публичные страницы (ARViewPage)
   ├─ routes/              # Конфигурация маршрутизации
   ├─ i18n/                # Локализация
   ├─ services/            # Сервисный слой для взаимодействия с бэкендом
...
## Критичная архитектура: рендеринг AR.js в Iframe

**ВАЖНО**: Контент AR.js должен рендериться с использованием iframe для корректной загрузки библиотек и выполнения скриптов.

### Почему Iframe необходим

Библиотеки AR.js (A-Frame и AR.js) требуют правильного контекста выполнения скриптов, который `dangerouslySetInnerHTML` в React предоставить не может:

-   **Изоляция скриптов**: Iframe создает изолированный контекст выполнения для скриптов AR.js.
-   **Загрузка библиотек**: Обеспечивает корректную загрузку внешних/локальных JavaScript-библиотек.
-   **Совместимость с браузером**: Предотвращает конфликты с виртуальным DOM React.
-   **Безопасность**: Изолирует код AR.js от основного контекста приложения.

### Паттерн реализации (ARViewPage.tsx)
...
iframeDoc.open()
iframeDoc.write(html) // HTML AR.js с тегами <script>
iframeDoc.close()
```

### Интеграция статических библиотек

Фронтенд работает с локальными библиотеками AR.js, которые раздаются непосредственно основным сервером Flowise:
...
-   **`ARJSBuilder`**: Высокоуровневый билдер, который теперь выступает в роли контроллера. Он определяет требуемый шаблон и делегирует весь процесс сборки соответствующему билдеру шаблона из реестра.
-   **`ARJSQuizBuilder`**: Конкретная реализация шаблона для генерации HTML-квизов AR.js. Он содержит собственный набор `обработчиков` для обработки различных узлов UPDL.
-   **`Обработчики (Handlers)`**: Специализированные процессоры для различных типов узлов UPDL (Space, Object, Camera, Light) теперь инкапсулированы внутри каждого шаблона (например, `apps/.../arjs/templates/quiz/handlers/`). Это делает каждый шаблон самодостаточным.

#### Возможности

-   **Максимальная расширяемость**: Легко добавлять новые целевые платформы (например, PlayCanvas), создавая новый класс шаблона, который наследует `AbstractTemplateBuilder`.
-   **Четкое разделение ответственности**: Высокоуровневый `ARJSBuilder` прост и только управляет процессом, в то время как шаблоны содержат все специфические детали реализации.
...
-   **Общая функциональность**: Общая логика, такая как разрешение источников библиотек и обертка HTML-документа, обрабатывается абстрактным базовым классом, что сокращает дублирование кода.
-   **Готовность к будущему**: Архитектура подготовлена к интеграции `Universo MMOOMM` с выделенным шаблоном для PlayCanvas.
...
1.  **Запрос с фронтенда**: Пользователь инициирует публикацию через компонент `ARJSPublisher`.
2.  **Вызов API**: Фронтенд отправляет запрос на эндпоинт `/api/v1/publish/arjs`.
...
5.  **Генерация билдером**: **`ARJSBuilder` делегирует процесс сборки зарегистрированному билдеру шаблона (например, `ARJSQuizBuilder`), который конвертирует пространство UPDL в элементы A-Frame.**
...
-   **Конфигурация для конкретной технологии**: Настройки AR.js хранятся в блоке `arjs` внутри `chatbotConfig`, сохраняя разделение с настройками чат-бота.
...
-   **Структура для нескольких технологий**: Настройки хранятся в поле `chatbotConfig` со структурой `{"chatbot": {...}, "arjs": {...}}`.
-   **Независимые состояния публикации**: Каждая технология (чат-бот, AR.js) имеет свой собственный флаг `isPublic`.
...
-   `UPDLProcessor` - Центральный класс для обработки потоков UPDL (мигрировал с бэкенда).
-   `ARJSPublisher` - Компонент для потоковой публикации проектов AR.js с интеграцией Supabase.
-   `ARJSExporter` - Демонстрационный компонент для экспорта кода AR.js.
-   `ARViewPage` - Компонент страницы для просмотра AR-пространства с использованием iframe.
...
-   **`PublicationApi`** - Базовый класс для функциональности публикации для всех технологий.
...
-   **Поддержка нескольких технологий**: API публикации разработано для поддержки AR.js, Chatbot и будущих технологий.
...
6.  При доступе к публичному URL (`/p/{publicationId}`) рендерится компонент `ARViewPage`.
7.  Компонент делает GET-запрос на `/api/v1/publish/arjs/public/:publicationId`, который возвращает данные потока с бэкенда.
8.  `UPDLProcessor` анализирует данные потока и конвертирует их в структуры UPDL на фронтенде.
9.  Система `ARJSBuilder` конвертирует пространство UPDL в элементы A-Frame, используя соответствующий шаблон.
...
5.  **Для контента AR.js**: Всегда используйте подход с iframe для правильного выполнения скриптов.
