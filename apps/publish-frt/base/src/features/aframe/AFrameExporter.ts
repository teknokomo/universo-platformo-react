...
export class AFrameVRExporter extends BaseAFrameExporter {
    ...
    protected convertToAFrameModel(updlScene: UPDLScene): AFrameScene {
        // Ensure objects, cameras, and lights are arrays before conversion
        const completeScene: any = {
            ...updlScene,
            objects: updlScene.objects || [],
            cameras: updlScene.cameras || [],
            lights: updlScene.lights || []
        }

        // Use the same converter as AR.js but customize for VR
        const convertedScene = this.converter.convertScene(completeScene)

        // Create a proper AFrameScene with required methods
        const aframeScene = createDefaultScene()

        // Copy attributes
        aframeScene.attributes = { ...convertedScene.attributes }

        // Преобразуем элементы с правильной типизацией для AFrameEntity
        aframeScene.children = convertedScene.children.map((child) => ({
            tag: (child as any).tag || AFrameTagType.ENTITY,
            attributes: (child as any).attributes || {},
            children: (((child as any).children || []) as any[]).map((subChild) => ({
                tag: subChild.tag || AFrameTagType.ENTITY,
