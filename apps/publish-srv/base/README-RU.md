...
-   **Интеграция с Canvas**: Полностью поддерживает новую структуру Canvas, сохраняя обратную совместимость с Canvas.
...
-   `POST /api/v1/publish/arjs` - Создает или обновляет запись о публикации (используйте `canvasId`; поле `canvasId` поддерживается для старых клиентов).
...
Получает сырые `flowData` для указанного `publicationId` (который может быть как `canvasId`, так и устаревшим `canvasId`). Поддерживает обратную совместимость.
