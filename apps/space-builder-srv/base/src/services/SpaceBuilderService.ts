...
export class SpaceBuilderService {
    ...
    async generate(input: {
        ...
        // First attempt
        let llmText = await callProvider({
            provider: input.selectedChatModel.provider,
            model: input.selectedChatModel.modelName,
    ...
    async proposeQuiz(input: {
        sourceText: string
        selectedChatModel: { provider: string; modelName: string; credentialId?: string }
        options: { questionsCount: number; answersPerQuestion: number }
        additionalConditions?: string
    }): Promise<QuizPlan> {
        const prompt = getPreparePrompt({
            sourceText: input.sourceText,
            questionsCount: input.options.questionsCount,
            answersPerQuestion: input.options.answersPerQuestion,
            additionalConditions: input.additionalConditions?.trim() || undefined
        })
    ...
    async generateFromPlan(input: {
        quizPlan: QuizPlan
        selectedChatModel: { provider: string; modelName: string; credentialId?: string }
        options?: { includeStartCollectName?: boolean; includeEndScore?: boolean; generateAnswerGraphics?: boolean }
    }): Promise<{ nodes: any[]; edges: any[] }> {
    ...
    private buildGraphFromPlan(
        plan: QuizPlan,
        opts?: { includeStartCollectName?: boolean; includeEndScore?: boolean; generateAnswerGraphics?: boolean }
    ): { nodes: any[]; edges: any[] } {
        ...
        plan.items.forEach((item, idx) => {
            ...
            nodes.push({
                ...
                data: {
                    ...
                    label: 'Data',
                    category: 'UPDL',
                    inputs: { dataName: qName, dataType: 'question', content: item.question }
                }
            })
