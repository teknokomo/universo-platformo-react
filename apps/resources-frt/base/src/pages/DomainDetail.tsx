      ...
      {error ? (
        ...
        <Stack spacing={2}>
                    ...
                    {resources.length === 0 ? (
                      ...
                      <Box display='grid' gridTemplateColumns='repeat(3, 1fr)' gap={2}>
                        {resources.map((resource) => (
                          <Card key={resource.id} sx={{ p: 2, cursor: 'pointer' }} onClick={() => navigate(clusterId ? `/clusters/${clusterId}/resources/${resource.id}` : `/resources/${resource.id}`)}>
          ...
          <Stack direction="row" spacing={1}>
            <Button
              variant="outlined"
              onClick={() => navigate(clusterId ? `/clusters/${clusterId}/domains` : '/domains')}
            >
