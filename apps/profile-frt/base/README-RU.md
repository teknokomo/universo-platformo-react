...
**Важное примечание**: В настоящее время SQL-функции для управления профилем включены в миграцию, связанную с Unik'ами `packages/server/src/database/migrations/postgres/1741277504476-AddUniks.ts`. Это временное решение для удобства разработки.

В будущем будет создана отдельная миграция специально для функциональности профиля пользователя для поддержания правильного разделения ответственности.

### Текущая структура миграции

Миграция `AddUniks1741277504476` включает:

-   Таблицы Unik (рабочих пространств) и связи
-   Политики Row Level Security (RLS)
-   **SQL-функции профиля пользователя** (временное размещение)

### План будущей миграции

Будет создана специальная миграция профиля, содержащая:

-   SQL-функции, специфичные для профиля пользователя
-   Схемы базы данных, связанные с профилем
...
-   **Связанность миграции**: Функции профиля временно в миграции Unik
