---
name: git-push
description: "Automates committing changes, creating GitHub issues, and opening pull requests with flexible push destination selection (fork vs upstream). Uses gh CLI for all GitHub operations. Never adds promotional or attribution lines."
...
3. **Detect Repository Context and Push Permissions**:
    ...
    - **Push Permission Detection**:
        ...
        - **GitHub API Check** (optional): Use `gh api repos/teknokomo/universo-platformo-react/collaborators/{username}/permission` to verify collaborator permissions (write/maintain/admin level)
        - **Protected Branch Check**: Use `gh api repos/teknokomo/universo-platformo-react/branches/{branch}/protection` to check if target branch has protection rules
...
5. **Create GitHub Issue in Upstream**:
    ...
    - **FIRST: Fetch repository labels** using `gh label list --repo teknokomo/universo-platformo-react` to get current list of available labels
    - Use **`gh` CLI** to create the issue:
      ```bash
      gh issue create --repo teknokomo/universo-platformo-react --title "<title>" --body "<body>" --label "<label1>,<label2>"
      ```
    ...
    - **IMPORTANT**: Issue Type (Task/Feature/Bug) must be set manually in GitHub UI after creation, or use appropriate labels:
...
9. **Create Pull Request to Upstream**:
    ...
    - Use **`gh` CLI** to create the PR:
      ```bash
      gh pr create --repo teknokomo/universo-platformo-react --base main --head "<source-branch>" --title "GH<issue_number> <title>" --body "<body>" --label "<label1>,<label2>"
      ```
    - **Source Branch Selection** (based on Step 8 push destination):
        - **If pushed to upstream**: Source = `<branch-name>`
        - **If pushed to fork**: Source = `<your-username>:<branch-name>`
...
-   **For fetching labels**: Use `gh label list --repo teknokomo/universo-platformo-react`
-   **For analysis step**: Read changed files to examine their content
-   **For git status**: Use `git status` command via Bash
...
-   **`gh` CLI Failure**: Provide clear error message with manual URLs:
...
-   **CLI→Manual**: If `gh` CLI fails, provide manual GitHub URLs and instructions
...
-   **"`gh` CLI unavailable"**: Use provided manual URLs for issue/PR creation, continue with git operations
...
2. **Gather Context Before Resolving**:
    ...
    - **GitHub context**: Use MCP GitHub tools or `gh pr list --state merged --search "<filename>"` to locate recent merged PRs that touched the conflicting files, then read their descriptions and review comments (`gh pr view <number>` or MCP GitHub) to understand the original intent
