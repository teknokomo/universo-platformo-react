---
name: mb
description: "Compresses Memory Bank files to optimal size while preserving all critical information. Acts as a Knowledge Curator, applying file-specific compression strategies with validation and backup safety."
tools: Bash, Edit, Write, Grep, Glob, Read, WebFetch
---
...
5. **GitHub Releases Update** (3 min):
   - Fetch latest releases via GitHub API:
     ```bash
     gh api repos/teknokomo/universo-platformo-react/releases --jq '.[] | {tag_name, published_at, name, body}'
     ```
