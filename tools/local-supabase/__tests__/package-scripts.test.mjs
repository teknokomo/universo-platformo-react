...
const legacyCoreFrontendBasePath = ['packages/universo-react-core-frontend', 'base', ''].join('/')
...
describe('local Supabase package scripts', () => {
    it('keeps destructive local start behind env generation and doctor checks', () => {
        ...
        expect(script).toContain('UNIVERSO_FRONTEND_ENV_FILE=packages/universo-react-core-frontend/.env.local-supabase')
    ...
    it('keeps minimal local app starts on the reduced Supabase stack', () => {
        ...
        expect(startScript).toContain('UNIVERSO_FRONTEND_ENV_FILE=packages/universo-react-core-frontend/.env.local-supabase')
        ...
        expect(allcleanScript).toContain('UNIVERSO_FRONTEND_ENV_FILE=packages/universo-react-core-frontend/.env.local-supabase')
    ...
    it('keeps local E2E commands behind the E2E profile doctor', () => {
        ...
        expect(buildScript).toContain('UNIVERSO_FRONTEND_ENV_FILE=packages/universo-react-core-frontend/.env.e2e.local-supabase')
        expect(smokeScript).toContain('UNIVERSO_FRONTEND_ENV_FILE=packages/universo-react-core-frontend/.env.e2e.local-supabase')
    ...
    it('keeps the agent E2E gate protected by package layout and naming guards', () => {
        const script = rootPackageJson.scripts['test:e2e:agent']

        expect(script).toContain('pnpm run check:no-package-base-paths')
        expect(script).toContain('pnpm run check:package-naming')
        expect(script).toContain('pnpm run check:apps-template-isolation')
        expect(script.indexOf('pnpm run check:no-package-base-paths')).toBeLessThan(script.indexOf('pnpm run build:e2e'))
        expect(script.indexOf('pnpm run check:package-naming')).toBeLessThan(script.indexOf('pnpm run build:e2e'))
        expect(script.indexOf('pnpm run check:apps-template-isolation')).toBeLessThan(script.indexOf('pnpm run build:e2e'))
    })
})
