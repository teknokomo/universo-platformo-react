...
const createMockSupabaseClient = (overrides: Partial<SupabaseClientMock> = {}): SupabaseClientMock => {
...
declare global {
    ...
    var createMockSupabaseClient: (overrides?: Partial<SupabaseClientMock>) => SupabaseClientMock
}
...
expect.extend({
    toHaveBeenCalledOnceWith(received: unknown, ...expected: unknown[]) {
        ...
        const passMessage = () => `expected mock not to be called once with ${this.utils.printExpected(expected)}`
        const failMessage = (error: string) => `${matcherName} expected a Jest mock. ${error}`
        ...
        return {
            ...
            message: () =>
                pass
                    ? passMessage()
                    : `expected mock to be called exactly once with ${this.utils.printExpected(
                          expected
