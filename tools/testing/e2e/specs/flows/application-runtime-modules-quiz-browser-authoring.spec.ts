...
export default class SpaceQuizWidget extends ExtensionModule {
    ...
    async submit(payload) {
        return this.ctx.callServerMethod('validateAnswer', [payload])
    }
...
async function createQuizModuleThroughBrowser(
    page: Page,
    metahubId: string,
    moduleName: string,
    ...
    await expectNoPageHorizontalOverflow(page, 'quiz module browser authoring dialog')
    await expectNoTechnicalLeakage(dialog, {
        label: 'quiz module browser authoring dialog',
        checkUuidSubstrings: true,
        allowTextPatterns: [/SpaceQuizWidget|@universo\/extension-sdk|@AtClient|@AtServer|@shared/i]
    })
...
async function configureQuizWidgetThroughBrowser(
    ...
    await expect(dialog).toBeVisible()
    await expect(dialog.getByText('Choose which published quiz behavior this widget should use')).toBeVisible()
    await expect(dialog.getByRole('combobox', { name: 'Quiz module', exact: true })).toBeVisible()
    await expect(dialog.getByText(/Module codename/i)).toHaveCount(0)
    await expect(dialog.getByText(/Mount method|Submit method|client method/i)).toHaveCount(0)
    await expectNoPageHorizontalOverflow(page, 'quiz widget configuration dialog')
    await expectNoTechnicalLeakage(dialog, { label: 'quiz widget configuration dialog', checkUuidSubstrings: true })
    await dialog.screenshot({ path: screenshotPath })

    await dialog.getByRole('combobox', { name: 'Quiz module', exact: true }).click()
...
async function createApplicationThroughBrowser(
    ...
    await syncApplicationSchema(api, application.id)
...
test('@flow quiz widget modules authoring can be completed through browser surfaces before runtime verification', async ({
    page,
    runManifest
}, testInfo) => {
    ...
    const moduleName = 'Runtime quiz widget'
    const moduleCodename = 'quiz-widget'
    ...
    try {
        ...
        await createQuizModuleThroughBrowser(
            page,
            metahub.id,
            moduleName,
            moduleCodename,
            testInfo.outputPath('quiz-modules-authoring-dialog.png')
        )

        const layoutId = await waitForLayoutId(api, metahub.id)
        await configureQuizWidgetThroughBrowser(
            page,
            api,
            metahub.id,
            layoutId,
            moduleCodename,
            testInfo.outputPath('quiz-widget-configuration-dialog.png')
        )
        ...
        const runtimeModulesResponse = page.waitForResponse(
            (response) =>
                ...
                response.url().includes(`/api/v1/applications/${application.id}/runtime/modules?`) &&
        ...
        const runtimeResponse = await runtimeModulesResponse
        expect(runtimeResponse.ok()).toBe(true)
        const runtimeModulesPayload = await runtimeResponse.json()
        expect(JSON.stringify(runtimeModulesPayload)).not.toMatch(
            /serverBundle|clientBundle|sourceCode|server bundle|client bundle|apiKey|token/i
        )
        expect(
            (runtimeModulesPayload?.items ?? []).every(
                (item: Record<string, unknown>) => item.config && Object.keys(item.config).length === 0
            )
        ).toBe(true)
        ...
        await expectNoPageHorizontalOverflow(page, 'quiz modules browser authored runtime')
        await expectNoTechnicalLeakage(page.locator('body'), {
            label: 'quiz modules browser authored runtime',
            checkUuidSubstrings: true
        })
        await expectNoVisibleTextPatterns(
            page.locator('body'),
            [/sourceCode|serverBundle|clientBundle|_app_modules|moduleRole|attachedToId|mount\(\)|submit\(\)|client bundle/i],
            {
                label: 'quiz modules browser authored runtime'
            }
        )
        await page.screenshot({ path: testInfo.outputPath('quiz-modules-browser-authored-runtime-en.png'), fullPage: true })
        ...
        for (const viewport of [
            ...
            await expect(page.getByText('Space Quiz', { exact: true })).toBeVisible()
            await expect(page.getByText('Which planet is known as the Red Planet?', { exact: true })).toBeVisible()
            await expectNoPageHorizontalOverflow(page, `quiz modules browser authored runtime ${viewport.name}`)
            await expectNoTechnicalLeakage(page.locator('body'), {
                label: `quiz modules browser authored runtime ${viewport.name}`,
                checkUuidSubstrings: true
            })
            await expectNoVisibleTextPatterns(
                page.locator('body'),
            ...
            await page.screenshot({
                path: testInfo.outputPath(`quiz-modules-browser-authored-runtime-en-${viewport.name}.png`),
                fullPage: true
            })
        }
