...
async function assertQuizQuestionUi(options: {
    ...
    await expectNoPageHorizontalOverflow(options.page, `quiz modules runtime ${options.locale} question ${options.questionIndex + 1}`)
    await expectNoTechnicalLeakage(options.page.locator('body'), {
        label: `quiz modules runtime ${options.locale}`,
        checkUuidSubstrings: true
    })
    await expectNoVisibleTextPatterns(
        options.page.locator('body'),
...
test('@flow quiz widget modules publish into runtime and execute through the real /a browser surface', async ({
    page,
    runManifest
}, testInfo) => {
    ...
    const metahubName = `E2E ${runManifest.runId} modules quiz metahub`
    const metahubCodename = `${runManifest.runId}-modules-quiz`
    const publicationName = `E2E ${runManifest.runId} Modules Quiz Publication`
    const applicationName = `E2E ${runManifest.runId} Modules Quiz Application`
    ...
    try {
        ...
        if (!metahub?.id) {
            throw new Error('Metahub creation did not return an id for modules quiz coverage')
        }
        ...
        await expectJsonResponse(
            await sendWithCsrf(api, 'POST', `/api/v1/metahub/${metahub.id}/modules`, {
                codename: moduleCodename,
                name: 'Quiz widget',
                description: 'Runtime modules quiz widget',
        ...
        if (!publication?.id) {
            throw new Error('Publication creation did not return an id for modules quiz coverage')
        }
        ...
        await createPublicationVersion(api, metahub.id, publication.id, {
            name: { en: `E2E ${runManifest.runId} Modules Quiz Version` },
            namePrimaryLocale: 'en'
        })
        ...
        if (typeof applicationId !== 'string') {
            throw new Error('Linked application creation did not return an id for modules quiz coverage')
        }
        ...
        const listModulesPayload = await listModulesResponse.json()
        const selectedModule = listModulesPayload?.items?.find((item) => item.codename === moduleCodename)
        expect(selectedModule?.clientBundle ?? null).toBeNull()
        expect(selectedModule?.serverBundle ?? null).toBeNull()
        ...
        await test.step('English runtime flow proves scoring semantics and backward navigation', async () => {
            ...
            await page.screenshot({ path: testInfo.outputPath('quiz-modules-runtime-en.png'), fullPage: true })
