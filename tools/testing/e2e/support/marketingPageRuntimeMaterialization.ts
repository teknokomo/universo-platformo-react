...
/**
 * Flatten the widget-owned transport records for semantic fixture assertions.
 * The same site settings or section-copy record may be supplied to multiple
 * widgets, so physical row IDs are used only to remove transport duplicates.
 */
export const flattenMarketingPageRecords = (payload: RuntimePayload): RuntimeRecord[] => {
    const records = payload.marketingPage?.widgets?.flatMap((widget) => widget.data?.records ?? []) ?? []
    const seen = new Set<string>()
    return records.filter((record, index) => {
        const id = typeof record.id === 'string' && record.id ? `id:${record.id}` : undefined
        const semanticIdentity =
            typeof record.kind === 'string' && typeof record.semanticKey === 'string'
                ? `semantic:${record.kind}:${record.semanticKey}`
                : `position:${index}`
        const identity = id ?? semanticIdentity
...
export function assertMarketingPageRuntimeMaterialization(payload: RuntimePayload, manifest: SeedManifest): void {
    ...
    assert.ok(widgets.length > 0, 'Marketing runtime must contain widget instances')
    assert.ok(
        widgets.every((widget) => typeof widget.widgetKey === 'string'),
        'Marketing runtime widgets must have semantic keys'
    )
    assert.deepEqual(
    ...
    assert.deepEqual(
        Object.fromEntries(
            recordsByKind(records, 'sectionCopy').map((record) => [
                readString(record.sectionKey),
                { title: readLocalized(record.title), description: readLocalized(record.description) }
            ])
        ),
        sectionCopies,
