...
} from './mmoommFlightFixtureContract'
...
export async function loadMmoommFlightSnapshotFixture(): Promise<{ fixturePath: string; metahubName: string; fixture: SnapshotFixture }> {
    const fixturePath = path.join(process.cwd(), 'tools', 'fixtures', MMOOMM_FLIGHT_FIXTURE_FILENAME)
    const rawFixture = await fs.readFile(fixturePath, 'utf8')
    const fixture = JSON.parse(rawFixture) as SnapshotFixture

    assertMmoommFlightFixtureEnvelopeContract(fixture)

    const metahubName = readLocalizedText((fixture as { metahub?: { name?: unknown } }).metahub?.name)
    if (!metahubName) {
        throw new Error('MMOOMM flight snapshot fixture does not contain a metahub name')
    }
...
async function submitSnapshotImportDialog(page: Page, dialog: Locator): Promise<Response> {
    ...
    throw new Error(`MMOOMM flight snapshot import failed with HTTP ${response.status()}: ${(await response.text()).slice(0, 500)}`)
}

export async function importMmoommFlightSnapshotThroughUi(page: Page): Promise<{
    fixturePath: string
    metahubName: string
    metahubId: string
    publicationId: string
}> {
    const fixture = await loadMmoommFlightSnapshotFixture()
    ...
    await expect(dialog.getByText(MMOOMM_FLIGHT_FIXTURE_FILENAME)).toBeVisible()
    ...
    if (typeof metahubId !== 'string' || typeof publicationId !== 'string') {
        throw new Error('MMOOMM flight snapshot import did not return metahub and publication ids')
    }
