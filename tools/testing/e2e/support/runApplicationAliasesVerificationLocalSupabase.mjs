...
try {
    ...
    await run(['test:e2e:application-aliases'], { env: localSupabaseEnv })
} catch (error) {
    failed = true
    throw error
} finally {
    try {
        ...
        process.stderr.write(`Failed to stop local E2E Supabase after application aliases verification: ${message}\n`)
        if (!failed) {
            process.exitCode = 1
        }
    }
