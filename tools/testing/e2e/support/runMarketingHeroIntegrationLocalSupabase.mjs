...
const artifactRunId = new Date()
    ...
    .replace(/[^0-9A-Za-z]+/g, '-')
    .replace(/-+$/g, '')
const artifactRoot = path.join(repoRoot, 'tools/testing/e2e/.artifacts/marketing-hero-integration', artifactRunId)
...
async function preserveArtifacts({ status, error, includeSharedArtifacts }) {
    ...
    await fs.writeFile(
        ...
        `${JSON.stringify(
            {
                gate: 'marketing-hero-real-postgresql-and-chromium',
                status,
                error: error ?? null,
...
try {
    await coordinateLocalSupabaseE2eGate({
        ...
        reportCleanupFailure: (error) => {
            ...
            process.stderr.write(`Marketing Hero integration cleanup failed: ${message}\n`)
            process.exitCode = 1
        }
    })
} catch (error) {
