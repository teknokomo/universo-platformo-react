...
import { MERIDIAN_73_FIXTURE_FILENAME, assertMeridian73FixtureEnvelopeContract } from './meridian73FixtureContract.ts'

const trackedPath = path.resolve(
    repoRoot,
    process.env.MERIDIAN_73_FIXTURE_TRACKED_PATH ?? path.join('tools', 'fixtures', MERIDIAN_73_FIXTURE_FILENAME)
)
const defaultGeneratedPath = path.resolve(repoRoot, 'tools', 'testing', 'e2e', '.artifacts', 'generated-' + MERIDIAN_73_FIXTURE_FILENAME)
const generatedPathArg = process.argv.slice(2).find((arg) => arg !== '--')
const generatedPathInput = generatedPathArg ?? process.env.MERIDIAN_73_FIXTURE_GENERATED_PATH ?? defaultGeneratedPath
...
const normalizeVolatileValues = (value: unknown, maps = createNormalizerMaps()): unknown => {
    ...
    if (typeof value === 'string') return normalizeString(value, maps)
    if (Array.isArray(value)) return value.map((item) => normalizeVolatileValues(item, maps))
...
const readFixture = (filePath: string): MetahubSnapshotTransportEnvelope => {
    if (!fs.existsSync(filePath)) {
        throw new Error(
            'Generated 73rd Meridian fixture does not exist: ' +
                filePath +
                '. Run the generator with MERIDIAN_73_FIXTURE_OUTPUT_PATH or pass an explicit generated fixture path.'
        )
    }
...
const validateContract = (label: string, fixture: MetahubSnapshotTransportEnvelope): void => {
    try {
        ...
        throw new Error(label + ' fixture does not satisfy the 73rd Meridian contract.\n' + message)
    }
}
...
if (stableStringify(normalizedTracked) !== stableStringify(normalizedGenerated)) {
    ...
    throw new Error(
        [
            '73rd Meridian fixture drift detected after normalizing volatile IDs, timestamps, and snapshot hashes.',
...
process.stdout.write('73rd Meridian fixture drift check passed: ' + path.relative(repoRoot, generatedPath) + '\n')
