...
Реализация Universo Platformo / Universo MMOOMM / Universo Kiberplano на основе React и связанного стека технологий.
...
-   **Минимальные изменения ядра**: Оригинальный код остаётся в основном нетронутым
...
3. Настройте переменные окружения
    ...
    - Создайте файл `.env` в директории `packages/universo-core-backend/base`
    - Добавьте необходимую конфигурацию Supabase:
        ...
        ```
    - Опционально создайте файл `.env` в директории `packages/universo-core-frontend/base` для UI-специфичных настроек, таких как `VITE_PORT`

    Примечание: После рефакторинга конфигурация Supabase должна указываться только в директории `packages/universo-core-backend/base`.
