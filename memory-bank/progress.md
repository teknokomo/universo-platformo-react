...
-   **Unbind no-op — root cause was the backend shallow-merge, not the surface.** The entity `update` PATCH (`MetahubObjectsService.updateObject`) merges `config` as `stripUndefinedEntries({...existing.config, ...input.config})`, so an absent key means "leave unchanged", not "remove" — `stripUndefinedEntries` only drops `undefined`, never `null`. `ProjectBindingSurface.writeBinding(null)` used `delete config.projectBinding`, so the omitted key let the server's existing binding survive the merge: the PATCH returned 200, the success toast fired, the `entityDetail` cache was invalidated and refetched — but the refetched config still carried the binding, so the bound card stayed. Fix: send the **documented clear signal** `projectBinding: null` (`config.projectBinding = nextBinding ?? null`). `validateProjectBindingConfigForEntity` already allows `null`/`undefined` as "clear"; `null` survives `stripUndefinedEntries`; and every reader (`readBinding`, `extractProjectBindingFromConfig`, the cascade `COUNT(... config->'projectBinding'->>'projectCodename' ...)`) treats `null` as unbound. `JSON.stringify` drops `undefined`, so an `undefined` would never have reached the server — `null` is the only client-side signal that does.
-   **Spurious dialog scrollbar — root cause was the stored resize height used as a hard cap.** `dialogPresentation` persists a custom dialog size keyed per metahub (`storageScopeKey: metahubId` in `MetahubDialogSettingsProvider`), so a size set on one dialog is read by every dialog in that metahub. It was applied as a fixed `height: min(viewportHeight, storedHeight)`, so when the stored height was shorter than another dialog's natural content, the paper clamped below the content and `DialogContent` (`overflowY: 'auto'`) showed an inner scrollbar even though the viewport still had room to grow. (The earlier P5 change set both overflow axes explicitly but was a no-op for this case — `overflowX:hidden`+`overflowY:visible` already computes to `auto`.) Fix: when idle, apply the stored height as `minHeight` (a floor) instead of a fixed `height`; the paper now grows to fit its content up to `maxHeight` and only scrolls when content genuinely exceeds the viewport. The exact `height` is still pinned during an active resize drag (`isResizing`) for 1:1 cursor feedback.
-   **Tests:** template-mui jest +1 (`EntityFormDialog`: a stored size is applied as a `minHeight` floor with no fixed `height`); metahubs-frontend vitest — rewrote the `ProjectBindingSurface` unbind test to assert the null-clear contract (`config: { sortOrder, projectBinding: null }`, unrelated config preserved); backend jest +1 (`MetahubObjectsService`: `projectBinding: null` clears the stored binding while an absent key keeps it — documents the merge semantics that caused the bug); E2E `metahub-projects-section.spec.ts` +unbind-confirm flow (clicks confirm, then asserts the empty-state card returns AND the persisted instance config has `projectBinding == null`). The pre-existing `metahub-entity-dialog-regressions` horizontal-overflow guards are unchanged.
-   **Verification:** template-mui jest 263/265 (the 2 failures are pre-existing `TooltipWithParser` XSS tests, unrelated); metahubs-frontend vitest 351/351; backend `MetahubObjectsService` 12/12; full backend sweep — only the 4 known pre-existing failing suites (`modulesRoutes`, `metahubMigrationsRoutes`, `playCanvasProjectsController`, `publicationsRoutes`), 0 new regressions; projects-section E2E green on local minimal Supabase (build 36/36, unbind-confirm + data-truth assertions pass). Lint + prettier clean; template-mui `tsc` 0 errors. Supabase profile stopped after the run; vendor PlayCanvas Editor untouched.

## 2026-06-18 - PlayCanvas Projects: QA Round 2 (5 user-reported defects)

Closed 5 defects from manual QA of a metahub imported from the tracked MMOOMM
app snapshot.

-   **P1 (no code change)**: `tools/fixtures/.generated-*.json` is a gitignored drift-comparison artifact (the MMOOMM-app-gate writes the freshly-generated snapshot there, then `check:mmoomm-app-fixture-drift` diffs it against the tracked fixture). Not garbage, not committed; safe to delete between runs.
-   **P2 (remove junk row action + page)**: removed the row "Open project" (`open-project-binding`) descriptor + `handleOpenProjectBindingRow`, the standalone `/metahub/:id/entities/:kindKey/instance/:entityId/project` route, the `ProjectBindingPage` lazy import/export, and the default page export. The row keeps "Open editor"; all binding management lives in the edit-dialog "PlayCanvas" tab. Renamed `ProjectBindingPage.tsx` → `ProjectBindingSurface.tsx` (it now exports only `ProjectBindingSurface`, always chrome-less).
-   **P3 (tab rename)**: the binding tab is now "PlayCanvas" (was "Project"/"Проект") — `projects.binding.resourceTabTitle` EN+RU, the `EntityInstanceListContent` fallback, and the backend preset surface `title`/`fallbackTitle`. Removed the now-unused `projects.binding.actions.openTab` key.
-   **P4 (editor opened wrong/"default" project)**: root cause was a stale cached `config.projectBinding.projectId`. (a) Snapshot import (`SnapshotRestoreService`) inserts entities before PlayCanvas projects, so the binding's `projectId` kept the source id; added `remapEntityProjectBindingReferences` post-pass that `jsonb_set`s the remapped project id once `projectIdMap` is known. (b) The row "Open editor" handler now resolves the LIVE project id by codename (canonical) against the projects list, falling back to the cached id — so existing (already-imported, stale) instances work without re-import. `hasBoundProjectId` → `hasBoundProject` (codename OR id). The package's `defaultProjectId` still governs opening the editor from Resources/Packages (unchanged).
-   **P5 (spurious dialog scrollbar)**: `EntityFormDialog` set `overflowY: 'visible'` on `DialogContent` while the presentation layer set `overflowX: 'hidden'`; per the CSS overflow spec a `visible` axis paired with a non-`visible` axis computes to `auto`, so the dialog body always showed a vertical scrollbar. Fixed at the source: `dialogPresentation.contentSx` now sets both axes explicitly (`overflowX: 'hidden'`, `overflowY: 'auto'`) and `EntityFormDialog` uses `presentation.contentSx` directly. Affects all entity dialogs (the bug was global, not project-specific).
-   **Tests**: +2 vitest (`ProjectBindingSurface`: loading-guard, cache invalidation — unchanged) still green; +1 backend test (`SnapshotRestoreService`: projectBinding projectId remap); reworked `metahub-projects-section.spec.ts` + the MMOOMM generator spec to drive binding through the edit-dialog "PlayCanvas" tab (no standalone page), added a dialog-scrollbar regression guard (`scrollHeight <= clientHeight`) and a row-menu guard ("Open editor" present, "Open project" absent).
-   **Verification**: FE vitest 349/349; BE full suite 1044 passed (same 4 pre-existing failing suites, 0 new regressions); FE+BE+template-mui build & lint green; prettier clean; `metahub-projects-section.spec.ts` 3/3 green on local minimal Supabase.

## 2026-06-19 - PlayCanvas Projects: Non-object-like preset + "Bind existing" + dialog fix

Three QA follow-ups closed in a single pass, all driven by the user's "make project like Enumeration" direction.

-   **Project preset is now a dedicated (non-object-like) type.** `PROJECT_TYPE_CAPABILITIES` reduced to `treeAssignment:enabled` + `projectBinding:enabled`; everything else (`dataSchema`, `records`, `physicalTable`, `hierarchy`, `actions`, `events`, `modules`, `layoutConfig`, `runtimeBehavior`, `identityFields`, `recordLifecycle`, `posting`, `ledgerSchema`, `relations`, `nestedCollections`, `optionValues`, `fixedValues`, `blockContent`) is `false`. `PROJECT_TYPE_UI.tabs` is now `['general','hubs','project']` — no more Компоненты / Макеты / Модули / Действия / События on a fresh metahub. The capability set is dependency-clean (`validateCapabilityDependencies` returns `[]`). All toggles exist in the Entity Type Constructor (`projectBinding` `:2027`, `treeAssignment` `:1865`, `physicalTable` `:2215`, `dataSchema` `:1795`) and the constructor's cascade (`disableRecursively`) auto-clears the dependents. The generic (null-behavior) CRUD path is already used by the frontend for `project` (`resolveEntityMetadataKind` returns `null` for non-builtin kinds), so no frontend routing/permission changes were needed; backend CREATE/UPDATE/COPY/DELETE handlers and `cascadeBoundProject` already cover the null-behavior branch. `recordLifecycle` and `hierarchy` were already forced off together with `dataSchema` per `CAPABILITY_DEPENDENCIES`.
-   **Tracked MMOOMM snapshot fixture regenerated.** `tools/fixtures/metahubs-mmoomm-app-snapshot.json` now reflects the new capability set (verified: `project.ui.tabs = ['general','hubs','project']`, `dataSchema:false`, `physicalTable:false`, etc., `projectBinding.projectId` correctly remapped to the freshly-generated project id via the existing `remapEntityProjectBindingReferences` snapshot-import post-pass). MMOOMM app gate: drift check clean, generator 2/2 passed, runtime import 2/2 passed.
-   **"Bind existing project" UI (Phase 3).** The `PlayCanvas` tab now offers a second action next to "Create & bind project" — "Bind existing project" — opening a `StandardDialog` with an MUI `Autocomplete` picker. The picker is fed by the existing `playcanvasProjectsApi.list` plus `listEntityInstances({kind:'project'})` for a diff, with a "Show only unbound projects" Switch (default on). Already-bound projects are still selectable (sharing is allowed) but are visibly marked with an "Already bound" warning chip. The "Bind" button calls the existing `writeBinding` — no new backend surface needed for the happy path. **Cascade-safety backend guard** (the real fix in this pass): `MetahubObjectsService.countActiveProjectBindingsByCodename` returns the count of ACTIVE instances still bound to a given codename; `entityCrudHandlers.cascadeBoundProject` consults it (excluding the about-to-be-deleted instance) and **skips the project deletion** if any other instance still references it, so deleting one instance no longer orphans the shared project. `countActiveProjectBindingsByCodename` filters on `_mhb_deleted = FALSE` so a soft-deleted sibling cannot keep a shared project alive either.
-   **Dialog label clipping fix (Phase 2).** The "Create & bind PlayCanvas project" dialog's content stack had `pt: 0.5`; the outlined TextField's floating label sat above the dialog's top edge and got clipped by the (now-scroll) `DialogContent`. Bumped the stack to `pt: 1.5` with a comment explaining the layout math.
-   **Tests:** `projectPresetTemplate.test.ts` rewritten to assert the new non-object-like contract (capabilities, dependency-clean, `resolveEntityMetadataKindFromType === null`); `MetahubObjectsService.test.ts` +2 unit tests for `countActiveProjectBindingsByCodename` (with/without excluded id); `entityInstancesRoutes.test.ts` +2 route tests (cascade fires when 0 siblings, cascade skipped when ≥1 sibling). Frontend: `ProjectBindingSurface.test.tsx` +2 tests — "Bind existing" picker writes the correct `config.projectBinding`, and the unbound-only filter hides already-bound projects by default and reveals them with an "Already bound" badge when toggled off (total 13/13 passing).
-   **Verification:** FE vitest 351/351, build + lint 0 errors, prettier clean; BE 1048 passed (4 pre-existing failing suites unchanged, +4 from this work, 0 new regressions); builds + lint clean; i18n docs check 98/98 OK; MMOOMM app gate green; projects-section E2E 3/3 green on local minimal Supabase; Supabase profile stopped after runs. Vendor PlayCanvas Editor (`packages/universo-react-playcanvas-editor-frontend/vendor`) untouched.

## 2026-06-18 - PlayCanvas Projects: Code-Review Remediation (10 findings)

Closed all 10 findings from the 2026-06-18 high-effort code review of the
PlayCanvas "Projects" entity-type binding work.

-   **Embedded binding tab (#1)**: `ProjectBindingSurface` takes explicit `metahubId`/`entityId` props (falls back to `useParams`); the form-dialog tab passes them since its route lacks `:entityId`, so the tab now loads/saves the real binding instead of showing the empty state.
-   **Branch-schema mismatch (#2)**: binding validation now resolves the project via `PlayCanvasProjectsService.resolveBoundProjectByCodename` (default-branch project store) — same schema as the cascade and `getAuthoringHost`. `validateAndResolveProjectBinding` takes a `resolveProject` resolver instead of `exec`/`schemaName`, removing schema-resolution duplication.
-   **Editor-open dedup (#4/#6/#7)**: new shared `packages/api/playcanvasEditorHost.ts` (`usePlayCanvasEditorHostQuery`, `resolveEditorDisplayMode`, `openPlayCanvasEditor`) used by both the binding card and the row action — one React Query cache key, one URL/`openSeparately`/`?projectId=` contract. Default export reduced to a thin `<ProjectBindingSurface />` (no double chrome).
-   **projectId shape (#5)**: `validateProjectBindingConfigForEntity` rejects non-string / empty `projectId`.
-   **Dead import (#8)**: removed unused `validateProjectBindingConfigForEntity` import from `entityCrudHandlers`.
-   **Tests (#3)**: 2 new `packagesRoutes` tests (`?projectId=` override + blank fallback), rewrote `validateAndResolveProjectBinding` tests for the resolver, added `resolveBoundProjectByCodename` + projectId-shape tests. Also memoized `hasBoundProjectId` and corrected two `useCallback` dep arrays (lint warnings → 0).
-   **Canon Refresh (#9) + activeContext compression (#10)**: repository version 0.65.0 → 0.69.0-alpha in `productContext.md`/`projectbrief.md`; added `project` preset + `playcanvas` template to canon; `activeContext.md` compressed 389 → 122 lines (within range).
-   **Verification**: frontend vitest 347/347; backend touched suites 139/139 (`projectBindingConfig`, `PlayCanvasProjectsService`, `packagesRoutes`); full backend jest shows only the 4 pre-existing failing suites (`modulesRoutes`, `metahubMigrationsRoutes`, `playCanvasProjectsController`, `publicationsRoutes`), 0 new regressions; frontend + backend build green; lint 0 errors; prettier clean; E2E `metahub-projects-section.spec.ts` 3/3 green on local minimal Supabase.
-   **Deliberately not changed**: `MetahubPackagesTab.openEditor` keeps its package-level always-new-tab behavior (no project pin), distinct from the project-binding surfaces — collapsing it would change behavior.

### Codex-review follow-up (2 P2 findings)

A subsequent Codex review of the same working tree surfaced 2 additional P2 findings, both confirmed against the code and fixed in `ProjectBindingPage.tsx`:

-   **Embedded tab exposed actions before data settled**: the `embedded` branch returned the actionable body without the loading/error guards the standalone path had, so on a slow request an already-bound instance could briefly render as empty and expose Create (which would `writeBinding` with `current` undefined, overwriting `config` from `{}`) or Unbind (clearing a still-loading valid binding). Refactored so a single `wrap()` helper applies the loading and error guards in BOTH modes (chrome-less when embedded, chromed otherwise) before any mutation action renders.
-   **Private cache key + narrow invalidation**: the surface used a private `['metahubs','entityInstance',…]` query key and invalidated only that, leaving the surrounding list/edit caches stale (row "Open editor" shortcut and bound state lagged until a full refresh). Switched to the canonical `useEntityInstanceQuery` (the shared `entityDetail` key) and, on every binding write, invalidate `invalidateEntitiesQueries.all(kind)` + `.detail(entityId)` — the exact pattern the canonical entity-update mutation uses. Kind is read from the loaded instance.
-   **Tests**: +2 vitest (`renderEmbedded` helper) — loading state hides Create/Unbind until data settles; unbind invalidates the entity-detail + entities caches. FE vitest 349/349; build + lint green; prettier clean; `metahub-projects-section.spec.ts` 3/3 green again on local minimal Supabase.

## 2026-06-17 - OntoIndex Generated Skills Committed + Complement Slimmed

-   `ontoindex setup` generated 6 Claude-native skills under
    `.claude/skills/ontoindex/` (guide, cli, exploring, impact-analysis,
    debugging, refactoring) plus an `<!-- ontoindex:start/end -->` block in
    `AGENTS.md` and a new root `CLAUDE.md`. Decision: **commit them** (not
    gitignore) so the team shares the same guidance. `.claude/skills/` is Claude
    Code's native discovery path (verified against official docs); `.agents/skills/`
    is the project convention referenced via `AGENTS.md`, so the generated skills
    correctly stay in `.claude/skills/`.
-   Slimmed the project skill `.agents/skills/ontoindex-code-intelligence/SKILL.md`
    from a how-to-use duplicate into a **thin complement**: keeps only the three
    things the generated skills lack — namespace-collision caveat, this repo's
    pnpm wrapper + golden queries, and the SQL-first edge-quality limitation —
    and points to the generated skills for mechanics. `references/usage.md` kept.
-   Added a Universo-complement note inside the AGENTS.md ontoindex block so the
    generated guidance and the project skill coexist (prefer `pnpm ontoindex:*`
    over bare `npx ontoindex`).
-   Registered the generated artifacts in `.agents/skills/SOURCES.md` under a new
    "Generated By OntoIndex CLI" section (AGPL-3.0-or-later, regenerable) and
    updated the `ontoindex-code-intelligence` row to reflect its complement role.
-   Verification: prettier clean on all touched `.md`; `check:agent-profiles`
    green; no CI `.md` lint gate and docs i18n untouched. Branch
    `feature/ontoindex-setup-skills` (PR #857 already merged — this is a follow-up).
...
## 2026-06-17 - PlayCanvas Metahub Template + "Projects" Entity Type (Code Complete, Live E2E Pending)

-   Added `projectBinding` capability in `@universo-react/types` and threaded it through the Entity Type Constructor (normalizer, default, capability toggle, resource surface). Removed the silent-drop bug for unknown capabilities.
-   Registered the `project` preset (object-like-minimal, `sidebarOrder: 5` — above Hubs) and the `playcanvas` metahub template (no package-declaration field; the `MetahubTemplateManifest` schema does not support that, confirmed in `metahubs.ts`).
-   Server-side `config.projectBinding` validation on create/update (`validateProjectBindingConfigForEntity`) and cascade-delete on instance removal (`PlayCanvasProjectsService.deleteBoundProject`, idempotent on stale binding).
-   New `ProjectBindingPage` resource surface on `template-mui` primitives; row action menu entry; route `entities/:kindKey/instance/:entityId/project`; EN/RU i18n; component tests (4 tests, vitest green).
-   Removed `PlayCanvasProjectsPanel` from `MetahubPackagesTab.tsx`; pruned obsolete test cases; deleted dead `packages.projects.*` i18n keys; lint clean; 16 MetahubPackagesTab tests green.
-   Reworked `metahubs-mmoomm-app-export.spec.ts` to author the project through the Projects section; updated `mmoommAppFixtureContract.assertDomainModel` to require a project instance with `config.projectBinding.projectCodename`; removed the stale `metahubs-mmoomm-app-snapshot.json` (regeneration requires the live `pnpm supabase:e2e:start:minimal` flow).
-   New `metahub-projects-section.spec.ts` (Playwright) ready for the live E2E run; asserts menu ordering (Projects above Hubs), Create & bind, Publish, no-raw-UUID leakage, responsive matrix.
-   Backend full jest suite: **0 regressions** (4 pre-existing failing suites unchanged on main, 2 new suites +12 tests green). Component vitest green.
-   GitBook docs EN/RU: `projects-entity-type.md`, `playcanvas-template.md`; cross-links in `playcanvas-projects.md` / `playcanvas-editor.md`; `SUMMARY.md` EN/RU updated.
-   Open: live Supabase + Playwright E2E pass (regenerate `metahubs-mmoomm-app-snapshot.json`, capture screenshots, run `expectNoTechnicalLeakage` matrix).

## 2026-06-17 - PlayCanvas Projects Section Live E2E Pass

-   Brought up `pnpm supabase:e2e:start:minimal` + `pnpm env:e2e:local-supabase` + `pnpm run build:e2e` (36 tasks, 0 failures) and ran `metahub-projects-section.spec.ts` through `tools/testing/e2e/run-playwright-suite.mjs` — **3/3 green** (auth setup, bound-state test, empty-state test).
-   Fixed three real spec bugs in `metahub-projects-section.spec.ts`: (1) passed `page` to `createLoggedInBrowserContext` instead of `browser`; (2) passed `runManifest` to `createMetahub`; (3) `expectNoTechnicalLeakage` was passed a `Page` instead of a `Locator`. Replaced the missing `listEntityInstances` import with a direct `/api/v1/metahub/.../entities` fetch and the cookie header.
-   Added `listEntityInstances` to `tools/testing/e2e/support/backend/api-session.mjs` (the function was imported by both this spec and `metahubs-mmoomm-app-export.spec.ts` but never exported) and corrected its URL to include the `/api/v1` prefix.
-   Added an empty-state test (no bound project → "No PlayCanvas project bound yet" + "Create & bind project" CTA) and a responsive viewport matrix (1920×1080, 768×1024, 390×844) with `expectNoPageHorizontalOverflow` for the binding surface.
-   Updated `metahubs-mmoomm-app-export.spec.ts` to pick the "PlayCanvas" template from the metahub-create `TemplateSelector` and to thread the `api` context into `createProjectInstanceAndBindThroughBrowser` so the project instance lookup can verify the binding; still blocked downstream on a separate PlayCanvas Editor iframe-load issue outside this branch.
-   Visual evidence captured: `projects-binding-bound-{desktop-1920,tablet-768,mobile-390}.png` (bound card with `Needs attention` chip + 1 scene/0 assets/0 scripts/0 generated artifacts + Open editor / Publish runtime / Unbind actions stacked on mobile, inlined on desktop) and `projects-binding-empty.png` (empty-state illustration + "Create & bind project" primary action). All screenshots show **Projects above Hubs** in the left menu and no raw UUIDs / JSON in the rendered surface.

## 2026-06-17 - MMOOMM-app-gate Live Regeneration (Generator + Drift + Runtime)

-   Closed the last deferred live-E2E item from the Projects-section rollout: `pnpm test:e2e:mmoomm-app-gate:local-supabase` is now **green** (generator 2/2, drift check, runtime 2/2). Tracked `tools/fixtures/metahubs-mmoomm-app-snapshot.json` regenerated with a `project` entity instance, a PlayCanvas project, a scene, and a runtime manifest.
-   Three root causes fixed along the way:
    1. **Editor fullscreen host blocked on a missing default project.** The PlayCanvas Editor fullscreen host renders the notice *"Select a default PlayCanvas project before opening the editor"* unless the editor package's `playcanvasProject.defaultProjectId` is set on the attachment config. The Projects-section flow binds a project to an entity instance, not to a package, so the test now calls a new helper `setEditorDefaultProjectThroughBrowser` (open Settings dialog → pick the bound project → PATCH `/api/v1/metahub/:metahubId/package/:attachmentId/config` → save) right after `createProjectInstanceAndBindThroughBrowser`. Subsequent settings saves (display mode = "Open separately") preserve the default project because the PATCH payload always includes the `playcanvasProject` block when the authoring surface is `playcanvasEditor`.
    2. **`publishPlayCanvasProjectThroughBrowser` had a module-scope `api` reference.** The test fn uses describe-scoped `let api: ApiContext`; the helper was calling `listEntityInstances(api, ...)` against a name that was no longer in its lexical scope. Added `api: ApiContext` as a parameter to the helper and updated the single call site.
    3. **`assertDomainModel` in `mmoommAppFixtureContract.ts` read a grouped-by-kind shape that the snapshot serializer doesn't emit.** The contract scanned `snapshot.entities?.project`, but the serializer (line 980) populates a flat `Record<id, {kind, ...}>`. Rewrote the project-instance lookup to scan the flat record, matching the existing `findEntityId` pattern used for `object` / `set` / `enumeration`.
-   Final verification: `metahubs-mmoomm-app-export.spec.ts` 2/2 passed (3.0m); `pnpm run check:mmoomm-app-fixture-drift` passed (1 project entity, 1 PlayCanvas project with 1 scene + 1 runtime manifest); `metahubs-mmoomm-app-runtime` 2/2 passed (1.7m); prettier + eslint + `pnpm docs:i18n:check` clean. Files changed: `tools/testing/e2e/specs/generators/metahubs-mmoomm-app-export.spec.ts`, `tools/testing/e2e/support/mmoommAppFixtureContract.ts`, `tools/fixtures/metahubs-mmoomm-app-snapshot.json`.

## 2026-06-18 - QA Defects Closure (Projects Entity Type)

Closed all 13 high/medium-severity issues from the QA report. No technical debt, no schema bump, no legacy code retained.

-   **Phase 1 (Defect 2 — i18n consolidation).** `consolidateMetahubsNamespace` was dropping the top-level `projects` key from the JSON files, so every `t('projects.*')` returned the raw key. Added `projects?: Record<string, unknown>` to `MetahubsBundle`/`MetahubsTranslation` and copied it into the consolidated return. New Vitest test asserts EN + RU for `projects.binding.title` and `projects.binding.actions.*` against the real consolidation, so any future regression on namespace flattening is caught.
-   **Phase 1 (Defect 5 — CI-breaker).** Removed the dead `metahub-packages-resources.spec.ts:1553-1603` block that tested the legacy `PlayCanvas projects` panel UI. Kept the kept Default-project Settings-dialog block (lines 1745+), which still uses the supported `displayMode` + `defaultProjectId` API.
-   **Phase 3 (Defect 1 — irrelevant tabs).** `PROJECT_TYPE_CAPABILITIES` now sets `actions/events/modules/layoutConfig/runtimeBehavior` to `false`; `PROJECT_TYPE_UI.resourceSurfaces` no longer carries `COMPONENTS_RESOURCE_SURFACE`; `tabs` no longer lists `'layout'` or `'modules'`. The Projects form dialog now shows only `General / Hubs / Project` tabs. The `runtimeBehavior: false` change is also required to break the `runtimeBehavior → layoutConfig` capability dependency that the validator enforces.
-   **Phase 4 (security).** Added `validateAndResolveProjectBinding` async validator that resolves the binding target by codename against the branch-scoped `_mhb_playcanvas_projects` store. Rejects: missing project, soft-deleted project, `projectId` mismatch with the live row. Called in place of the shape-only validator on `create` and `update` entity handlers, so the cascade-delete no longer silently no-ops on a stale binding. Wrapped `cascadeBoundProject` in `try/catch` with structured `log.warn(...)` so an `OptimisticLockError` or file-cleanup failure cannot surface AFTER the instance is already soft-deleted (which would leave the user with a 409 and an orphan project).
-   **Phase 5 + 6 (Defect 4 — `?projectId=` URL + mode-aware `openEditor`).** `packagesController.getAuthoringHost` accepts `?projectId=…` and uses it to override the package's `defaultProjectId` for the bridge session (non-destructive deep-link). `PlayCanvasEditorHostPage` reads `?projectId` via `useSearchParams` and forwards it to the API. `ProjectBindingPage.openEditor` reads the package's `display.mode` via a new `editorHostQuery`; `openSeparately` opens `/editor/fullscreen?projectId=…` in a new tab; `embeddedIframe` opens `/editor?projectId=…` in the same tab. The editor now pins to the bound project instead of the metahub's default, which was the cause of the "new project shows MMOOMM scene" bug.
-   **Phase 7 (Defect 3 — separate page vs tab).** Split `ProjectBindingPage.tsx` into `<ProjectBindingSurface embedded>` (reusable body) + a thin route-only `<ProjectBindingPage>` wrapper. `EntityInstanceListContent.buildFormTabs` now adds a `PlayCanvas project` tab when `showProjectBindingTab && mode === 'edit' && entityId`. The standalone route stays as a deep-link target.
-   **Phase 7.2 (Defect 4 — row menu).** Added an "Open editor" row descriptor in `renderEntityActionMenu`. Same mode-aware + `?projectId=…` flow as the card; only offered when the row carries a `config.projectBinding.projectId`.
-   **Phase 8 (test gaps).** 4 new `deleteBoundProject` tests (id-first, codename-fallback, optimistic-lock throws, missing-ids no-op); 6 new `validateAndResolveProjectBinding` tests (project-found, project-missing, projectId-mismatch, absent-binding, null-binding, malformed-payload shape-first). Backend jest: 1037/1046 passed (4 pre-existing failing suites unchanged, 0 regressions).
-   **Phase 9 (Constructor-UI reproducibility E2E — plan line 149).** New `metahub-projects-constructor.spec.ts` opens the Entity Type Constructor, enables the `projectBinding` capability toggle, saves, creates an instance of the new kind, opens its edit dialog, and asserts the form has ONLY `General / Hubs / Project` tabs (the QA brief's "no Components / Layouts / Modules / Actions / Events" requirement). This is the headline proof the constructor is universal.
-   **Phase 10 (bound-card click-through).** Extended `metahub-projects-section.spec.ts` to click every primary button (Unbind confirm + cancel, Publish runtime, Open editor) and assert the editor popup URL pins to the bound project via `?projectId=…`. The previous version never clicked any of the three buttons; the new version exercises the user flow.
-   **Phase 11 (frontend Vitest).** 4 new `ProjectBindingPage` tests: warning state ("project no longer available" branch with no Open editor / Publish buttons), error snackbar on create-and-bind failure, orphan-project rollback via `playcanvasProjectsApi.remove`, RU localization against the real consolidated namespace.
-   **Phase 12 (orphan cleanup on create+bind failure).** `createAndBindMutation.mutationFn` now wraps `writeBinding` in `try/catch`; on failure, it calls `playcanvasProjectsApi.remove(project.id, project.version)` to roll back the just-created project. Prevents the "create succeeded, bind failed, project is now orphaned" leak.
-   **Final state.** Backend jest 1037/1046 passed (4 pre-existing failing suites unchanged, 0 regressions). Frontend vitest 347/347 passed. `pnpm run build:e2e` green. `npx eslint` 0 errors on all touched files. `npx prettier --check` clean. Live E2E in progress against the local Supabase; `metahub-projects-section.spec.ts` is being re-run to confirm the new click-through assertions are green.
