...
-   Runtime sync must fail closed when `_app_modules` bootstrap or persistence fails.
...
-   Embedded LMS `quiz_ref` rendering now depends on explicit `quizId` context passed into `QuizWidget`; ambient module selection alone is not a sufficient contract when module content can reference multiple quizzes.
...
-   Import proof: `snapshot-import-quiz-runtime.spec.ts` validates browser import, restored design-time modules, application creation, and the full EN/RU quiz runtime contract.
...
-   Snapshot export emits `sharedComponents`, `sharedFixedValues`, `sharedOptionValues`, and `sharedEntityOverrides`.
