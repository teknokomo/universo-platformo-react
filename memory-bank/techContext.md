...
-   **Frontend page-shell spacing seam**: `pageSpacing.ts` is now the shared source of truth for route-aware metahub inset decisions through `isMetahubShellRoute(...)`, `getPageGutterPx(...)`, and `getHeaderInsetPx(...)`. `MainLayoutMUI` still applies the narrower metahub shell gutter (`px: { xs: 1, md: 1.5 }`) while the default workspace shell keeps `px: { xs: 1.5, md: 2 }`, but metahubs frontend pages should neither widen standalone content with local negative `mx` bleed offsets nor add extra horizontal `px` wrappers on top of that shell gutter. Metahub loading states now align through `SkeletonGrid insetMode='content'` plus the stable `skeleton-grid` test id rather than repeated local `mx={0}` overrides.
-   **Shell-spacing proof nuance**: do not remove the shared `Header` route-aware inset merely because `MainLayoutMUI` already owns shell `px`; the targeted Chromium `metahub-shell-spacing.spec.ts` proof currently fails with a `16px` breadcrumb/title drift when that inset is removed.
...
-   **Compatibility-read seam**: legacy-compatible hub/set/enumeration list routes must validate explicit V2 `kindKey` requests but then widen the read scope back to the full compatible kind array, so fresh-import V2 surfaces continue showing legacy rows instead of filtering down to the custom kind only.
