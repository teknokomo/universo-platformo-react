...
> Goal: Update `@universo-react/types` to reflect the new unified entity model before touching backend/frontend code.
...
**File**: `packages/universo-react-types/base/src/common/entityTypeDefinition.ts`
...
> **TZ #3 rationale**: _"я пока не вижу необходимости их как-то специальным образом помечать в таблице типов сущностей, поэтому нужно убрать колонку 'Источник'"_. No replacement field is needed.
...
**File**: `packages/universo-react-types/base/src/common/builtinEntityTypes.ts` → DELETE entirely

**Actions**:

-   Delete `builtinEntityTypes.ts` from `@universo-react/types` — the `BUILTIN_ENTITY_TYPE_REGISTRY` must not exist as a runtime registry
...
-   Entity type preset definitions move to backend-only seed data: `packages/universo-react-metahubs-backend/base/src/domains/templates/data/presets/` (already there, just remove `@universo-react/types` dependency)
...
**File**: `packages/universo-react-types/base/src/common/metahubs.ts` or `entityTypeDefinition.ts`
...
**Files**: `packages/universo-react-types/base/src/index.ts`, other barrel files
...
-   [ ] Step 1.3: Delete `builtinEntityTypes.ts` from `@universo-react/types` — presets are backend-only seed data
...
-   [ ] Build `@universo-react/types` to verify: `pnpm --filter @universo-react/types build`
...
**Files**: All preset files in `packages/universo-react-metahubs-backend/base/src/domains/templates/data/presets/`
...
-   Preset definitions now ONLY exist here as builtin seed sources (not in `@universo-react/types`); they seed the existing template catalog and are later consumed through that catalog during metahub creation
-   Move component/UI definitions that were previously in `@universo-react/types` `CATALOG_TYPE`, `HUB_TYPE`, etc. into these preset files directly
...
-   `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/MetahubList.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/metahubListUtils.ts`
...
**File**: `packages/universo-react-metahubs-backend/base/src/domains/metahubs/controllers/metahubsController.ts`
...
-   Preserve derived `hubsCount` / `catalogsCount` response fields temporarily only as frontend compatibility aliases until `MetahubBoard.tsx`, `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/api/metahubs.ts`, and related board tests switch to `entityCounts`
...
-   `packages/universo-react-metahubs-frontend/base/src/domains/entities/ui/EntityInstanceList.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/components/TargetEntitySelector.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/domains/entities/api/entityTypes.ts`
...
```
packages/universo-react-metahubs-frontend/base/src/domains/catalogs/   ← REMOVE
packages/universo-react-metahubs-frontend/base/src/domains/hubs/       ← REMOVE
packages/universo-react-metahubs-frontend/base/src/domains/sets/       ← REMOVE
packages/universo-react-metahubs-frontend/base/src/domains/enumerations/ ← REMOVE
```

**Files to delete**:

-   `packages/universo-react-metahubs-frontend/base/src/domains/shared/legacyCompatibleRoutePaths.ts`
...
```
packages/universo-react-metahubs-backend/base/src/domains/hubs/           ← REMOVE entirely
packages/universo-react-metahubs-backend/base/src/domains/catalogs/       ← REMOVE entirely
packages/universo-react-metahubs-backend/base/src/domains/sets/           ← REMOVE entirely
packages/universo-react-metahubs-backend/base/src/domains/enumerations/   ← REMOVE entirely
```
...
**New file**: `packages/universo-react-metahubs-backend/base/src/domains/entities/services/HubBehaviorService.ts`
...
**File**: `packages/universo-react-metahubs-backend/base/src/domains/metahubs/router.ts`
...
-   `packages/universo-react-metahubs-backend/base/src/domains/shared/legacyCompatibility.ts` — DELETE
-   `packages/universo-react-metahubs-backend/base/src/domains/catalogs/services/catalogCompatibility.ts` — DELETE
-   `packages/universo-react-metahubs-backend/base/src/domains/shared/entityTypeResolver.ts` — refactor to DB-only resolution (remove `resolveBuiltin()` method)

### Step 4.6: Simplify EntityTypeService

**File**: `packages/universo-react-metahubs-backend/base/src/domains/entities/services/EntityTypeService.ts`
...
**File**: `packages/universo-react-metahubs-backend/base/src/domains/entities/controllers/entityTypesController.ts`
...
```typescript
const buildEntityTypeMenuItems = (metahubId: string, entityTypes: MetahubMenuEntityType[]): TemplateMenuItem[] => {
    return entityTypes
...
-   `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/MetahubList.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/metahubListUtils.ts`
-   `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/__tests__/MetahubList.test.tsx`
...
**File**: `packages/universo-react-schema-ddl/base/src/legacyCompatibleKinds.ts`
...
export const isNonPhysicalEntity = (entity: { kind: string }): boolean => ['hub', 'set', 'enumeration'].includes(entity.kind)
```
...
export const RUNTIME_SECTION_FILTER_SQL = (kindColumn: string) => `${kindColumn} NOT IN ('hub', 'set', 'enumeration')`
```
...
-   Remove `isBuiltin` from `MetahubEntityTypeDefinitionSnapshot` interface in `@universo-react/types`
...
**File**: `packages/universo-react-types/base/src/common/metahubs.ts` (METAHUB_SETTINGS_REGISTRY)
...
-   Update `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/__tests__/MetahubBoard.test.tsx` and `packages/universo-react-metahubs-backend/base/src/tests/routes/metahubBoardSummary.test.ts` for the new `entityCounts` + compatibility-alias contract
-   Update `packages/universo-react-metahubs-frontend/base/src/domains/metahubs/ui/__tests__/MetahubList.test.tsx` for preset-driven create options
...
-   `packages/universo-react-metahubs-backend/base/src/domains/hubs/` (controllers, routes, services, tests)
-   `packages/universo-react-metahubs-backend/base/src/domains/catalogs/` (controllers, routes, services, tests)
-   `packages/universo-react-metahubs-backend/base/src/domains/sets/` (controllers, routes, services, tests)
-   `packages/universo-react-metahubs-backend/base/src/domains/enumerations/` (controllers, routes, services, tests)
-   `packages/universo-react-metahubs-backend/base/src/domains/shared/legacyCompatibility.ts`

**Frontend legacy domains**:

-   `packages/universo-react-metahubs-frontend/base/src/domains/catalogs/`
-   `packages/universo-react-metahubs-frontend/base/src/domains/hubs/`
-   `packages/universo-react-metahubs-frontend/base/src/domains/sets/`
-   `packages/universo-react-metahubs-frontend/base/src/domains/enumerations/`
-   `packages/universo-react-metahubs-frontend/base/src/domains/shared/legacyCompatibleRoutePaths.ts`
...
-   `packages/universo-react-types/base/src/common/builtinEntityTypes.ts` — DELETE entirely (presets are backend-only seed data)
...
-   `packages/universo-react-metahubs-frontend/base/src/domains/entities/ui/renderers/CatalogListRenderer.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/domains/entities/ui/renderers/HubListRenderer.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/domains/entities/ui/renderers/SetListRenderer.tsx`
-   `packages/universo-react-metahubs-frontend/base/src/domains/entities/ui/renderers/EnumerationListRenderer.tsx`
...
**@universo-react/types**: `entityTypeDefinition.ts`, `metahubs.ts`, `entityComponents.ts`, `index.ts`
