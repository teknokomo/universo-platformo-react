...
-   private Manager brief, `private Manager projects`, `private Manager ecosystem`, and `private Manager prompts`; only exact archived files listed in the rename-audit allowlist may retain historical old names.
...
rg -n "@universo/|packages/(admin-|applications-|apps-template-mui|auth-|extension-sdk|metahubs-|metapanel-|migration-guard-shared|modules-engine|profile-|schema-ddl|start-|universo-)" \
  ...
  .github .kiro .agents .codex .gemini .qoder .claude private Manager .vscode tools docs memory-bank packages \
...
const toNewDirectoryName = (directoryName) => {
    const baseName = directoryName.startsWith('universo-') ? directoryName.slice('universo-'.length) : directoryName
...
-   [ ] Update private Manager brief, `private Manager projects`, `private Manager ecosystem`, and `private Manager prompts` wherever files are active current guidance or current planning context. Historical files may keep old examples only when their exact path is present in `memory-bank/implementation/packages-naming-legacy-reference-allowlist.json`.
...
-   [ ] Use a tracked-file scanner based on `git ls-files`, the Phase 1 ledger, and the explicit allowlist file. The scanner must:
    -   scan active tracked files, including dot-directories such as `.gemini`, `.qoder`, `.claude`, `.vscode`, `private Manager`, `.github`, `.kiro`, and root files such as `AGENTS.md`, `.dockerignore`, and `pnpm-workspace.yaml`;
