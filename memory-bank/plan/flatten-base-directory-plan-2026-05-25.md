...
-   [ ] 5. Rewrite active path references with a scoped script, then review.
    ...
    -   Treat `memory-bank/**` and `private Manager files` as historical unless a current
    ...
    -   Treat `private Manager files` as a mixed area: archived specs can remain historical,
        but operational project context such as `private Manager projects` must be
