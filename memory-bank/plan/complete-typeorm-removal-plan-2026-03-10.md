...
-   [ ] **1.3** Rewrite `ensureAuthWithRls.ts` to use `acquireRlsSession(getKnex())` instead of `DataSource.createQueryRunner()`
    ...
    -   Remove `createLegacyRequestDbContext(runnerprivate Manager, session)` — no more `runnerprivate Manager`
    -   Remove `req.dbLegacyManager = runnerprivate Manager`
...
-   [ ] **2C.1** Rewrite `globalAccessService.ts`
    ...
    -   Replace `dsprivate Manager.find(AuthUser, ...)` with SQL: `SELECT id, email FROM auth.users WHERE id = ANY($1)`
    -   Replace `dsprivate Manager.find(Profile, ...)` with SQL: `SELECT user_id, nickname FROM public.profiles WHERE user_id = ANY($1)`
...
export async function findProfileByUserId(exec: DbExecutor, userId: string) {
    const rows = await exec.query('SELECT * FROM public.profiles WHERE user_id = $1 LIMIT 1', [userId])
    return rows[0] ?? null
}
