...
- [ ] Update `memory-bank/currentResearch.md`, `memory-bank/progress.md`, or `memory-bank/tasks.md` only where they describe the current state. Do not rewrite completed historical research/plan artifacts unless the final stale-reference policy requires zero old-name hits outside private manager-only files.
...
- [ ] If old names remain in historical Memory Bank artifacts, either:
  ...
  - update them if the implementation acceptance criteria require zero old-name hits outside private manager-only files.
...
- Historical Memory Bank files can legitimately contain old names. Decide during implementation whether acceptance is "no old active references" or "no old references anywhere outside private manager-only files".
